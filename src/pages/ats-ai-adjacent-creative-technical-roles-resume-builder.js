...
const Resume = ({ seoData, buildTimestamp }) => {
  ...
  const safeFaqDates = faqDates || Array(3).fill(freshnessIndicator);

  const defaultExperience = () => ({ company: '', position: '', team: '', startDate: '', endDate: '', description: '', isEditing: false, editIndex: null });
  const defaultEducation = () => ({ institution: '', degree: '', concentration: '', startDate: '', endDate: '', isEditing: false, editIndex: null });
  const defaultSpecialty = () => ({ name: '', isEditing: false, editIndex: null });
  const defaultCertification = () => ({ name: '', issuingAuthority: '', credentialId: '', issueDate: '', isEditing: false, editIndex: null });
  const defaultProject = () => ({ name: '', technologies: '', description: '', isEditing: false, editIndex: null });
  const defaultTool = () => ({ name: '', proficiency: '', isEditing: false, editIndex: null });
  ...
  const [formData, setFormData] = useState({ fullName: '', email: '', phone: '', location: '', title: '', portfolio: '', summary: '', experience: [], education: [], specialties: [], certifications: [], projects: [], tools: [], languages: [], socialLinks: [] });
  ...
  const hasContent = () => formData.fullName || formData.email || formData.summary || formData.socialLinks.length > 0 || formData.experience.length > 0 || formData.education.length > 0 || formData.specialties.length > 0 || formData.certifications.length > 0 || formData.projects.length > 0 || formData.tools.length > 0 || formData.languages.length > 0;
    ...
    return (
      <div className="cf-universal-template">
        <header className="cf-resume-header">
          ...
          {formData.title && <div className="cf-professional-title" style={{ fontSize: `${fontSizes.jobTitle}pt` }}>{formData.title}</div>}
          <div className="cf-contact-row">
            ...
            {formData.portfolio && <div className="cf-contact-item" style={{ fontSize: `${fontSizes.contactInfo}pt` }}><FiGlobe /> {formatSocialUrl(formData.portfolio)}</div>}
        ...
        {hasExperience && <section className="cf-section"><h2 className="cf-section-title" style={{ fontSize: `${fontSizes.sectionTitle}pt` }}>TECHNICAL EXPERIENCE</h2>{formData.experience.map((exp, i) => (<div key={i} className="cf-experience-item"><div className="cf-experience-header"><h3 style={{ fontSize: `${fontSizes.jobTitle}pt` }}>{exp.position}</h3><p className="cf-company" style={{ fontSize: `${fontSizes.company}pt` }}>{exp.company}{exp.team && ` – ${exp.team}`} | {exp.startDate} – {exp.endDate || 'Present'}</p></div><ul className="cf-bullet-list">{exp.description.split('\n').filter(l => l.trim()).map((line, j) => <li key={j} style={{ fontSize: `${fontSizes.bulletText}pt` }}>{line}</li>)}</ul></div>))}</section>}
        {hasProjects && <section className="cf-section"><h2 className="cf-section-title" style={{ fontSize: `${fontSizes.sectionTitle}pt` }}>TECHNICAL PROJECTS</h2>{formData.projects.map((project, i) => (<div key={i} className="cf-project-item" style={{ fontSize: `${fontSizes.regularText}pt` }}><strong style={{ fontWeight: 700 }}>{project.name}</strong>{project.technologies && ` – ${project.technologies}`}{project.description && ` – ${project.description}`}</div>))}</section>}
        {hasEducation && <section className="cf-section"><h2 className="cf-section-title" style={{ fontSize: `${fontSizes.sectionTitle}pt` }}>EDUCATION</h2>{formData.education.map((edu, i) => (<div key={i} className="cf-education-item"><h3 style={{ fontSize: `${fontSizes.degree}pt` }}>{edu.degree}{edu.concentration && ` – ${edu.concentration}`}</h3><p className="cf-institution-text" style={{ fontSize: `${fontSizes.institution}pt` }}>{edu.institution} | <span className="cf-institution-date" style={{ fontSize: `${fontSizes.institutionDate}pt` }}>{edu.startDate} – {edu.endDate || 'Expected'}</span></p></div>))}</section>}
        {hasTools && <section className="cf-section"><h2 className="cf-section-title" style={{ fontSize: `${fontSizes.sectionTitle}pt` }}>TECHNICAL SKILLS</h2><ul className="cf-skills-list">{formData.tools.map((t, i) => (<li key={i} style={{ fontSize: `${fontSizes.skillText}pt` }}>{t.name}{t.proficiency && ` (${t.proficiency})`}</li>))}</ul></section>}
        {hasSpecialties && <section className="cf-section"><h2 className="cf-section-title" style={{ fontSize: `${fontSizes.sectionTitle}pt` }}>AI/ML SPECIALTIES</h2><ul className="cf-skills-list">{formData.specialties.map((s, i) => <li key={i} style={{ fontSize: `${fontSizes.skillText}pt` }}>{s.name}</li>)}</ul></section>}
        {hasCertifications && <section className="cf-section"><h2 className="cf-section-title" style={{ fontSize: `${fontSizes.sectionTitle}pt` }}>CERTIFICATIONS</h2>{formData.certifications.map((c, i) => (<div key={i} className="cf-cert-item" style={{ fontSize: `${fontSizes.certificationText}pt` }}><strong style={{ fontWeight: 700 }}>{c.name}</strong>{c.issuingAuthority && ` – ${c.issuingAuthority}`}{c.credentialId && ` (ID: ${c.credentialId})`}{c.issueDate && ` – Issued: ${c.issueDate}`}</div>))}</section>}
      ...
      <div className="cf-resume-builder" lang="en-US">
        ...
        <nav className="cf-breadcrumb" aria-label="Breadcrumb">
          <ol itemScope itemType="https://schema.org/BreadcrumbList">
            <li itemProp="itemListElement" itemScope itemType="https://schema.org/ListItem">
              <Link href="https://professionalresumefree.com" className="cf-breadcrumb-link" itemProp="item">
                <FiHome className="cf-breadcrumb-icon"/><span className="cf-breadcrumb-text" itemProp="name">Home</span>
              </Link>
              <meta itemProp="position" content="1" />
            </li>
            <li className="cf-breadcrumb-separator" aria-hidden="true"><FiChevronRightIcon/></li>
            <li itemProp="itemListElement" itemScope itemType="https://schema.org/ListItem">
              <Link href="/ats-ai-adjacent-creative-technical-roles-resume-builder" className="cf-breadcrumb-link" itemProp="item">
                <span className="cf-breadcrumb-text" itemProp="name">Free Technical Templates</span>
              </Link>
          ...
          <div className="cf-container">
            <div className="cf-hero-content">
              ...
              <div className="cf-hero-stats">
                ...
                <div className="cf-stat-item"><span className="cf-stat-number">3M+</span><span className="cf-stat-label">Users Worldwide</span></div>
              </div>
              <div className="cf-industry-badges"><div className="cf-badge-grid"><span className="cf-badge-item"><FiCpu/> AI Engineer</span><span className="cf-badge-item"><FiDatabase/> Data Scientist</span><span className="cf-badge-item"><FiFeather/> Creative Tech</span><span className="cf-badge-item"><FiTrendingUp/> ML Researcher</span><span className="cf-badge-item"><FiCode/> Software Eng</span><span className="cf-badge-item"><FiCloud/> Cloud Architect</span></div></div>
              <div style={{ marginTop: '1.5rem', fontSize: '0.8rem', color: '#6b7280' }}>Last updated: {safeCurrentDate}</div>
            </div>
          </div>
        </section>
        ...
        <div className="cf-layout">
          <div className="cf-preview-section">
            <div className="cf-preview-header">
              <div className="cf-preview-actions">
                <button onClick={() => setShowFullPreview(!showFullPreview)} className="cf-preview-btn"><FiEye/> {showFullPreview ? 'Hide Full Preview' : 'Show Full Preview'}</button>
                <button onClick={generatePDF} className="cf-download-btn" disabled={isGeneratingPDF || !hasContent()}><FiDownload/>{isGeneratingPDF ? 'Generating PDF...' : 'Download PDF'}</button>
              </div>
            </div>
            <div className={`cf-preview-container ${showFullPreview ? 'cf-full-preview' : ''}`}>
              <div className="cf-resume-card">
                <div className="cf-preview-content">
                  <div className="cf-resume-preview" ref={resumeRef}><AITechnicalTemplate formData={formData}/></div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="cf-form-section">
            <div className="cf-form-nav" role="tablist" aria-label="Resume builder sections">
              {[{id:'personal',label:'Personal',icon:<FiUser/>},{id:'experience',label:'Experience',icon:<FiBriefcase/>},{id:'projects',label:'Projects',icon:<FiCode/>},{id:'education',label:'Education',icon:<FiBook/>},{id:'skills',label:'Skills',icon:<FiTool/>},{id:'settings',label:'Font Settings',icon:<FiSettings/>}].map(item => (
                <button key={item.id} className={`cf-nav-btn ${activeSection===item.id?'cf-nav-btn-active':''}`} onClick={()=>setActiveSection(item.id)} role="tab" aria-selected={activeSection===item.id}>{item.icon}<span>{item.label}</span></button>
              ))}
            </div>
            ...
            <div className="cf-form-content">
              {activeSection==='personal'&&(
                <div className="cf-form-section-content">
                  ...
                  <div className="cf-form-card">
                    <div className="cf-form-group"><label className="cf-form-label">Full Name*<input type="text" name="fullName" value={formData.fullName} onChange={handleInputChange} placeholder="Alex Chen" required className="cf-form-input"/></label><label className="cf-form-label">Professional Title<input type="text" name="title" value={formData.title} onChange={handleInputChange} placeholder="Senior AI Engineer | Machine Learning Specialist" className="cf-form-input"/></label></div>
                    <div className="cf-form-group"><label className="cf-form-label">Email*<input type="email" name="email" value={formData.email} onChange={handleInputChange} placeholder="alex@example.com" required className="cf-form-input"/></label><label className="cf-form-label">Phone<input type="tel" name="phone" value={formData.phone} onChange={handleInputChange} placeholder="(555) 123-4567" className="cf-form-input"/></label></div>
                    <div className="cf-form-group"><label className="cf-form-label">Location<input type="text" name="location" value={formData.location} onChange={handleInputChange} placeholder="San Francisco, CA | Remote" className="cf-form-input"/></label><label className="cf-form-label">Portfolio Website<input type="url" name="portfolio" value={formData.portfolio} onChange={handleInputChange} placeholder="https://alexchen.dev" className="cf-form-input"/></label></div>
                  </div>
                  <div className="cf-form-card">
                    <label className="cf-form-label">Technical Summary*<textarea name="summary" value={formData.summary} onChange={handleInputChange} placeholder="Senior AI Engineer with 5+ years of experience developing machine learning solutions at scale. Specialized in computer vision, natural language processing, and reinforcement learning. Proven track record of delivering AI products that improve efficiency by 40+%. Passionate about ethical AI and open-source contributions." required className="cf-form-textarea" rows="6"/><div className="cf-char-count">{formData.summary.length}/500 characters</div></label>
                  </div>
                  <div className="cf-form-card">
                    <h4 className="cf-sub-section-title"><FiGlobe/> Technical Links</h4><p className="cf-section-desc">Add your technical profiles (GitHub, LinkedIn, Kaggle, etc.)</p>
                    <div className="cf-social-input">
                      <select value={currentSocialLink.platform} onChange={e=>setCurrentSocialLink({...currentSocialLink,platform:e.target.value})} className="cf-form-select"><option value="">Select Platform</option><option value="GitHub">GitHub</option><option value="LinkedIn">LinkedIn</option><option value="Kaggle">Kaggle</option><option value="Portfolio">Portfolio</option><option value="Medium">Medium/Blog</option><option value="DockerHub">DockerHub</option><option value="ResearchGate">ResearchGate</option></select>
                    ...
                    <div className="cf-items-list">{formData.socialLinks.length===0?<p className="cf-empty-msg">No technical links added yet</p>:formData.socialLinks.map((link,i)=>(<div key={i} className="cf-list-item"><div className="cf-item-content"><span className="cf-item-platform">{link.platform}</span><span className="cf-item-url">{formatSocialUrl(link.url)}</span></div><div className="cf-item-actions"><button onClick={()=>editSocialLink(i)} className="cf-edit-btn" aria-label={`Edit ${link.platform} link`}><FiEdit2/></button><button onClick={()=>deleteSocialLink(i)} className="cf-delete-btn" aria-label={`Delete ${link.platform} link`}><FiTrash2/></button></div></div>))}</div>
                  </div>
                </div>
              )}
              ...
              {activeSection==='experience'&&(
                <div className="cf-form-section-content">
                  <h3 className="cf-form-section-title"><FiBriefcase/> Technical Experience</h3><p className="cf-section-desc">List your technical positions in reverse chronological order (most recent first)</p>
                  <div className="cf-form-card">
                    <div className="cf-form-group"><label className="cf-form-label">Position Title*<input value={currentExperience.position} onChange={e=>setCurrentExperience({...currentExperience,position:e.target.value})} placeholder="Senior AI Engineer" required className="cf-form-input"/></label><label className="cf-form-label">Company*<input value={currentExperience.company} onChange={e=>setCurrentExperience({...currentExperience,company:e.target.value})} placeholder="Tech Innovations Inc." required className="cf-form-input"/></label></div>
                    <label className="cf-form-label">Team / Department / Focus Area<input value={currentExperience.team} onChange={e=>setCurrentExperience({...currentExperience,team:e.target.value})} placeholder="Machine Learning Research Team" className="cf-form-input"/></label>
                    <div className="cf-form-group"><label className="cf-form-label">Start Date*<input type="text" placeholder="Month Year (e.g., January 2022)" value={currentExperience.startDate} onChange={e=>setCurrentExperience({...currentExperience,startDate:e.target.value})} required className="cf-form-input"/></label><label className="cf-form-label">End Date<input type="text" placeholder="Month Year or Present" value={currentExperience.endDate} onChange={e=>setCurrentExperience({...currentExperience,endDate:e.target.value})} className="cf-form-input"/></label></div>
                    <label className="cf-form-label">Technical Responsibilities & Achievements*<textarea value={currentExperience.description} onChange={e=>setCurrentExperience({...currentExperience,description:e.target.value})} placeholder="• Developed and deployed computer vision models that improved object detection accuracy by 35%\n• Led a team of 4 ML engineers on a natural language processing project serving 2M+ users\n• Implemented MLOps pipeline reducing model deployment time from 2 weeks to 2 days\n• Published research paper on novel reinforcement learning approach at NeurIPS 2023\n• Mentored 3 junior engineers and conducted technical interviews for AI positions" required className="cf-form-textarea" rows="8"/><div className="cf-char-count">{currentExperience.description.length}/2000 characters</div></label>
                  ...
                  <div className="cf-form-card"><h4 className="cf-sub-section-title">Your Technical Experience</h4>{formData.experience.length===0?<p className="cf-empty-msg">No technical experience added yet</p>:<div className="cf-items-list">{formData.experience.map((exp,i)=>(<div key={i} className="cf-list-item"><div className="cf-item-content"><div className="cf-item-header"><strong className="cf-item-title">{exp.position}</strong><span className="cf-item-subtitle">at {exp.company}</span></div><div className="cf-item-meta"><span>{exp.startDate} – {exp.endDate||'Present'}</span>{exp.team&&<span>{exp.team}</span>}</div><div className="cf-item-desc">{exp.description.split('\n').filter(l=>l.trim()).map((line,j)=><p key={j} className="cf-bullet-point">• {line}</p>)}</div></div><div className="cf-item-actions"><button onClick={()=>editExperience(i)} className="cf-edit-btn" aria-label={`Edit ${exp.position} experience`}><FiEdit2/></button><button onClick={()=>deleteExperience(i)} className="cf-delete-btn" aria-label={`Delete ${exp.position} experience`}><FiTrash2/></button></div></div>))}</div>}</div>
                </div>
              )}

              {activeSection==='projects'&&(
                <div className="cf-form-section-content">
                  <h3 className="cf-form-section-title"><FiCode/> Technical Projects</h3><p className="cf-section-desc">Showcase your AI/ML projects, open-source contributions, and technical side projects</p>
                  <div className="cf-form-card">
                    <h4 className="cf-sub-section-title">Technical Projects</h4><p className="cf-section-desc">Add your AI/ML projects, research, or open-source contributions</p>
                    <div className="cf-skills-input">
                      <div className="cf-form-group"><label className="cf-form-label">Project Name*<input value={currentProject.name} onChange={e=>setCurrentProject({...currentProject,name:e.target.value})} placeholder="Real-time Object Detection System" className="cf-form-input"/></label><label className="cf-form-label">Technologies Used<input value={currentProject.technologies} onChange={e=>setCurrentProject({...currentProject,technologies:e.target.value})} placeholder="Python, TensorFlow, OpenCV, Docker" className="cf-form-input"/></label></div>
                      <label className="cf-form-label">Description / Key Results<textarea value={currentProject.description} onChange={e=>setCurrentProject({...currentProject,description:e.target.value})} placeholder="Built a real-time object detection system achieving 95% accuracy. Deployed on edge devices processing 30 FPS. Open-source project with 500+ GitHub stars." className="cf-form-textarea" rows="4"/></label>
                    ...
                    <div className="cf-items-list">{formData.projects.map((p,i)=>(<div key={i} className="cf-list-item"><div className="cf-item-content"><strong className="cf-item-title">{p.name}</strong>{p.technologies&&<span className="cf-item-subtitle"> – {p.technologies}</span>}{p.description&&<div className="cf-item-desc">{p.description}</div>}</div><div className="cf-item-actions"><button onClick={()=>editProject(i)} className="cf-edit-btn"><FiEdit2/></button><button onClick={()=>deleteProject(i)} className="cf-delete-btn"><FiTrash2/></button></div></div>))}{formData.projects.length===0&&<p className="cf-empty-msg">No projects added yet</p>}</div>
                  </div>
                  <div className="cf-form-card">
                    <h4 className="cf-sub-section-title"><FiActivity/> AI/ML Specialties</h4><p className="cf-section-desc">List your areas of AI/ML expertise and research focus</p>
                    <div className="cf-skills-input"><input value={currentSpecialty.name} onChange={e=>setCurrentSpecialty({...currentSpecialty,name:e.target.value})} placeholder="Computer Vision, Natural Language Processing, Reinforcement Learning" className="cf-form-input"/><div className="cf-form-actions"><button onClick={addSpecialty} className="cf-add-btn" disabled={!currentSpecialty.name.trim()}><FiPlus/> {currentSpecialty.isEditing?'Update Specialty':'Add Specialty'}</button>{currentSpecialty.isEditing&&<button onClick={()=>setCurrentSpecialty(defaultSpecialty())} className="cf-cancel-btn"><FiX/> Cancel</button>}</div></div>
                  ...
                  <div className="cf-form-card">
                    <h4 className="cf-sub-section-title"><FiAward/> Technical Certifications</h4><p className="cf-section-desc">Add your technical certifications and credentials</p>
                    <div className="cf-skills-input">
                      <div className="cf-form-group"><label className="cf-form-label">Certification Name*<input value={currentCertification.name} onChange={e=>setCurrentCertification({...currentCertification,name:e.target.value})} placeholder="AWS Certified Machine Learning - Specialty" className="cf-form-input"/></label><label className="cf-form-label">Issuing Authority<input value={currentCertification.issuingAuthority} onChange={e=>setCurrentCertification({...currentCertification,issuingAuthority:e.target.value})} placeholder="Amazon Web Services" className="cf-form-input"/></label></div>
                      <div className="cf-form-group"><label className="cf-form-label">Credential ID<input value={currentCertification.credentialId} onChange={e=>setCurrentCertification({...currentCertification,credentialId:e.target.value})} placeholder="MLS-001" className="cf-form-input"/></label><label className="cf-form-label">Issue Date<input value={currentCertification.issueDate} onChange={e=>setCurrentCertification({...currentCertification,issueDate:e.target.value})} placeholder="Month Year" className="cf-form-input"/></label></div>
                    ...
                    <div className="cf-items-list">{formData.certifications.map((c,i)=>(<div key={i} className="cf-list-item"><div className="cf-item-content"><strong className="cf-item-title">{c.name}</strong>{c.issuingAuthority&&<span className="cf-item-subtitle"> – {c.issuingAuthority}</span>}{c.credentialId&&<span className="cf-item-meta"> (#{c.credentialId})</span>}{c.issueDate&&<span className="cf-item-meta"> – Issued: {c.issueDate}</span>}</div><div className="cf-item-actions"><button onClick={()=>editCertification(i)} className="cf-edit-btn"><FiEdit2/></button><button onClick={()=>deleteCertification(i)} className="cf-delete-btn"><FiTrash2/></button></div></div>))}{formData.certifications.length===0&&<p className="cf-empty-msg">No certifications added yet</p>}</div>
                  </div>
                </div>
              )}
              ...
              {activeSection==='education'&&(
                <div className="cf-form-section-content">
                  <h3 className="cf-form-section-title"><FiBook/> Education</h3><p className="cf-section-desc">List your technical education, degrees, and relevant coursework</p>
                  <div className="cf-form-card">
                    <div className="cf-form-group"><label className="cf-form-label">Institution*<input value={currentEducation.institution} onChange={e=>setCurrentEducation({...currentEducation,institution:e.target.value})} placeholder="Stanford University" required className="cf-form-input"/></label><label className="cf-form-label">Degree*<input value={currentEducation.degree} onChange={e=>setCurrentEducation({...currentEducation,degree:e.target.value})} placeholder="Master of Science in Computer Science" required className="cf-form-input"/></label></div>
                    <label className="cf-form-label">Concentration / Specialization<input value={currentEducation.concentration} onChange={e=>setCurrentEducation({...currentEducation,concentration:e.target.value})} placeholder="Artificial Intelligence & Machine Learning" className="cf-form-input"/></label>
                  ...
                  <div className="cf-form-card"><h4 className="cf-sub-section-title">Your Education</h4>{formData.education.length===0?<p className="cf-empty-msg">No education added yet</p>:<div className="cf-items-list">{formData.education.map((edu,i)=>(<div key={i} className="cf-list-item"><div className="cf-item-content"><div className="cf-item-header"><strong className="cf-item-title">{edu.degree}</strong>{edu.concentration&&<span className="cf-item-subtitle"> – {edu.concentration}</span>}</div><div className="cf-item-meta"><span>{edu.institution}</span><span>{edu.startDate} – {edu.endDate||'Expected'}</span></div></div><div className="cf-item-actions"><button onClick={()=>editEducation(i)} className="cf-edit-btn" aria-label={`Edit ${edu.degree}`}><FiEdit2/></button><button onClick={()=>deleteEducation(i)} className="cf-delete-btn" aria-label={`Delete ${edu.degree}`}><FiTrash2/></button></div></div>))}</div>}</div>
                </div>
              )}

              {activeSection==='skills'&&(
                <div className="cf-form-section-content">
                  <h3 className="cf-form-section-title"><FiTool/> Technical Skills</h3>
                  <div className="cf-form-card">
                    <h4 className="cf-sub-section-title">Technical Tools & Technologies</h4><p className="cf-section-desc">List your programming languages, frameworks, libraries, and tools</p>
                    <div className="cf-skills-input">
                      <div className="cf-form-group"><label className="cf-form-label">Technology/Tool*<input value={currentTool.name} onChange={e=>setCurrentTool({...currentTool,name:e.target.value})} placeholder="Python" className="cf-form-input"/></label><label className="cf-form-label">Proficiency Level<input value={currentTool.proficiency} onChange={e=>setCurrentTool({...currentTool,proficiency:e.target.value})} placeholder="Expert, Advanced, Intermediate" className="cf-form-input"/></label></div>
                  ...
                  <div className="cf-form-card">
                    ...
                    <div className="cf-skills-input">
                      <div className="cf-form-group"><label className="cf-form-label">Language*<input value={currentLanguage.name} onChange={e=>setCurrentLanguage({...currentLanguage,name:e.target.value})} placeholder="English" className="cf-form-input"/></label><label className="cf-form-label">Proficiency Level<input value={currentLanguage.proficiency} onChange={e=>setCurrentLanguage({...currentLanguage,proficiency:e.target.value})} placeholder="Native, Fluent, Professional" className="cf-form-input"/></label></div>
              ...
              {activeSection==='settings'&&(
                <div className="cf-form-section-content">
                  <h3 className="cf-form-section-title"><FiSettings/> Font Size Settings</h3><p className="cf-section-desc">Customize font sizes for your resume PDF. All sizes are in points (pt).</p>
                  <div className="cf-form-card">
                    <div className="cf-font-grid">
                      {[{key:'name',label:'Name',min:8,max:24},{key:'sectionTitle',label:'Section Titles',min:6,max:18},{key:'jobTitle',label:'Job Titles',min:6,max:16},{key:'degree',label:'Degrees',min:6,max:16},{key:'institution',label:'Company/Institution',min:6,max:14},{key:'institutionDate',label:'Institution Dates',min:4,max:12},{key:'regularText',label:'Regular Text',min:6,max:14},{key:'bulletText',label:'Bullet Points',min:6,max:14},{key:'contactInfo',label:'Contact Info',min:6,max:12},{key:'skillText',label:'Skills Text',min:6,max:12},{key:'certificationText',label:'Certification Text',min:6,max:14}].map(({key,label,min,max})=>(<div key={key} className="cf-font-control"><label className="cf-font-label"><span>{label}</span><span className="cf-font-value">{fontSizes[key]}pt</span></label><input type="range" min={min} max={max} value={fontSizes[key]} onChange={e=>handleFontSizeChange(key,e.target.value)} className="cf-font-slider"/></div>))}
                    </div>
        ...
        {showFullPreview&&(<div className="cf-modal" onClick={()=>setShowFullPreview(false)}><div className="cf-modal-content" onClick={e=>e.stopPropagation()}><div className="cf-modal-header"><h3>Full Technical Resume Preview</h3><button className="cf-close-btn" onClick={()=>setShowFullPreview(false)} aria-label="Close preview"><FiX/></button></div><div className="cf-modal-pages"><div className="cf-modal-page"><AITechnicalTemplate formData={formData}/></div></div></div></div>)}
...
export async function getStaticProps() {
  ...
  const faqDates = Array(3).fill(null).map((_, i) => {
    const date = new Date(buildTimestamp);
