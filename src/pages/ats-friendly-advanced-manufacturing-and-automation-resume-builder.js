...
const careerFlowStyles = `
  ...
  * { box-sizing: border-box; }
  ...
  .cf-mfg-template { color: #000000; line-height: 1.25; height: 100%; }
  ...
  .cf-modal-page .cf-mfg-template { width: 100%; height: 100%; padding: 10mm 15mm; box-sizing: border-box; overflow-y: auto; }
  ...
  @media (max-width: 768px) {
    ...
    .cf-modal-page .cf-mfg-template { padding: 8mm 12mm; }
...
const AdvancedManufacturingResume = ({ seoData, buildTimestamp }) => {
  ...
  const defaultProject = () => ({ name: '', company: '', technologies: '', description: '', isEditing: false, editIndex: null });
  const defaultSoftware = () => ({ name: '', category: '', proficiency: '', isEditing: false, editIndex: null });
  ...
  const [formData, setFormData] = useState({ fullName: '', email: '', phone: '', address: '', summary: '', experience: [], education: [], technicalSkills: [], certifications: [], projects: [], softwareProficiencies: [], languages: [], socialLinks: [] });
  ...
  const ManufacturingTemplate = ({ formData }) => {
    ...
    const skillsByCategory = formData.technicalSkills.reduce((acc, skill) => { const category = skill.category || 'General'; if (!acc[category]) acc[category] = []; acc[category].push(skill); return acc; }, {});

    return (
      <div className="cf-mfg-template">
        ...
        {hasTechnicalSkills && <section className="cf-section"><h2 className="cf-section-title" style={{ fontSize: `${fontSizes.sectionTitle}pt`, fontWeight: 700 }}>TECHNICAL SKILLS</h2>{Object.entries(skillsByCategory).map(([category, skills], index) => (<div key={index} style={{ marginBottom: '4pt' }}><h4 style={{ fontSize: `${fontSizes.regularText}pt`, fontWeight: 700 }}>{category}:</h4><ul className="cf-skills-list">{skills.map((skill, i) => (<li key={i} style={{ fontSize: `${fontSizes.skillText}pt`, fontWeight: 400 }}>{skill.name}{skill.proficiency && ` (${skill.proficiency})`}</li>))}</ul></div>))}</section>}
        {hasCertifications && <section className="cf-section"><h2 className="cf-section-title" style={{ fontSize: `${fontSizes.sectionTitle}pt`, fontWeight: 700 }}>CERTIFICATIONS & LICENSES</h2>{formData.certifications.map((cert, i) => (<div key={i} className="cf-cert-item" style={{ fontSize: `${fontSizes.certificationText}pt`, fontWeight: 400 }}><strong style={{ fontWeight: 700 }}>{cert.name}</strong>{cert.issuingAuthority && ` – ${cert.issuingAuthority}`}{cert.certificationNumber && ` (Cert #: ${cert.certificationNumber})`}{cert.expiryDate && ` – Expires: ${cert.expiryDate}`}</div>))}</section>}
        {hasProjects && <section className="cf-section"><h2 className="cf-section-title" style={{ fontSize: `${fontSizes.sectionTitle}pt`, fontWeight: 700 }}>KEY PROJECTS</h2>{formData.projects.map((project, i) => (<div key={i} className="cf-project-item"><h4 style={{ fontSize: `${fontSizes.jobTitle}pt`, fontWeight: 700 }}>{project.name}</h4><p className="cf-company" style={{ fontSize: `${fontSizes.company}pt`, fontWeight: 400 }}>{project.company && `${project.company} | `}{project.technologies}</p><p style={{ fontSize: `${fontSizes.regularText}pt`, fontWeight: 400 }}>{project.description}</p></div>))}</section>}
        {hasSoftware && <section className="cf-section"><h2 className="cf-section-title" style={{ fontSize: `${fontSizes.sectionTitle}pt`, fontWeight: 700 }}>SOFTWARE PROFICIENCY</h2><ul className="cf-bullet-list">{formData.softwareProficiencies.map((software, i) => (<li key={i} style={{ fontSize: `${fontSizes.bulletText}pt`, fontWeight: 400 }}>{software.name}{software.category && ` (${software.category})`}{software.proficiency && ` – ${software.proficiency}`}</li>))}</ul></section>}
  ...
  return (
    <>
      ...
      <div className="cf-resume-builder" lang="en-US">
        ...
        <nav className="cf-breadcrumb" aria-label="Breadcrumb"><ol><li><Link href="https://professionalresumefree.com" className="cf-breadcrumb-link"><FiHome className="cf-breadcrumb-icon"/><span className="cf-breadcrumb-text">Home</span></Link></li><li className="cf-breadcrumb-separator"><FiChevronRightIcon/></li><li><Link href="/ats-friendly-advanced-manufacturing-and-automation-resume-builder" className="cf-breadcrumb-link"><span className="cf-breadcrumb-text">Free Industry Templates</span></Link></li></ol></nav>
          ...
          <div className="cf-container">
            <div className="cf-hero-content">
              ...
              <div className="cf-cta-buttons">
                ...
                <button onClick={generatePDF} className="cf-btn-secondary" disabled={isGeneratingPDF || !hasContent()} aria-label="Download manufacturing resume as PDF"><FiDownload className="cf-btn-icon"/><span className="cf-btn-text">Download Manufacturing Resume PDF</span></button>
              </div>
              <div className="cf-hero-stats">
                <div className="cf-stat-item"><span className="cf-stat-number">500K+</span><span className="cf-stat-label">Industrial Professionals</span></div>
              ...
              <div className="cf-industry-badges"><div className="cf-badge-grid"><span className="cf-badge-item"><FiCpu/> Automation Engineering</span><span className="cf-badge-item"><FiTool/> Manufacturing Technician</span><span className="cf-badge-item"><FiCode/> PLC Programming</span><span className="cf-badge-item"><FiGrid/> Industrial Robotics</span><span className="cf-badge-item"><FiZap/> Process Engineering</span><span className="cf-badge-item"><FiShield/> Quality Control</span></div></div>
            </div>
          </div>
        </section>
        ...
        <div className="cf-layout">
          <div className="cf-preview-section">
            ...
            <div className={`cf-preview-container ${showFullPreview ? 'cf-full-preview' : ''}`}><div className="cf-resume-card"><div className="cf-preview-content"><div className="cf-resume-preview" ref={resumeRef}><ManufacturingTemplate formData={formData}/></div></div></div></div>
          </div>
          <div className="cf-form-section">
            <div className="cf-form-nav">
              {[{id:'personal',label:'Personal',icon:<FiUser/>},{id:'experience',label:'Industry Exp.',icon:<FiTool/>},{id:'education',label:'Education',icon:<FiBook/>},{id:'technical',label:'Tech Skills',icon:<FiCpu/>},{id:'certifications',label:'Certifications',icon:<FiShield/>},{id:'settings',label:'Font Settings',icon:<FiSettings/>}].map(item => (<button key={item.id} className={`cf-nav-btn ${activeSection===item.id?'cf-nav-btn-active':''}`} onClick={()=>setActiveSection(item.id)}>{item.icon}<span>{item.label}</span></button>))}
            </div>
            <div className="cf-form-content">
              {activeSection==='personal'&&(
                <div className="cf-form-section-content">
                  ...
                  <div className="cf-form-card">
                    <div className="cf-form-group"><label className="cf-form-label">Full Name*<input type="text" name="fullName" value={formData.fullName} onChange={handleInputChange} placeholder="John Smith, PE" required className="cf-form-input"/></label><label className="cf-form-label">Email*<input type="email" name="email" value={formData.email} onChange={handleInputChange} placeholder="john.smith@industrial.com" required className="cf-form-input"/></label></div>
                  ...
                  <div className="cf-form-card"><label className="cf-form-label">Professional Manufacturing Summary*<textarea name="summary" value={formData.summary} onChange={handleInputChange} placeholder="Results-driven Manufacturing Engineer with 8+ years of experience in automation systems, lean manufacturing, and industrial robotics. Skilled in PLC programming, process optimization, and Six Sigma methodologies. Proven track record of improving production efficiency by 35% through implementation of advanced manufacturing technologies and continuous improvement initiatives." required className="cf-form-textarea" rows="6"/><div className="cf-char-count">{formData.summary.length}/500 characters</div></label></div>
                  <div className="cf-form-card">
                    <h3 className="cf-sub-section-title"><FiGlobe/> Professional Links</h3><p className="cf-section-desc">Add your professional profiles (LinkedIn, GitHub, portfolio, etc.)</p>
                    <div className="cf-social-input">
                      <select value={currentSocialLink.platform} onChange={e=>setCurrentSocialLink({...currentSocialLink,platform:e.target.value})} className="cf-form-select"><option value="">Select Platform</option><option value="LinkedIn">LinkedIn</option><option value="GitHub">GitHub</option><option value="Portfolio">Professional Portfolio</option><option value="Website">Personal Website</option><option value="ResearchGate">ResearchGate</option><option value="IEEE">IEEE Publications</option></select>
              ...
              {activeSection==='experience'&&(
                <div className="cf-form-section-content">
                  <h2 className="cf-form-section-title"><FiTool/> Industry Experience</h2>
                  <p className="cf-section-desc">List your manufacturing positions in reverse chronological order (most recent first)</p>
                  <div className="cf-form-card">
                    <div className="cf-form-group"><label className="cf-form-label">Position Title*<input value={currentExperience.position} onChange={e=>setCurrentExperience({...currentExperience,position:e.target.value})} placeholder="Automation Engineer, Senior Manufacturing Technician" required className="cf-form-input"/></label><label className="cf-form-label">Company/Organization*<input value={currentExperience.employer} onChange={e=>setCurrentExperience({...currentExperience,employer:e.target.value})} placeholder="Advanced Manufacturing Corp" required className="cf-form-input"/></label></div>
                    <label className="cf-form-label">Department / Facility / Plant<input value={currentExperience.department} onChange={e=>setCurrentExperience({...currentExperience,department:e.target.value})} placeholder="Automation Department / Main Production Facility" className="cf-form-input"/></label>
                    ...
                    <label className="cf-form-label">Key Responsibilities & Industrial Achievements*<textarea value={currentExperience.description} onChange={e=>setCurrentExperience({...currentExperience,description:e.target.value})} placeholder="• Designed and implemented PLC-based automation systems for production line
...
• Developed and executed capital projects worth $2M for equipment upgrades" required className="cf-form-textarea" rows="8"/><div className="cf-char-count">{currentExperience.description.length}/2000 characters</div></label>
                    <div className="cf-form-actions"><button onClick={addExperience} className="cf-add-btn" disabled={!currentExperience.position||!currentExperience.employer||!currentExperience.startDate}><FiPlus/> {currentExperience.isEditing?'Update Industry Experience':'Add Industry Experience'}</button>{currentExperience.isEditing&&<button onClick={()=>setCurrentExperience(defaultExperience())} className="cf-cancel-btn"><FiX/> Cancel</button>}</div>
                  </div>
                  <div className="cf-form-card"><h3 className="cf-sub-section-title">Your Industry Experience</h3>{formData.experience.length===0?<p className="cf-empty-msg">No industry experience added yet</p>:<div className="cf-items-list">{formData.experience.map((exp,i)=>(<div key={i} className="cf-list-item"><div className="cf-item-content"><div className="cf-item-header"><strong className="cf-item-title">{exp.position}</strong><span className="cf-item-subtitle">at {exp.employer}</span></div><div className="cf-item-meta"><span>{exp.startDate} – {exp.endDate||'Present'}</span>{exp.department&&<span>{exp.department}</span>}</div><div className="cf-item-desc">{exp.description.split('\n').filter(l=>l.trim()).map((line,j)=><p key={j} className="cf-bullet-point">• {line}</p>)}</div></div><div className="cf-item-actions"><button onClick={()=>editExperience(i)} className="cf-edit-btn" aria-label={`Edit ${exp.position} experience`}><FiEdit2/></button><button onClick={()=>deleteExperience(i)} className="cf-delete-btn" aria-label={`Delete ${exp.position} experience`}><FiTrash2/></button></div></div>))}</div>}</div>
                </div>
              )}
              {activeSection==='education'&&(
                <div className="cf-form-section-content">
                  <h2 className="cf-form-section-title"><FiBook/> Education & Training</h2>
                  <p className="cf-section-desc">List your engineering education, degrees, and relevant technical training</p>
                  <div className="cf-form-card">
                    <div className="cf-form-group"><label className="cf-form-label">Institution*<input value={currentEducation.institution} onChange={e=>setCurrentEducation({...currentEducation,institution:e.target.value})} placeholder="Massachusetts Institute of Technology" required className="cf-form-input"/></label><label className="cf-form-label">Degree/Certification*<input value={currentEducation.degree} onChange={e=>setCurrentEducation({...currentEducation,degree:e.target.value})} placeholder="Bachelor of Science in Mechanical Engineering" required className="cf-form-input"/></label></div>
                    <label className="cf-form-label">Program / Specialization / Concentration<input value={currentEducation.program} onChange={e=>setCurrentEducation({...currentEducation,program:e.target.value})} placeholder="Robotics and Automation, Manufacturing Systems" className="cf-form-input"/></label>
              ...
              {activeSection==='technical'&&(
                <div className="cf-form-section-content">
                  <h2 className="cf-form-section-title"><FiCpu/> Technical Skills & Expertise</h2>
                  <div className="cf-form-card">
                    <h3 className="cf-sub-section-title">Technical Skills</h3><p className="cf-section-desc">List your technical skills, grouped by category</p>
                    <div className="cf-skills-input">
                      <div className="cf-form-group"><label className="cf-form-label">Skill Name*<input value={currentTechnicalSkill.name} onChange={e=>setCurrentTechnicalSkill({...currentTechnicalSkill,name:e.target.value})} placeholder="PLC Programming, Industrial Robotics, CAD/CAM" className="cf-form-input"/></label><label className="cf-form-label">Category<select value={currentTechnicalSkill.category} onChange={e=>setCurrentTechnicalSkill({...currentTechnicalSkill,category:e.target.value})} className="cf-form-select"><option value="">Select Category</option><option value="Automation">Automation</option><option value="Robotics">Robotics</option><option value="PLC Programming">PLC Programming</option><option value="CAD/CAM">CAD/CAM</option><option value="CNC Machining">CNC Machining</option><option value="Quality Control">Quality Control</option><option value="Lean Manufacturing">Lean Manufacturing</option><option value="Maintenance">Maintenance</option><option value="Electrical">Electrical</option><option value="Mechanical">Mechanical</option></select></label></div>
                      <label className="cf-form-label">Proficiency Level<input value={currentTechnicalSkill.proficiency} onChange={e=>setCurrentTechnicalSkill({...currentTechnicalSkill,proficiency:e.target.value})} placeholder="Expert, Advanced, Intermediate, Basic" className="cf-form-input"/></label>
                    ...
                    <div className="cf-items-list">{formData.technicalSkills.map((skill,i)=>(<div key={i} className="cf-list-item"><div><strong>{skill.name}</strong>{skill.category&&` (${skill.category})`}{skill.proficiency&&` – ${skill.proficiency}`}</div><div className="cf-item-actions"><button onClick={()=>editTechnicalSkill(i)} className="cf-edit-btn"><FiEdit2/></button><button onClick={()=>deleteTechnicalSkill(i)} className="cf-delete-btn"><FiTrash2/></button></div></div>))}{formData.technicalSkills.length===0&&<p className="cf-empty-msg">No technical skills added yet</p>}</div>
                  </div>
                  <div className="cf-form-card">
                    <h3 className="cf-sub-section-title"><FiMonitor/> Software Proficiency</h3><p className="cf-section-desc">List industrial software, CAD/CAM, and programming tools</p>
                    <div className="cf-skills-input">
                      <div className="cf-form-group"><label className="cf-form-label">Software Name*<input value={currentSoftware.name} onChange={e=>setCurrentSoftware({...currentSoftware,name:e.target.value})} placeholder="Siemens TIA Portal, AutoCAD, SolidWorks" className="cf-form-input"/></label><label className="cf-form-label">Category<select value={currentSoftware.category} onChange={e=>setCurrentSoftware({...currentSoftware,category:e.target.value})} className="cf-form-select"><option value="">Select Category</option><option value="CAD/CAM">CAD/CAM</option><option value="PLC Programming">PLC Programming</option><option value="SCADA">SCADA Systems</option><option value="MES">MES Software</option><option value="ERP">ERP Systems</option><option value="Simulation">Simulation Software</option><option value="Programming">Programming Languages</option></select></label></div>
                      <label className="cf-form-label">Proficiency Level<input value={currentSoftware.proficiency} onChange={e=>setCurrentSoftware({...currentSoftware,proficiency:e.target.value})} placeholder="Expert, Advanced, Intermediate" className="cf-form-input"/></label>
                    ...
                    <div className="cf-items-list">{formData.softwareProficiencies.map((software,i)=>(<div key={i} className="cf-list-item"><div><strong>{software.name}</strong>{software.category&&` (${software.category})`}{software.proficiency&&` – ${software.proficiency}`}</div><div className="cf-item-actions"><button onClick={()=>editSoftware(i)} className="cf-edit-btn"><FiEdit2/></button><button onClick={()=>deleteSoftware(i)} className="cf-delete-btn"><FiTrash2/></button></div></div>))}{formData.softwareProficiencies.length===0&&<p className="cf-empty-msg">No software added yet</p>}</div>
                  </div>
                  <div className="cf-form-card">
                    <h3 className="cf-sub-section-title">Key Industrial Projects</h3><p className="cf-section-desc">Add significant manufacturing or automation projects</p>
                    <div className="cf-skills-input">
                      <div className="cf-form-group"><label className="cf-form-label">Project Name*<input value={currentProject.name} onChange={e=>setCurrentProject({...currentProject,name:e.target.value})} placeholder="Automated Production Line Upgrade" className="cf-form-input"/></label><label className="cf-form-label">Company/Organization<input value={currentProject.company} onChange={e=>setCurrentProject({...currentProject,company:e.target.value})} placeholder="Advanced Manufacturing Corp" className="cf-form-input"/></label></div>
                      <label className="cf-form-label">Technologies Used<input value={currentProject.technologies} onChange={e=>setCurrentProject({...currentProject,technologies:e.target.value})} placeholder="Siemens PLCs, Fanuc Robots, Vision Systems" className="cf-form-input"/></label>
                      <label className="cf-form-label">Project Description & Achievements*<textarea value={currentProject.description} onChange={e=>setCurrentProject({...currentProject,description:e.target.value})} placeholder="• Led the upgrade of main production line with new automation systems
...
• Reduced quality defects by 40% through automated inspection" className="cf-form-textarea" rows="6"/></label>
                  ...
                  <div className="cf-form-card">
                    ...
                    <div className="cf-skills-input">
                      <div className="cf-form-group"><label className="cf-form-label">Language*<input value={currentLanguage.name} onChange={e=>setCurrentLanguage({...currentLanguage,name:e.target.value})} placeholder="Spanish" className="cf-form-input"/></label><label className="cf-form-label">Proficiency Level<input value={currentLanguage.proficiency} onChange={e=>setCurrentLanguage({...currentLanguage,proficiency:e.target.value})} placeholder="Fluent, Technical Proficient" className="cf-form-input"/></label></div>
              ...
              {activeSection==='certifications'&&(
                <div className="cf-form-section-content">
                  <h2 className="cf-form-section-title"><FiShield/> Certifications & Licenses</h2>
                  <p className="cf-section-desc">Add your professional certifications and industry licenses</p>
                  <div className="cf-form-card">
                    <div className="cf-form-group"><label className="cf-form-label">Certification/License Name*<input value={currentCertification.name} onChange={e=>setCurrentCertification({...currentCertification,name:e.target.value})} placeholder="Six Sigma Black Belt, Professional Engineer (PE)" className="cf-form-input"/></label><label className="cf-form-label">Issuing Authority<input value={currentCertification.issuingAuthority} onChange={e=>setCurrentCertification({...currentCertification,issuingAuthority:e.target.value})} placeholder="ASQ, State Licensing Board" className="cf-form-input"/></label></div>
                    <div className="cf-form-group"><label className="cf-form-label">Certification Number<input value={currentCertification.certificationNumber} onChange={e=>setCurrentCertification({...currentCertification,certificationNumber:e.target.value})} placeholder="SSBB1234567" className="cf-form-input"/></label><label className="cf-form-label">Expiry Date<input value={currentCertification.expiryDate} onChange={e=>setCurrentCertification({...currentCertification,expiryDate:e.target.value})} placeholder="Month Year" className="cf-form-input"/></label></div>
        ...
        <section className="cf-faq-section" aria-labelledby="faq-title"><div className="cf-container"><div className="cf-section-header"><h2 className="cf-section-title" id="faq-title">Frequently Asked Questions</h2><p>Everything you need to know about creating professional manufacturing resumes with our tool.</p></div><div className="cf-faq-grid">{faqs.map((faq,i)=>(<div key={i} className="cf-faq-item"><h3 className="cf-faq-question">{faq.question}</h3><p className="cf-faq-answer">{faq.answer}</p></div>))}</div></div></section>
