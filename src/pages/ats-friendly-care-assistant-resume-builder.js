...
const careerFlowStyles = `
  ...
  * { box-sizing: border-box; }
  ...
  .cf-cert-item, .cf-affiliation-item, .cf-reference-item { margin-bottom: 3pt; line-height: 1.25; }
  .cf-reference-detail { color: #6c757d; font-size: 7pt; }
...
const Resume = ({ seoData, buildTimestamp }) => {
  ...
  const defaultAffiliation = () => ({ organization: '', membershipType: '', isEditing: false, editIndex: null });
  ...
  const defaultReference = () => ({ name: '', relationship: '', phone: '', email: '', isEditing: false, editIndex: null });

  const [formData, setFormData] = useState({ fullName: '', email: '', phone: '', address: '', summary: '', experience: [], education: [], specialties: [], certifications: [], affiliations: [], skills: [], languages: [], references: [] });
  const [fontSizes, setFontSizes] = useState({ name: 14, sectionTitle: 10, contactInfo: 7, jobTitle: 9, company: 7, qualification: 9, institution: 7, institutionDate: 6, regularText: 8, bulletText: 8, skillText: 7, certificateText: 8 });
  ...
  const resetFontSizes = () => setFontSizes({ name: 14, sectionTitle: 10, contactInfo: 7, jobTitle: 9, company: 7, qualification: 9, institution: 7, institutionDate: 6, regularText: 8, bulletText: 8, skillText: 7, certificateText: 8 });
  const formatSocialUrl = (url) => url ? url.replace(/(https?:\/\/)?(www\.)?/, '').replace(/\/$/, '') : '';
  const handleInputChange = (e) => setFormData({ ...formData, [e.target.name]: e.target.value });
  const hasContent = () => formData.fullName || formData.email || formData.summary || formData.experience.length > 0 || formData.education.length > 0 || formData.specialties.length > 0 || formData.certifications.length > 0 || formData.affiliations.length > 0 || formData.skills.length > 0 || formData.languages.length > 0 || formData.references.length > 0;
  ...
  const CareAssistantTemplate = ({ formData }) => {
    ...
    return (
      <div className="cf-universal-template">
        ...
        {hasSummary && <section className="cf-section"><h2 className="cf-section-title" style={{ fontSize: `${fontSizes.sectionTitle}pt` }}>CARE PROFILE</h2><p className="cf-summary-text" style={{ fontSize: `${fontSizes.regularText}pt` }}>{formData.summary}</p></section>}
        {hasExperience && <section className="cf-section"><h2 className="cf-section-title" style={{ fontSize: `${fontSizes.sectionTitle}pt` }}>CARE EXPERIENCE</h2>{formData.experience.map((exp, i) => (<div key={i} className="cf-experience-item"><div className="cf-experience-header"><h3 style={{ fontSize: `${fontSizes.jobTitle}pt` }}>{exp.position}</h3><p className="cf-company" style={{ fontSize: `${fontSizes.company}pt` }}>{exp.employer}{exp.clientType && ` – ${exp.clientType}`} | {exp.startDate} – {exp.endDate || 'Present'}</p></div><ul className="cf-bullet-list">{exp.description.split('\n').filter(l => l.trim()).map((line, j) => <li key={j} style={{ fontSize: `${fontSizes.bulletText}pt` }}>{line}</li>)}</ul></div>))}</section>}
        {hasEducation && <section className="cf-section"><h2 className="cf-section-title" style={{ fontSize: `${fontSizes.sectionTitle}pt` }}>EDUCATION & QUALIFICATIONS</h2>{formData.education.map((edu, i) => (<div key={i} className="cf-education-item"><h3 style={{ fontSize: `${fontSizes.qualification}pt` }}>{edu.qualification}{edu.certificateNumber && ` (${edu.certificateNumber})`}</h3><p className="cf-institution-text" style={{ fontSize: `${fontSizes.institution}pt` }}>{edu.institution} | <span className="cf-institution-date" style={{ fontSize: `${fontSizes.institutionDate}pt` }}>{edu.startDate} – {edu.endDate || 'Ongoing'}</span></p></div>))}</section>}
        {hasCertifications && <section className="cf-section"><h2 className="cf-section-title" style={{ fontSize: `${fontSizes.sectionTitle}pt` }}>CERTIFICATIONS & TRAINING</h2>{formData.certifications.map((cert, i) => (<div key={i} className="cf-cert-item" style={{ fontSize: `${fontSizes.certificateText}pt` }}><strong style={{ fontWeight: 700 }}>{cert.name}</strong>{cert.issuingAuthority && ` – ${cert.issuingAuthority}`}{cert.certificateNumber && ` (Certificate #: ${cert.certificateNumber})`}{cert.expiryDate && ` – Expires: ${cert.expiryDate}`}</div>))}</section>}
        {hasSkills && <section className="cf-section"><h2 className="cf-section-title" style={{ fontSize: `${fontSizes.sectionTitle}pt` }}>CARE SKILLS & COMPETENCIES</h2><ul className="cf-skills-list">{formData.skills.map((s, i) => (<li key={i} style={{ fontSize: `${fontSizes.skillText}pt` }}>{s.name}{s.proficiency && ` (${s.proficiency})`}</li>))}</ul></section>}
        {hasSpecialties && <section className="cf-section"><h2 className="cf-section-title" style={{ fontSize: `${fontSizes.sectionTitle}pt` }}>AREAS OF SPECIALISM</h2><ul className="cf-bullet-list">{formData.specialties.map((s, i) => <li key={i} style={{ fontSize: `${fontSizes.bulletText}pt` }}>{s.name}</li>)}</ul></section>}
        {hasAffiliations && <section className="cf-section"><h2 className="cf-section-title" style={{ fontSize: `${fontSizes.sectionTitle}pt` }}>PROFESSIONAL AFFILIATIONS</h2>{formData.affiliations.map((a, i) => (<div key={i} className="cf-affiliation-item" style={{ fontSize: `${fontSizes.regularText}pt` }}><strong style={{ fontWeight: 700 }}>{a.organization}</strong>{a.membershipType && ` – ${a.membershipType}`}</div>))}</section>}
      ...
      <div className="cf-resume-builder" lang="en-GB">
        ...
        <nav className="cf-breadcrumb" aria-label="Breadcrumb">
          <ol>
            ...
            <li>
              <Link href="/ats-friendly-care-assistant-resume-builder" className="cf-breadcrumb-link" prefetch={false}>
                <span className="cf-breadcrumb-text">Free Care Assistant CV Builder</span>
              </Link>
            </li>
          </ol>
        </nav>
          ...
          <div className="cf-container">
            <div className="cf-hero-content">
              ...
              <p className="cf-hero-subtitle">
                Create a <strong className="cf-hero-highlight">professional, ATS-optimized care assistant CV for free in minutes.</strong> Our UK-specific CV builder ensures your care experience and qualifications get noticed by UK care homes, NHS trusts, and healthcare agencies.
              </p>
              ...
              <div className="cf-cta-buttons">
                ...
                >
                  <span className="cf-btn-text">Start Building Your Care CV Now</span>
                ...
                >
                  ...
                  <span className="cf-btn-text">Download Care CV PDF</span>
                </button>
              </div>

              <div className="cf-industry-badges">
                <div className="cf-badge-grid">
                  <span className="cf-badge-item"><FiUsers /> Care Assistant</span>
                  <span className="cf-badge-item"><FiHomeIcon /> Domiciliary Care</span>
                  <span className="cf-badge-item"><FiClipboard /> NHS HCA</span>
                  <span className="cf-badge-item"><FiHeart /> Elderly Care</span>
                  <span className="cf-badge-item"><FiShield /> Mental Health</span>
                  <span className="cf-badge-item"><FiWatch /> Dementia Care</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="cf-layout">
          <div className="cf-preview-section">
            ...
            <div className={`cf-preview-container ${showFullPreview ? 'cf-full-preview' : ''}`}><div className="cf-resume-card"><div className="cf-preview-content"><div className="cf-resume-preview" ref={resumeRef}><CareAssistantTemplate formData={formData}/></div></div></div></div>
          </div>

          <div className="cf-form-section">
            <div className="cf-form-nav">
              {[{id:'personal',label:'Personal',icon:<FiUser/>},{id:'experience',label:'Care Exp',icon:<FiHeart/>},{id:'education',label:'Qualifications',icon:<FiBook/>},{id:'certifications',label:'Certifications',icon:<FiShield/>},{id:'skills',label:'Skills',icon:<FiActivity/>},{id:'settings',label:'Font Settings',icon:<FiSettings/>}].map(item => (<button key={item.id} className={`cf-nav-btn ${activeSection===item.id?'cf-nav-btn-active':''}`} onClick={()=>setActiveSection(item.id)}>{item.icon}<span>{item.label}</span></button>))}
            </div>
              ...
              {activeSection==='personal'&&(
                <div className="cf-form-section-content">
                  ...
                  <div className="cf-form-card">
                    <div className="cf-form-group"><label className="cf-form-label">Full Name*<input type="text" name="fullName" value={formData.fullName} onChange={handleInputChange} placeholder="Sarah Johnson" required className="cf-form-input"/></label><label className="cf-form-label">Email*<input type="email" name="email" value={formData.email} onChange={handleInputChange} placeholder="sarah.johnson@email.co.uk" required className="cf-form-input"/></label></div>
                    <div className="cf-form-group"><label className="cf-form-label">Phone*<input type="tel" name="phone" value={formData.phone} onChange={handleInputChange} placeholder="07700 900123" required className="cf-form-input"/></label><label className="cf-form-label">UK Address<input type="text" name="address" value={formData.address} onChange={handleInputChange} placeholder="London, UK" className="cf-form-input"/></label></div>
                  </div>
                  <div className="cf-form-card"><label className="cf-form-label">Care Profile Summary*<textarea name="summary" value={formData.summary} onChange={handleInputChange} placeholder="Compassionate and dedicated Care Assistant with 5+ years of experience in elderly care and dementia support. NVQ Level 3 qualified with proven track record in providing person-centered care, medication administration, and supporting daily living activities. Committed to maintaining dignity, respect, and independence for all clients while working collaboratively within multidisciplinary teams." required className="cf-form-textarea" rows="6"/><div className="cf-char-count">{formData.summary.length}/500 characters</div></label></div>
                </div>
              )}

              {activeSection==='experience'&&(
                <div className="cf-form-section-content">
                  <h2 className="cf-form-section-title"><FiHeart/> Care Experience</h2><p className="cf-section-desc">List your care positions in reverse chronological order</p>
                  <div className="cf-form-card">
                    <div className="cf-form-group"><label className="cf-form-label">Position Title*<input value={currentExperience.position} onChange={e=>setCurrentExperience({...currentExperience,position:e.target.value})} placeholder="Care Assistant" required className="cf-form-input"/></label><label className="cf-form-label">Care Provider*<input value={currentExperience.employer} onChange={e=>setCurrentExperience({...currentExperience,employer:e.target.value})} placeholder="Sunrise Care Home" required className="cf-form-input"/></label></div>
                    <label className="cf-form-label">Client Type / Specialism<input value={currentExperience.clientType} onChange={e=>setCurrentExperience({...currentExperience,clientType:e.target.value})} placeholder="Elderly & Dementia Care" className="cf-form-input"/></label>
                    ...
                    <label className="cf-form-label">Key Care Responsibilities*<textarea value={currentExperience.description} onChange={e=>setCurrentExperience({...currentExperience,description:e.target.value})} placeholder="• Provided person-centered care for 8 residents in dementia unit
...
• Trained 2 new care assistants in safeguarding procedures and care protocols" required className="cf-form-textarea" rows="8"/><div className="cf-char-count">{currentExperience.description.length}/2000 characters</div></label>
                  ...
                  <div className="cf-form-card"><h3 className="cf-sub-section-title">Your Care Experience</h3>{formData.experience.length===0?<p className="cf-empty-msg">No care experience added yet</p>:<div className="cf-items-list">{formData.experience.map((exp,i)=>(<div key={i} className="cf-list-item"><div className="cf-item-content"><div className="cf-item-header"><strong className="cf-item-title">{exp.position}</strong><span className="cf-item-subtitle">at {exp.employer}</span></div><div className="cf-item-meta"><span>{exp.startDate} – {exp.endDate||'Present'}</span>{exp.clientType&&<span>{exp.clientType}</span>}</div><div className="cf-item-desc">{exp.description.split('\n').filter(l=>l.trim()).map((line,j)=><p key={j} className="cf-bullet-point">• {line}</p>)}</div></div><div className="cf-item-actions"><button onClick={()=>editExperience(i)} className="cf-edit-btn" aria-label={`Edit ${exp.position} experience`}><FiEdit2/></button><button onClick={()=>deleteExperience(i)} className="cf-delete-btn" aria-label={`Delete ${exp.position} experience`}><FiTrash2/></button></div></div>))}</div>}</div>
                  <div className="cf-form-card">
                    <h3 className="cf-sub-section-title">Areas of Specialism</h3><p className="cf-section-desc">List your care specialisms</p>
                    <div className="cf-skills-input"><input value={currentSpecialty.name} onChange={e=>setCurrentSpecialty({...currentSpecialty,name:e.target.value})} placeholder="Dementia Care, Palliative Care" className="cf-form-input"/><div className="cf-form-actions"><button onClick={addSpecialty} className="cf-add-btn" disabled={!currentSpecialty.name.trim()}><FiPlus/> {currentSpecialty.isEditing?'Update':'Add Specialism'}</button>{currentSpecialty.isEditing&&<button onClick={()=>setCurrentSpecialty(defaultSpecialty())} className="cf-cancel-btn"><FiX/> Cancel</button>}</div></div>
              ...
              {activeSection==='education'&&(
                <div className="cf-form-section-content">
                  <h2 className="cf-form-section-title"><FiBook/> Qualifications & Training</h2><p className="cf-section-desc">List your care qualifications and NVQ/SVQ certifications</p>
                  <div className="cf-form-card">
                    <div className="cf-form-group"><label className="cf-form-label">Institution*<input value={currentEducation.institution} onChange={e=>setCurrentEducation({...currentEducation,institution:e.target.value})} placeholder="City College / NHS Trust" required className="cf-form-input"/></label><label className="cf-form-label">Qualification*<input value={currentEducation.qualification} onChange={e=>setCurrentEducation({...currentEducation,qualification:e.target.value})} placeholder="NVQ Level 3 in Health and Social Care" required className="cf-form-input"/></label></div>
                    <label className="cf-form-label">Certificate Number<input value={currentEducation.certificateNumber} onChange={e=>setCurrentEducation({...currentEducation,certificateNumber:e.target.value})} placeholder="CERT1234567" className="cf-form-input"/></label>
                    <div className="cf-form-group"><label className="cf-form-label">Start Date<input type="text" placeholder="Month Year" value={currentEducation.startDate} onChange={e=>setCurrentEducation({...currentEducation,startDate:e.target.value})} className="cf-form-input"/></label><label className="cf-form-label">Completion Date<input type="text" placeholder="Month Year" value={currentEducation.endDate} onChange={e=>setCurrentEducation({...currentEducation,endDate:e.target.value})} className="cf-form-input"/></label></div>
                    <div className="cf-form-actions"><button onClick={addEducation} className="cf-add-btn" disabled={!currentEducation.institution||!currentEducation.qualification}><FiPlus/> {currentEducation.isEditing?'Update':'Add Qualification'}</button>{currentEducation.isEditing&&<button onClick={()=>setCurrentEducation(defaultEducation())} className="cf-cancel-btn"><FiX/> Cancel</button>}</div>
                  </div>
                  <div className="cf-form-card"><h3 className="cf-sub-section-title">Your Qualifications</h3>{formData.education.length===0?<p className="cf-empty-msg">No qualifications added yet</p>:<div className="cf-items-list">{formData.education.map((edu,i)=>(<div key={i} className="cf-list-item"><div className="cf-item-content"><div className="cf-item-header"><strong className="cf-item-title">{edu.qualification}</strong>{edu.certificateNumber&&<span className="cf-item-subtitle"> – #{edu.certificateNumber}</span>}</div><div className="cf-item-meta"><span>{edu.institution}</span><span>{edu.startDate} – {edu.endDate||'In Progress'}</span></div></div><div className="cf-item-actions"><button onClick={()=>editEducation(i)} className="cf-edit-btn" aria-label={`Edit ${edu.qualification}`}><FiEdit2/></button><button onClick={()=>deleteEducation(i)} className="cf-delete-btn" aria-label={`Delete ${edu.qualification}`}><FiTrash2/></button></div></div>))}</div>}</div>
                </div>
              )}

              {activeSection==='certifications'&&(
                <div className="cf-form-section-content">
                  <h2 className="cf-form-section-title"><FiShield/> Certifications & Compliance</h2>
                  <div className="cf-form-card">
                    <h3 className="cf-sub-section-title">Certifications & Training</h3><p className="cf-section-desc">Add your care certifications and DBS status</p>
                    <div className="cf-skills-input">
                      <div className="cf-form-group"><label className="cf-form-label">Certification Name*<input value={currentCertification.name} onChange={e=>setCurrentCertification({...currentCertification,name:e.target.value})} placeholder="Care Certificate" className="cf-form-input"/></label><label className="cf-form-label">Issuing Authority<input value={currentCertification.issuingAuthority} onChange={e=>setCurrentCertification({...currentCertification,issuingAuthority:e.target.value})} placeholder="Skills for Care" className="cf-form-input"/></label></div>
                      <div className="cf-form-group"><label className="cf-form-label">Certificate Number<input value={currentCertification.certificateNumber} onChange={e=>setCurrentCertification({...currentCertification,certificateNumber:e.target.value})} placeholder="CERT1234567" className="cf-form-input"/></label><label className="cf-form-label">Expiry Date<input value={currentCertification.expiryDate} onChange={e=>setCurrentCertification({...currentCertification,expiryDate:e.target.value})} placeholder="Month Year" className="cf-form-input"/></label></div>
                  ...
                  <div className="cf-form-card">
                    <h3 className="cf-sub-section-title">Professional Affiliations</h3><p className="cf-section-desc">Add your care organization memberships</p>
                    <div className="cf-skills-input">
                      <div className="cf-form-group"><label className="cf-form-label">Organization*<input value={currentAffiliation.organization} onChange={e=>setCurrentAffiliation({...currentAffiliation,organization:e.target.value})} placeholder="Skills for Care" className="cf-form-input"/></label><label className="cf-form-label">Membership Type<input value={currentAffiliation.membershipType} onChange={e=>setCurrentAffiliation({...currentAffiliation,membershipType:e.target.value})} placeholder="Registered Member" className="cf-form-input"/></label></div>
                    ...
                    <div className="cf-items-list">{formData.affiliations.map((a,i)=>(<div key={i} className="cf-list-item"><div><strong>{a.organization}</strong>{a.membershipType&&` – ${a.membershipType}`}</div><div className="cf-item-actions"><button onClick={()=>editAffiliation(i)} className="cf-edit-btn"><FiEdit2/></button><button onClick={()=>deleteAffiliation(i)} className="cf-delete-btn"><FiTrash2/></button></div></div>))}{formData.affiliations.length===0&&<p className="cf-empty-msg">No affiliations added yet</p>}</div>
                  </div>
                  <div className="cf-form-card">
                    <h3 className="cf-sub-section-title">Professional References</h3><p className="cf-section-desc">Add your references (usually 2 required)</p>
                    <div className="cf-skills-input">
                      <div className="cf-form-group"><label className="cf-form-label">Reference Name*<input value={currentReference.name} onChange={e=>setCurrentReference({...currentReference,name:e.target.value})} placeholder="Jane Smith" className="cf-form-input"/></label><label className="cf-form-label">Relationship*<input value={currentReference.relationship} onChange={e=>setCurrentReference({...currentReference,relationship:e.target.value})} placeholder="Care Home Manager" className="cf-form-input"/></label></div>
                      <div className="cf-form-group"><label className="cf-form-label">Phone<input value={currentReference.phone} onChange={e=>setCurrentReference({...currentReference,phone:e.target.value})} placeholder="07700 900123" className="cf-form-input"/></label><label className="cf-form-label">Email<input value={currentReference.email} onChange={e=>setCurrentReference({...currentReference,email:e.target.value})} placeholder="jane.smith@carehome.co.uk" className="cf-form-input"/></label></div>
                      <div className="cf-form-actions"><button onClick={addReference} className="cf-add-btn" disabled={!currentReference.name.trim()||!currentReference.relationship.trim()}><FiPlus/> {currentReference.isEditing?'Update':'Add Reference'}</button>{currentReference.isEditing&&<button onClick={()=>setCurrentReference(defaultReference())} className="cf-cancel-btn"><FiX/> Cancel</button>}</div>
                    </div>
                    <div className="cf-items-list">{formData.references.map((r,i)=>(<div key={i} className="cf-list-item"><div><strong>{r.name}</strong> – {r.relationship}{r.phone&&<div className="cf-reference-detail">Phone: {r.phone}</div>}{r.email&&<div className="cf-reference-detail">Email: {r.email}</div>}</div><div className="cf-item-actions"><button onClick={()=>editReference(i)} className="cf-edit-btn"><FiEdit2/></button><button onClick={()=>deleteReference(i)} className="cf-delete-btn"><FiTrash2/></button></div></div>))}{formData.references.length===0&&<p className="cf-empty-msg">No references added yet</p>}</div>
                  </div>
                </div>
              )}
              ...
              {activeSection==='skills'&&(
                <div className="cf-form-section-content">
                  <h2 className="cf-form-section-title"><FiActivity/> Care Skills & Competencies</h2>
                  <div className="cf-form-card">
                    <h3 className="cf-sub-section-title">Care Skills</h3><p className="cf-section-desc">List your care skills and competencies</p>
                    <div className="cf-skills-input">
                      <div className="cf-form-group"><label className="cf-form-label">Skill*<input value={currentSkill.name} onChange={e=>setCurrentSkill({...currentSkill,name:e.target.value})} placeholder="Personal Care Assistance" className="cf-form-input"/></label><label className="cf-form-label">Proficiency<input value={currentSkill.proficiency} onChange={e=>setCurrentSkill({...currentSkill,proficiency:e.target.value})} placeholder="Advanced" className="cf-form-input"/></label></div>
                  ...
                  <div className="cf-form-card">
                    ...
                    <div className="cf-skills-input">
                      <div className="cf-form-group"><label className="cf-form-label">Language*<input value={currentLanguage.name} onChange={e=>setCurrentLanguage({...currentLanguage,name:e.target.value})} placeholder="Punjabi" className="cf-form-input"/></label><label className="cf-form-label">Proficiency<input value={currentLanguage.proficiency} onChange={e=>setCurrentLanguage({...currentLanguage,proficiency:e.target.value})} placeholder="Fluent" className="cf-form-input"/></label></div>
              ...
              {activeSection==='settings'&&(
                <div className="cf-form-section-content">
                  <h2 className="cf-form-section-title"><FiSettings/> Font Size Settings</h2><p className="cf-section-desc">Customize font sizes for your CV PDF. All sizes in points (pt).</p>
                  <div className="cf-form-card">
                    <div className="cf-font-grid">
                      {[{key:'name',label:'Name'},{key:'sectionTitle',label:'Section Titles'},{key:'jobTitle',label:'Job Titles'},{key:'qualification',label:'Qualifications'},{key:'institution',label:'Company/Institution'},{key:'institutionDate',label:'Dates'},{key:'regularText',label:'Regular Text'},{key:'bulletText',label:'Bullet Points'},{key:'contactInfo',label:'Contact Info'},{key:'skillText',label:'Skills Text'},{key:'certificateText',label:'Certificate Text'}].map(({key,label})=>(<div key={key} className="cf-font-control"><label className="cf-font-label"><span>{label}</span><span className="cf-font-value">{fontSizes[key]}pt</span></label><input type="range" min={key==='institutionDate'?6:8} max={key==='name'?24:key==='sectionTitle'?18:key==='jobTitle'||key==='qualification'?16:14} value={fontSizes[key]} onChange={e=>handleFontSizeChange(key,e.target.value)} className="cf-font-slider"/></div>))}
                    </div>
        ...
        <section className="cf-faq-section" aria-labelledby="faq-title"><div className="cf-container"><div className="cf-section-header"><h2 id="faq-title">Frequently Asked Questions</h2><p>Everything you need to know about creating professional care assistant CVs for the UK job market.</p></div><div className="cf-faq-grid">{faqs.map((faq,i)=>(<div key={i} className="cf-faq-item"><h3 className="cf-faq-question">{faq.question}</h3><p className="cf-faq-answer">{faq.answer}</p></div>))}</div></div></section>
