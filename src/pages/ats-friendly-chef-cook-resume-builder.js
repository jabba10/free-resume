...
const chefResumeStyles = `
  ...
  * { box-sizing: border-box; }
  ...
  .cf-chef-template { color: #000000; line-height: 1.25; height: 100%; }
  ...
  .cf-certification-item, .cf-affiliation-item { margin-bottom: 3pt; line-height: 1.25; }
  ...
  .cf-modal-page .cf-chef-template { width: 100%; height: 100%; padding: 10mm 15mm; box-sizing: border-box; overflow-y: auto; }
  ...
  @media (max-width: 768px) {
    ...
    .cf-modal-page .cf-chef-template { padding: 8mm 12mm; }
...
const Resume = ({ seoData, buildTimestamp }) => {
  ...
  const defaultCuisine = () => ({ name: '', isEditing: false, editIndex: null });
  ...
  const defaultTechnique = () => ({ name: '', isEditing: false, editIndex: null });
  ...
  const [formData, setFormData] = useState({ fullName: '', email: '', phone: '', address: '', summary: '', experience: [], education: [], cuisines: [], certifications: [], affiliations: [], techniques: [], languages: [], socialLinks: [] });
  const [fontSizes, setFontSizes] = useState({ name: 14, sectionTitle: 10, contactInfo: 7, jobTitle: 9, restaurant: 7, degree: 9, institution: 7, institutionDate: 6, regularText: 8, bulletText: 8, skillText: 7, certificationText: 8 });
  ...
  const faqs = [
    ...
    { question: "How does your chef resume builder work?", answer: "Our builder uses ATS-optimized culinary templates with proper industry terminology formatting. We guide you to highlight kitchen experience, certifications, menu development, and specialized culinary skills that employers look for." },
    { question: "Can I edit my chef resume after downloading it?", answer: "Yes, you can always come back and edit your culinary resume. Your work saves automatically, and you can download updated versions as many times as needed—completely free." }
  ];

  const handleFontSizeChange = (key, value) => setFontSizes(prev => ({ ...prev, [key]: Math.max(4, Math.min(24, parseInt(value) || prev[key])) }));
  const resetFontSizes = () => setFontSizes({ name: 14, sectionTitle: 10, contactInfo: 7, jobTitle: 9, restaurant: 7, degree: 9, institution: 7, institutionDate: 6, regularText: 8, bulletText: 8, skillText: 7, certificationText: 8 });
  const getSocialIcon = () => <FiGlobe />;
  const formatSocialUrl = (url) => url ? url.replace(/(https?:\/\/)?(www\.)?/, '').replace(/\/$/, '') : '';
  const handleInputChange = (e) => setFormData({ ...formData, [e.target.name]: e.target.value });
  const hasContent = () => formData.fullName || formData.email || formData.summary || formData.socialLinks.length > 0 || formData.experience.length > 0 || formData.education.length > 0 || formData.cuisines.length > 0 || formData.certifications.length > 0 || formData.affiliations.length > 0 || formData.techniques.length > 0 || formData.languages.length > 0;
  ...
  const ChefTemplate = ({ formData }) => {
    ...
    return (
      <div className="cf-chef-template">
        ...
        {hasSummary && <section className="cf-section"><h2 className="cf-section-title" style={{ fontSize: `${fontSizes.sectionTitle}pt`, fontWeight: 700 }}>CULINARY PROFILE</h2><p className="cf-summary-text" style={{ fontSize: `${fontSizes.regularText}pt`, fontWeight: 400 }}>{formData.summary}</p></section>}
        {hasExperience && <section className="cf-section"><h2 className="cf-section-title" style={{ fontSize: `${fontSizes.sectionTitle}pt`, fontWeight: 700 }}>CULINARY EXPERIENCE</h2>{formData.experience.map((exp, i) => (<div key={i} className="cf-experience-item"><div className="cf-experience-header"><h3 style={{ fontSize: `${fontSizes.jobTitle}pt`, fontWeight: 700 }}>{exp.position}</h3><p className="cf-company" style={{ fontSize: `${fontSizes.restaurant}pt`, fontWeight: 400 }}>{exp.restaurant}{exp.cuisineType && ` – ${exp.cuisineType}`} | {exp.startDate} – {exp.endDate || 'Present'}</p></div><ul className="cf-bullet-list">{exp.description.split('\n').filter(l => l.trim()).map((line, j) => <li key={j} style={{ fontSize: `${fontSizes.bulletText}pt`, fontWeight: 400 }}>{line}</li>)}</ul></div>))}</section>}
        {hasEducation && <section className="cf-section"><h2 className="cf-section-title" style={{ fontSize: `${fontSizes.sectionTitle}pt`, fontWeight: 700 }}>CULINARY EDUCATION</h2>{formData.education.map((edu, i) => (<div key={i} className="cf-education-item"><h3 style={{ fontSize: `${fontSizes.degree}pt`, fontWeight: 700 }}>{edu.degree}{edu.program && ` – ${edu.program}`}</h3><p className="cf-institution-text" style={{ fontSize: `${fontSizes.institution}pt`, fontWeight: 400 }}>{edu.institution} | <span className="cf-institution-date" style={{ fontSize: `${fontSizes.institutionDate}pt`, fontWeight: 400 }}>{edu.startDate} – {edu.endDate || 'Present'}</span></p></div>))}</section>}
        {hasCuisines && <section className="cf-section"><h2 className="cf-section-title" style={{ fontSize: `${fontSizes.sectionTitle}pt`, fontWeight: 700 }}>CUISINE SPECIALTIES</h2><ul className="cf-skills-list">{formData.cuisines.map((s, i) => <li key={i} style={{ fontSize: `${fontSizes.skillText}pt`, fontWeight: 400 }}>{s.name}</li>)}</ul></section>}
        {hasCertifications && <section className="cf-section"><h2 className="cf-section-title" style={{ fontSize: `${fontSizes.sectionTitle}pt`, fontWeight: 700 }}>CERTIFICATIONS & TRAINING</h2>{formData.certifications.map((c, i) => (<div key={i} className="cf-certification-item" style={{ fontSize: `${fontSizes.certificationText}pt`, fontWeight: 400 }}><strong style={{ fontWeight: 700 }}>{c.name}</strong>{c.issuingAuthority && ` – ${c.issuingAuthority}`}{c.certificateNumber && ` (Cert #: ${c.certificateNumber})`}{c.expiryDate && ` – Expires: ${c.expiryDate}`}</div>))}</section>}
        {hasTechniques && <section className="cf-section"><h2 className="cf-section-title" style={{ fontSize: `${fontSizes.sectionTitle}pt`, fontWeight: 700 }}>CULINARY TECHNIQUES & SKILLS</h2><ul className="cf-bullet-list">{formData.techniques.map((p, i) => <li key={i} style={{ fontSize: `${fontSizes.bulletText}pt`, fontWeight: 400 }}>{p.name}</li>)}</ul></section>}
    ...
    <>
      ...
      <div className="cf-resume-builder" lang="en-US">
        ...
        <nav className="cf-breadcrumb" aria-label="Breadcrumb">
          <ol>
            <li><Link href="https://professionalresumefree.com" className="cf-breadcrumb-link" prefetch={false}><FiHome className="cf-breadcrumb-icon"/><span className="cf-breadcrumb-text">Chef Resume Builder</span></Link></li>
            <li className="cf-breadcrumb-separator"><FiChevronRightIcon/></li>
            <li><Link href="/ats-friendly-chef-cook-resume-builder" className="cf-breadcrumb-link" prefetch={false}><span className="cf-breadcrumb-text">Free Culinary Template</span></Link></li>
          </ol>
        </nav>
            ...
            <div className="cf-hero-content">
              ...
              <div className="cf-cta-buttons">
                <button onClick={() => setActiveSection('personal')} className="cf-btn-primary" aria-label="Start building your free chef resume now—no sign-up required">
                  <span className="cf-btn-text">Start Building Your Chef Resume Now</span>
                  <FiArrowRight className="cf-btn-icon" />
                  <div className="cf-btn-pulse"></div>
                </button>
                <button onClick={generatePDF} className="cf-btn-secondary" aria-label="Download chef resume as PDF" disabled={isGeneratingPDF || !hasContent()}><FiDownload className="cf-btn-icon"/><span className="cf-btn-text">Download Chef Resume PDF</span></button>
              </div>
              ...
              <div className="cf-industry-badges">
                <div className="cf-badge-grid">
                  <span className="cf-badge-item"><FiCoffee/> Executive Chef Templates</span>
                  <span className="cf-badge-item"><FiHeart/> Pastry Chef Resumes</span>
                  <span className="cf-badge-item"><FiUsers/> Sous Chef Templates</span>
                  <span className="cf-badge-item"><FiTool/> Line Cook Resumes</span>
                  <span className="cf-badge-item"><FiHome/> Private Chef CVs</span>
                  <span className="cf-badge-item"><FiBriefcase/> Restaurant Manager</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="cf-layout">
          <div className="cf-preview-section">
            ...
            <div className={`cf-preview-container ${showFullPreview ? 'cf-full-preview' : ''}`}><div className="cf-resume-card"><div className="cf-preview-content"><div className="cf-resume-preview" ref={resumeRef}><ChefTemplate formData={formData}/></div></div></div></div>
          </div>
          <div className="cf-form-section">
            <div className="cf-form-nav">
              {[{id:'personal',label:'Personal',icon:<FiUser/>},{id:'experience',label:'Culinary Experience',icon:<FiBriefcase/>},{id:'education',label:'Education',icon:<FiBook/>},{id:'cuisines',label:'Specialties',icon:<FiUsers/>},{id:'certifications',label:'Certifications',icon:<FiAward/>},{id:'settings',label:'Font Settings',icon:<FiSettings/>}].map(item => (<button key={item.id} className={`cf-nav-btn ${activeSection===item.id?'cf-nav-btn-active':''}`} onClick={()=>setActiveSection(item.id)}>{item.icon}<span>{item.label}</span></button>))}
            </div>
            <div className="cf-form-content">
              {activeSection==='personal'&&(
                <div className="cf-form-section-content">
                  ...
                  <div className="cf-form-card">
                    <div className="cf-form-group"><label className="cf-form-label">Full Name*<input type="text" name="fullName" value={formData.fullName} onChange={handleInputChange} placeholder="Marco Rossi, Executive Chef" required className="cf-form-input"/></label><label className="cf-form-label">Email*<input type="email" name="email" value={formData.email} onChange={handleInputChange} placeholder="marco.rossi@culinary.com" required className="cf-form-input"/></label></div>
                    <div className="cf-form-group"><label className="cf-form-label">Phone<input type="tel" name="phone" value={formData.phone} onChange={handleInputChange} placeholder="(555) 123-4567" className="cf-form-input"/></label><label className="cf-form-label">Location<input type="text" name="address" value={formData.address} onChange={handleInputChange} placeholder="City, Country" className="cf-form-input"/></label></div>
                  </div>
                  <div className="cf-form-card"><label className="cf-form-label">Culinary Profile & Summary*<textarea name="summary" value={formData.summary} onChange={handleInputChange} placeholder="Creative and passionate Executive Chef with 12+ years of experience in fine dining and Michelin-starred restaurants. Specialized in modern Italian and French cuisine with expertise in menu development, kitchen management, and culinary innovation. Proven track record of increasing restaurant revenue by 30% through innovative menu concepts and exceptional quality control." required className="cf-form-textarea" rows="6"/><div className="cf-char-count">{formData.summary.length}/500 characters</div></label></div>
                  <div className="cf-form-card">
                    <h4 className="cf-sub-section-title"><FiGlobe/> Professional Links</h4><p className="cf-section-desc">Add your professional culinary profiles (LinkedIn, Instagram, portfolio, etc.)</p>
                    <div className="cf-social-input">
                      <select value={currentSocialLink.platform} onChange={e=>setCurrentSocialLink({...currentSocialLink,platform:e.target.value})} className="cf-form-select"><option value="">Select Platform</option><option value="LinkedIn">LinkedIn</option><option value="Instagram">Instagram</option><option value="Portfolio">Culinary Portfolio</option><option value="Website">Personal Website</option><option value="YouTube">YouTube Channel</option><option value="Facebook">Facebook Page</option></select>
              ...
              {activeSection==='experience'&&(
                <div className="cf-form-section-content">
                  <h3 className="cf-form-section-title"><FiBriefcase/> Culinary Experience</h3>
                  <p className="cf-section-desc">List your kitchen positions in reverse chronological order (most recent first)</p>
                  <div className="cf-form-card">
                    <div className="cf-form-group"><label className="cf-form-label">Position Title*<input value={currentExperience.position} onChange={e=>setCurrentExperience({...currentExperience,position:e.target.value})} placeholder="Executive Chef, Head Chef, Sous Chef" required className="cf-form-input"/></label><label className="cf-form-label">Restaurant/Establishment*<input value={currentExperience.restaurant} onChange={e=>setCurrentExperience({...currentExperience,restaurant:e.target.value})} placeholder="La Belle Cuisine Restaurant" required className="cf-form-input"/></label></div>
                    <label className="cf-form-label">Cuisine Type / Restaurant Style<input value={currentExperience.cuisineType} onChange={e=>setCurrentExperience({...currentExperience,cuisineType:e.target.value})} placeholder="Fine Dining Italian / Michelin-starred French" className="cf-form-input"/></label>
                    <div className="cf-form-group"><label className="cf-form-label">Start Date*<input type="text" placeholder="Month Year (e.g., March 2018)" value={currentExperience.startDate} onChange={e=>setCurrentExperience({...currentExperience,startDate:e.target.value})} required className="cf-form-input"/></label><label className="cf-form-label">End Date<input type="text" placeholder="Month Year or Present" value={currentExperience.endDate} onChange={e=>setCurrentExperience({...currentExperience,endDate:e.target.value})} className="cf-form-input"/></label></div>
                    <label className="cf-form-label">Key Culinary Responsibilities & Achievements*<textarea value={currentExperience.description} onChange={e=>setCurrentExperience({...currentExperience,description:e.target.value})} placeholder="• Managed kitchen operations with team of 15 chefs and cooks
...
• Implemented HACCP and food safety protocols achieving 100% compliance" required className="cf-form-textarea" rows="8"/><div className="cf-char-count">{currentExperience.description.length}/2000 characters</div></label>
                    <div className="cf-form-actions"><button onClick={addExperience} className="cf-add-btn" disabled={!currentExperience.position||!currentExperience.restaurant||!currentExperience.startDate}><FiPlus/> {currentExperience.isEditing?'Update Culinary Experience':'Add Culinary Experience'}</button>{currentExperience.isEditing&&<button onClick={()=>setCurrentExperience(defaultExperience())} className="cf-cancel-btn"><FiX/> Cancel</button>}</div>
                  </div>
                  <div className="cf-form-card"><h4 className="cf-sub-section-title">Your Culinary Experience</h4>{formData.experience.length===0?<p className="cf-empty-msg">No culinary experience added yet</p>:<div className="cf-items-list">{formData.experience.map((exp,i)=>(<div key={i} className="cf-list-item"><div className="cf-item-content"><div className="cf-item-header"><strong className="cf-item-title">{exp.position}</strong><span className="cf-item-subtitle">at {exp.restaurant}</span></div><div className="cf-item-meta"><span>{exp.startDate} – {exp.endDate||'Present'}</span>{exp.cuisineType&&<span>{exp.cuisineType}</span>}</div><div className="cf-item-desc">{exp.description.split('\n').filter(l=>l.trim()).map((line,j)=><p key={j} className="cf-bullet-point">• {line}</p>)}</div></div><div className="cf-item-actions"><button onClick={()=>editExperience(i)} className="cf-edit-btn" aria-label={`Edit ${exp.position} experience`}><FiEdit2/></button><button onClick={()=>deleteExperience(i)} className="cf-delete-btn" aria-label={`Delete ${exp.position} experience`}><FiTrash2/></button></div></div>))}</div>}</div>
                </div>
              )}
              {activeSection==='education'&&(
                <div className="cf-form-section-content">
                  <h3 className="cf-form-section-title"><FiBook/> Culinary Education & Training</h3>
                  <p className="cf-section-desc">List your culinary education, degrees, and professional training</p>
                  <div className="cf-form-card">
                    <div className="cf-form-group"><label className="cf-form-label">Institution*<input value={currentEducation.institution} onChange={e=>setCurrentEducation({...currentEducation,institution:e.target.value})} placeholder="Le Cordon Bleu Culinary Institute" required className="cf-form-input"/></label><label className="cf-form-label">Degree/Certification*<input value={currentEducation.degree} onChange={e=>setCurrentEducation({...currentEducation,degree:e.target.value})} placeholder="Grand Diplôme in Culinary Arts" required className="cf-form-input"/></label></div>
                    <label className="cf-form-label">Program / Specialization<input value={currentEducation.program} onChange={e=>setCurrentEducation({...currentEducation,program:e.target.value})} placeholder="French Culinary Arts, Pastry & Baking" className="cf-form-input"/></label>
                  ...
                  <div className="cf-form-card"><h4 className="cf-sub-section-title">Your Culinary Education</h4>{formData.education.length===0?<p className="cf-empty-msg">No education added yet</p>:<div className="cf-items-list">{formData.education.map((edu,i)=>(<div key={i} className="cf-list-item"><div className="cf-item-content"><div className="cf-item-header"><strong className="cf-item-title">{edu.degree}</strong>{edu.program&&<span className="cf-item-subtitle"> – {edu.program}</span>}</div><div className="cf-item-meta"><span>{edu.institution}</span><span>{edu.startDate} – {edu.endDate||'Present'}</span></div></div><div className="cf-item-actions"><button onClick={()=>editEducation(i)} className="cf-edit-btn" aria-label={`Edit ${edu.degree}`}><FiEdit2/></button><button onClick={()=>deleteEducation(i)} className="cf-delete-btn" aria-label={`Delete ${edu.degree}`}><FiTrash2/></button></div></div>))}</div>}</div>
                </div>
              )}
              {activeSection==='cuisines'&&(
                <div className="cf-form-section-content">
                  <h3 className="cf-form-section-title"><FiUsers/> Culinary Specialties & Certifications</h3>
                  <div className="cf-form-card">
                    <h4 className="cf-sub-section-title">Cuisine Specialties</h4><p className="cf-section-desc">List your culinary specialties and cuisine expertise</p>
                    <div className="cf-skills-input">
                      <input value={currentCuisine.name} onChange={e=>setCurrentCuisine({...currentCuisine,name:e.target.value})} placeholder="Modern Italian, French Haute Cuisine, Japanese Sushi, Vegan & Plant-based" className="cf-form-input"/>
                      <div className="cf-form-actions"><button onClick={addCuisine} className="cf-add-btn" disabled={!currentCuisine.name.trim()}><FiPlus/> {currentCuisine.isEditing?'Update Cuisine':'Add Cuisine'}</button>{currentCuisine.isEditing&&<button onClick={()=>setCurrentCuisine(defaultCuisine())} className="cf-cancel-btn"><FiX/> Cancel</button>}</div>
                    </div>
                    <div className="cf-items-list">{formData.cuisines.map((s,i)=>(<div key={i} className="cf-list-item"><span>{s.name}</span><div className="cf-item-actions"><button onClick={()=>editCuisine(i)} className="cf-edit-btn"><FiEdit2/></button><button onClick={()=>deleteCuisine(i)} className="cf-delete-btn"><FiTrash2/></button></div></div>))}{formData.cuisines.length===0&&<p className="cf-empty-msg">No cuisine specialties added yet</p>}</div>
                  </div>
                  <div className="cf-form-card">
                    <h4 className="cf-sub-section-title"><FiAward/> Culinary Certifications</h4><p className="cf-section-desc">Add your professional culinary certifications and training</p>
                    <div className="cf-skills-input">
                      <div className="cf-form-group"><label className="cf-form-label">Certification Name*<input value={currentCertification.name} onChange={e=>setCurrentCertification({...currentCertification,name:e.target.value})} placeholder="ServSafe Food Protection Manager" className="cf-form-input"/></label><label className="cf-form-label">Issuing Authority<input value={currentCertification.issuingAuthority} onChange={e=>setCurrentCertification({...currentCertification,issuingAuthority:e.target.value})} placeholder="National Restaurant Association" className="cf-form-input"/></label></div>
                      <div className="cf-form-group"><label className="cf-form-label">Certificate Number<input value={currentCertification.certificateNumber} onChange={e=>setCurrentCertification({...currentCertification,certificateNumber:e.target.value})} placeholder="123456789" className="cf-form-input"/></label><label className="cf-form-label">Expiry Date<input value={currentCertification.expiryDate} onChange={e=>setCurrentCertification({...currentCertification,expiryDate:e.target.value})} placeholder="Month Year" className="cf-form-input"/></label></div>
                  ...
                  <div className="cf-form-card">
                    <h4 className="cf-sub-section-title">Culinary Techniques & Skills</h4><p className="cf-section-desc">List your culinary techniques, cooking methods, and kitchen skills</p>
                    <div className="cf-skills-input">
                      <input value={currentTechnique.name} onChange={e=>setCurrentTechnique({...currentTechnique,name:e.target.value})} placeholder="Sous-vide, Molecular Gastronomy, Sauce Making, Butchery, Plating" className="cf-form-input"/>
                      <div className="cf-form-actions"><button onClick={addTechnique} className="cf-add-btn" disabled={!currentTechnique.name.trim()}><FiPlus/> {currentTechnique.isEditing?'Update Technique':'Add Technique'}</button>{currentTechnique.isEditing&&<button onClick={()=>setCurrentTechnique(defaultTechnique())} className="cf-cancel-btn"><FiX/> Cancel</button>}</div>
                    </div>
                    <div className="cf-items-list">{formData.techniques.map((p,i)=>(<div key={i} className="cf-list-item"><span>{p.name}</span><div className="cf-item-actions"><button onClick={()=>editTechnique(i)} className="cf-edit-btn"><FiEdit2/></button><button onClick={()=>deleteTechnique(i)} className="cf-delete-btn"><FiTrash2/></button></div></div>))}{formData.techniques.length===0&&<p className="cf-empty-msg">No techniques added yet</p>}</div>
                  </div>
                  <div className="cf-form-card">
                    <h4 className="cf-sub-section-title">Professional Culinary Affiliations</h4><p className="cf-section-desc">Add your professional culinary organization memberships</p>
                    <div className="cf-skills-input">
                      <div className="cf-form-group"><label className="cf-form-label">Organization*<input value={currentAffiliation.organization} onChange={e=>setCurrentAffiliation({...currentAffiliation,organization:e.target.value})} placeholder="American Culinary Federation (ACF)" className="cf-form-input"/></label><label className="cf-form-label">Role / Membership Type<input value={currentAffiliation.role} onChange={e=>setCurrentAffiliation({...currentAffiliation,role:e.target.value})} placeholder="Certified Executive Chef (CEC), Member" className="cf-form-input"/></label></div>
                  ...
                  <div className="cf-form-card">
                    ...
                    <div className="cf-skills-input">
                      <div className="cf-form-group"><label className="cf-form-label">Language*<input value={currentLanguage.name} onChange={e=>setCurrentLanguage({...currentLanguage,name:e.target.value})} placeholder="Italian, French, Japanese" className="cf-form-input"/></label><label className="cf-form-label">Proficiency Level<input value={currentLanguage.proficiency} onChange={e=>setCurrentLanguage({...currentLanguage,proficiency:e.target.value})} placeholder="Native, Fluent, Conversational" className="cf-form-input"/></label></div>
        ...
        <section className="cf-faq-section" aria-labelledby="faq-title">
          <div className="cf-container">
            <div className="cf-section-header">
              ...
              <p className="cf-section-subtitle">Everything you need to know about creating professional chef resumes with our tool.</p>
            </div>
        ...
        <section className="cf-cta-section" aria-labelledby="cta-title">
          <div className="cf-container">
            <div className="cf-cta-content">
              <h2 className="cf-cta-title" id="cta-title">Ready to Advance Your Culinary Career?</h2>
              <p className="cf-cta-subtitle">Join 2 million+ culinary professionals who landed their dream jobs with our free ATS-friendly chef resume builder.</p>
              <div className="cf-cta-btn-wrap">
                <button onClick={()=>setActiveSection('personal')} className="cf-cta-btn" aria-label="Create your free chef resume now—no sign-up required">
                  <span className="cf-cta-btn-text">Create Your Free Chef Resume Now</span>
                  <FiArrowRight className="cf-cta-btn-icon"/>
                </button>
              </div>
              <div className="cf-cta-guarantee"><FiCheck className="cf-guarantee-icon"/><span className="cf-guarantee-text">No credit card required • Free forever • Download in minutes • ATS Optimized for Culinary Industry</span></div>
            </div>
          </div>
        </section>
...
export async function getStaticProps() {
  ...
  const breadcrumbData = [
    ...
    { name: 'Chef Resume Builder', item: 'https://professionalresumefree.com/ats-friendly-chef-cook-resume-builder' }
  ];
