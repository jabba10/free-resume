...
const careerFlowStyles = `
  ...
  * { box-sizing: border-box; }
  ...
  .cf-construction-template { color: #000000; line-height: 1.25; height: 100%; }
  ...
  .cf-cert-item, .cf-safety-item, .cf-equipment-item { margin-bottom: 3pt; line-height: 1.25; }
  ...
  .cf-modal-page .cf-construction-template { width: 100%; height: 100%; padding: 10mm 15mm; box-sizing: border-box; overflow-y: auto; }
  ...
  @media (max-width: 768px) {
    ...
    .cf-modal-page .cf-construction-template { padding: 8mm 12mm; }
  ...
  @media (max-width: 480px) {
    ...
    .cf-modal-page .cf-construction-template { padding: 6mm 8mm; }
  ...
  @media print {
    ...
    .cf-construction-template, .cf-section { page-break-inside: avoid; }
  }
`;

const ConstructionResume = ({ seoData, buildTimestamp }) => {
  const {
    currentDate,
    lastModifiedDate,
    reviewDates,
  ...
  const defaultSkill = () => ({ name: '', category: '', yearsExperience: '', isEditing: false, editIndex: null });
  const defaultCertification = () => ({ name: '', issuingAuthority: '', licenseNumber: '', expiryDate: '', isEditing: false, editIndex: null });
  const defaultSafety = () => ({ name: '', level: '', isEditing: false, editIndex: null });
  const defaultEquipment = () => ({ name: '', yearsExperience: '', isEditing: false, editIndex: null });
  ...
  const [formData, setFormData] = useState({ fullName: '', email: '', phone: '', address: '', summary: '', experience: [], education: [], skills: [], certifications: [], safetyTraining: [], equipment: [], languages: [], socialLinks: [], specialization: '', yearsExperience: '', availability: '' });
  ...
  const faqs = [
    { question: "Is this construction resume builder really free with no hidden costs?", answer: "Yes, our construction resume builder is completely free with no hidden costs or watermarks. Create, edit, and download your professional construction resume in PDF format without any payment required." },
    { question: "What does ATS-friendly mean for construction resumes?", answer: "ATS-friendly means our construction resume templates are optimized to pass through Applicant Tracking Systems used by 85% of construction companies and contractors. This ensures your trade skills and experience are properly scanned and recognized." },
    { question: "Can I download my construction resume as PDF without creating an account?", answer: "Absolutely! Download your professional construction resume in PDF format without creating an account. Everything is completely free and accessible immediately for all construction trades." },
    { question: "How many construction resume templates are available for free?", answer: "We offer professionally designed ATS-friendly construction resume templates for carpenters, electricians, plumbers, heavy equipment operators, foremen, supervisors, and all construction trades. All templates are completely free and optimized for construction hiring." },
    { question: "How does your construction resume builder work?", answer: "Our builder uses ATS-optimized construction templates with proper trade terminology formatting. We guide you to highlight construction experience, certifications, safety training, and specialized skills that contractors look for." },
    { question: "Can I edit my construction resume after downloading it?", answer: "Yes, you can always come back and edit your construction resume. Your work saves automatically, and you can download updated versions as many times as needed—completely free." }
  ];
  ...
  const hasContent = () => formData.fullName || formData.email || formData.summary || formData.socialLinks.length > 0 || formData.experience.length > 0 || formData.education.length > 0 || formData.skills.length > 0 || formData.certifications.length > 0 || formData.safetyTraining.length > 0 || formData.equipment.length > 0 || formData.languages.length > 0;
  ...
  const ConstructionTemplate = ({ formData }) => {
    ...
    return (
      <div className="cf-construction-template">
        <header className="cf-resume-header">
          ...
          {formData.specialization && <p className="cf-professional-title" style={{ fontSize: `${fontSizes.jobTitle}pt`, fontWeight: 400 }}>{formData.specialization}</p>}
          <div className="cf-contact-row">
            ...
            {formData.yearsExperience && <><div className="cf-contact-separator">•</div><div className="cf-contact-item" style={{ fontSize: `${fontSizes.contactInfo}pt`, fontWeight: 400 }}><FiClock /> {formData.yearsExperience} years experience</div></>}
            {formData.availability && <><div className="cf-contact-separator">•</div><div className="cf-contact-item" style={{ fontSize: `${fontSizes.contactInfo}pt`, fontWeight: 400 }}><FiCheck /> {formData.availability}</div></>}
          </div>
        </header>
        {hasSummary && <section className="cf-section"><h2 className="cf-section-title" style={{ fontSize: `${fontSizes.sectionTitle}pt`, fontWeight: 700 }}>PROFESSIONAL PROFILE</h2><p className="cf-summary-text" style={{ fontSize: `${fontSizes.regularText}pt`, fontWeight: 400 }}>{formData.summary}</p></section>}
        {hasExperience && <section className="cf-section"><h2 className="cf-section-title" style={{ fontSize: `${fontSizes.sectionTitle}pt`, fontWeight: 700 }}>CONSTRUCTION EXPERIENCE</h2>{formData.experience.map((exp, i) => (<div key={i} className="cf-experience-item"><div className="cf-experience-header"><h3 style={{ fontSize: `${fontSizes.jobTitle}pt`, fontWeight: 700 }}>{exp.position}</h3><p className="cf-company" style={{ fontSize: `${fontSizes.company}pt`, fontWeight: 400 }}>{exp.employer}{exp.projectType && ` – ${exp.projectType}`} | {exp.startDate} – {exp.endDate || 'Present'}</p></div><ul className="cf-bullet-list">{exp.description.split('\n').filter(l => l.trim()).map((line, j) => <li key={j} style={{ fontSize: `${fontSizes.bulletText}pt`, fontWeight: 400 }}>{line}</li>)}</ul></div>))}</section>}
        {hasSkills && <section className="cf-section"><h2 className="cf-section-title" style={{ fontSize: `${fontSizes.sectionTitle}pt`, fontWeight: 700 }}>TRADE SKILLS & EXPERTISE</h2>{formData.skills.map((s, i) => (<div key={i} style={{ marginBottom: '3pt' }}><strong style={{ fontSize: `${fontSizes.skillText}pt`, fontWeight: 700 }}>{s.name}</strong>{s.category && <span style={{ fontSize: `${fontSizes.skillText}pt`, fontWeight: 400 }}> ({s.category})</span>}{s.yearsExperience && <span style={{ fontSize: `${fontSizes.skillText}pt`, fontWeight: 400 }}> – {s.yearsExperience} years</span>}</div>))}</section>}
        {hasCertifications && <section className="cf-section"><h2 className="cf-section-title" style={{ fontSize: `${fontSizes.sectionTitle}pt`, fontWeight: 700 }}>CERTIFICATIONS & LICENSES</h2>{formData.certifications.map((c, i) => (<div key={i} className="cf-cert-item" style={{ fontSize: `${fontSizes.licenseText}pt`, fontWeight: 400 }}><strong style={{ fontWeight: 700 }}>{c.name}</strong>{c.issuingAuthority && ` – ${c.issuingAuthority}`}{c.licenseNumber && ` (#${c.licenseNumber})`}{c.expiryDate && ` – Valid until: ${c.expiryDate}`}</div>))}</section>}
        {hasSafety && <section className="cf-section"><h2 className="cf-section-title" style={{ fontSize: `${fontSizes.sectionTitle}pt`, fontWeight: 700 }}>SAFETY TRAINING & QUALIFICATIONS</h2>{formData.safetyTraining.map((s, i) => (<div key={i} className="cf-safety-item" style={{ fontSize: `${fontSizes.skillText}pt`, fontWeight: 400 }}><strong style={{ fontWeight: 700 }}>{s.name}</strong>{s.level && ` – ${s.level}`}</div>))}</section>}
        {hasEquipment && <section className="cf-section"><h2 className="cf-section-title" style={{ fontSize: `${fontSizes.sectionTitle}pt`, fontWeight: 700 }}>EQUIPMENT OPERATION</h2>{formData.equipment.map((e, i) => (<div key={i} className="cf-equipment-item" style={{ fontSize: `${fontSizes.skillText}pt`, fontWeight: 400 }}><strong style={{ fontWeight: 700 }}>{e.name}</strong>{e.yearsExperience && ` – ${e.yearsExperience} years`}</div>))}</section>}
      ...
      <div className="cf-resume-builder" lang="en-US">
        ...
        <nav className="cf-breadcrumb" aria-label="Breadcrumb">
          <ol>
            ...
            <li><span className="cf-breadcrumb-text">Free Trade Templates</span></li>
          </ol>
        </nav>
        ...
        <section className="cf-hero">
          <div className="cf-container">
            <div className="cf-hero-content">
              <div className="cf-trust-badge"><FiStar className="cf-trust-icon"/><span className="cf-trust-text">Best Free Construction Resume Builder 2026</span></div>
              <h1 className="cf-hero-title">Free Construction Resume <span className="cf-gradient-text">Builder 2026</span></h1>
              <p className="cf-hero-subtitle">Create a <strong className="cf-hero-highlight">professional, ATS-optimized construction resume for free in minutes.</strong> Our trade resume builder ensures your construction experience and skills get noticed by contractors and employers globally.</p>
              <div className="cf-cta-buttons">
                <button onClick={() => setActiveSection('personal')} className="cf-btn-primary" aria-label="Start building your free construction resume now—no sign-up required">
                  <span className="cf-btn-text">Start Building Your Construction Resume Now</span>
                  <FiArrowRight className="cf-btn-icon" />
                  <div className="cf-btn-pulse"></div>
                </button>
                <button onClick={generatePDF} className="cf-btn-secondary" aria-label="Download construction resume as PDF" disabled={isGeneratingPDF || !hasContent()}><FiDownload className="cf-btn-icon"/><span className="cf-btn-text">Download Construction Resume PDF</span></button>
              </div>
              <div className="cf-hero-stats">
                <div className="cf-stat-item"><span className="cf-stat-number">3M+</span><span className="cf-stat-label">Construction Pros</span></div>
              ...
              <div className="cf-industry-badges">
                <div className="cf-badge-grid">
                  <span className="cf-badge-item"><FiTool/> Carpenter</span>
                  <span className="cf-badge-item"><FiDroplet/> Plumber</span>
                  <span className="cf-badge-item"><FiThermometer/> Electrician</span>
                  <span className="cf-badge-item"><FiTruck/> Equipment Operator</span>
                  <span className="cf-badge-item"><FiShield/> Construction Laborer</span>
                  <span className="cf-badge-item"><FiBriefcase/> Construction Manager</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="cf-layout">
          <div className="cf-preview-section">
            ...
            <div className={`cf-preview-container ${showFullPreview ? 'cf-full-preview' : ''}`}><div className="cf-resume-card"><div className="cf-preview-content"><div className="cf-resume-preview" ref={resumeRef}><ConstructionTemplate formData={formData}/></div></div></div></div>
          </div>
          <div className="cf-form-section">
            <div className="cf-form-nav">
              {[{id:'personal',label:'Personal',icon:<FiUser/>},{id:'experience',label:'Construction Exp.',icon:<FiBriefcase/>},{id:'skills',label:'Trade Skills',icon:<FiTool/>},{id:'certifications',label:'Certifications',icon:<FiShield/>},{id:'settings',label:'Font Settings',icon:<FiSettings/>}].map(item => (<button key={item.id} className={`cf-nav-btn ${activeSection===item.id?'cf-nav-btn-active':''}`} onClick={()=>setActiveSection(item.id)}>{item.icon}<span>{item.label}</span></button>))}
            </div>
            <div className="cf-form-content">
              {activeSection==='personal'&&(
                <div className="cf-form-section-content">
                  ...
                  <div className="cf-form-card">
                    <div className="cf-form-group"><label className="cf-form-label">Full Name*<input type="text" name="fullName" value={formData.fullName} onChange={handleInputChange} placeholder="John Smith, Carpenter Foreman" required className="cf-form-input"/></label><label className="cf-form-label">Email*<input type="email" name="email" value={formData.email} onChange={handleInputChange} placeholder="john.smith@construction.com" required className="cf-form-input"/></label></div>
                    ...
                    <div className="cf-form-group"><label className="cf-form-label">Primary Trade/Specialization*<input type="text" name="specialization" value={formData.specialization} onChange={handleInputChange} placeholder="Carpenter Foreman, Licensed Electrician" required className="cf-form-input"/></label><label className="cf-form-label">Years of Experience*<input type="text" name="yearsExperience" value={formData.yearsExperience} onChange={handleInputChange} placeholder="10+ years" required className="cf-form-input"/></label></div>
                    <label className="cf-form-label">Availability/Work Status*<input type="text" name="availability" value={formData.availability} onChange={handleInputChange} placeholder="Available Immediately, Open to Relocation" required className="cf-form-input"/></label>
                  </div>
                  <div className="cf-form-card"><label className="cf-form-label">Professional Construction Summary*<textarea name="summary" value={formData.summary} onChange={handleInputChange} placeholder="Skilled and safety-conscious Carpenter Foreman with 12+ years of experience in commercial and residential construction. Proven track record of leading teams, managing projects on schedule and budget, and maintaining strict quality standards. Expertise in blueprint reading, framing, finishing work, and training apprentices. Committed to workplace safety and delivering exceptional results." required className="cf-form-textarea" rows="6"/><div className="cf-char-count">{formData.summary.length}/500 characters</div></label></div>
                  <div className="cf-form-card">
              ...
              {activeSection==='experience'&&(
                <div className="cf-form-section-content">
                  <h3 className="cf-form-section-title"><FiBriefcase/> Construction Experience</h3>
                  <p className="cf-section-desc">List your construction positions in reverse chronological order (most recent first)</p>
                  <div className="cf-form-card">
                    <div className="cf-form-group"><label className="cf-form-label">Position Title*<input value={currentExperience.position} onChange={e=>setCurrentExperience({...currentExperience,position:e.target.value})} placeholder="Carpenter Foreman, Journeyman Electrician" required className="cf-form-input"/></label><label className="cf-form-label">Company/Contractor*<input value={currentExperience.employer} onChange={e=>setCurrentExperience({...currentExperience,employer:e.target.value})} placeholder="ABC Construction Company" required className="cf-form-input"/></label></div>
                    <label className="cf-form-label">Project Type / Sector<input value={currentExperience.projectType} onChange={e=>setCurrentExperience({...currentExperience,projectType:e.target.value})} placeholder="Commercial High-Rise, Residential Housing" className="cf-form-input"/></label>
                    ...
                    <label className="cf-form-label">Key Responsibilities & Achievements*<textarea value={currentExperience.description} onChange={e=>setCurrentExperience({...currentExperience,description:e.target.value})} placeholder="• Led team of 8 carpenters on $5M commercial project
...
• Completed projects 10% under budget through efficient resource management" required className="cf-form-textarea" rows="8"/><div className="cf-char-count">{currentExperience.description.length}/2000 characters</div></label>
                    <div className="cf-form-actions"><button onClick={addExperience} className="cf-add-btn" disabled={!currentExperience.position||!currentExperience.employer||!currentExperience.startDate}><FiPlus/> {currentExperience.isEditing?'Update Construction Experience':'Add Construction Experience'}</button>{currentExperience.isEditing&&<button onClick={()=>setCurrentExperience(defaultExperience())} className="cf-cancel-btn"><FiX/> Cancel</button>}</div>
                  </div>
                  <div className="cf-form-card"><h4 className="cf-sub-section-title">Your Construction Experience</h4>{formData.experience.length===0?<p className="cf-empty-msg">No construction experience added yet</p>:<div className="cf-items-list">{formData.experience.map((exp,i)=>(<div key={i} className="cf-list-item"><div className="cf-item-content"><div className="cf-item-header"><strong className="cf-item-title">{exp.position}</strong><span className="cf-item-subtitle">at {exp.employer}</span></div><div className="cf-item-meta"><span>{exp.startDate} – {exp.endDate||'Present'}</span>{exp.projectType&&<span>{exp.projectType}</span>}</div><div className="cf-item-desc">{exp.description.split('\n').filter(l=>l.trim()).map((line,j)=><p key={j} className="cf-bullet-point">• {line}</p>)}</div></div><div className="cf-item-actions"><button onClick={()=>editExperience(i)} className="cf-edit-btn" aria-label={`Edit ${exp.position} experience`}><FiEdit2/></button><button onClick={()=>deleteExperience(i)} className="cf-delete-btn" aria-label={`Delete ${exp.position} experience`}><FiTrash2/></button></div></div>))}</div>}</div>
                  <div className="cf-form-card">
                    <h4 className="cf-sub-section-title"><FiBook/> Education & Training</h4><p className="cf-section-desc">List your trade education, apprenticeships, and relevant training</p>
                    <div className="cf-form-group"><label className="cf-form-label">Institution/Trade School*<input value={currentEducation.institution} onChange={e=>setCurrentEducation({...currentEducation,institution:e.target.value})} placeholder="ABC Trade School" required className="cf-form-input"/></label><label className="cf-form-label">Degree/Certification*<input value={currentEducation.degree} onChange={e=>setCurrentEducation({...currentEducation,degree:e.target.value})} placeholder="Journeyman Carpenter License" required className="cf-form-input"/></label></div>
                    <label className="cf-form-label">Program / Specialization<input value={currentEducation.program} onChange={e=>setCurrentEducation({...currentEducation,program:e.target.value})} placeholder="Commercial Carpentry" className="cf-form-input"/></label>
                    <div className="cf-form-group"><label className="cf-form-label">Start Date<input type="text" placeholder="Month Year" value={currentEducation.startDate} onChange={e=>setCurrentEducation({...currentEducation,startDate:e.target.value})} className="cf-form-input"/></label><label className="cf-form-label">End Date / Expected<input type="text" placeholder="Month Year or Completed" value={currentEducation.endDate} onChange={e=>setCurrentEducation({...currentEducation,endDate:e.target.value})} className="cf-form-input"/></label></div>
                    ...
                    <div className="cf-items-list">{formData.education.map((edu,i)=>(<div key={i} className="cf-list-item"><div className="cf-item-content"><div className="cf-item-header"><strong className="cf-item-title">{edu.degree}</strong>{edu.program&&<span className="cf-item-subtitle"> – {edu.program}</span>}</div><div className="cf-item-meta"><span>{edu.institution}</span><span>{edu.startDate} – {edu.endDate||'Completed'}</span></div></div><div className="cf-item-actions"><button onClick={()=>editEducation(i)} className="cf-edit-btn" aria-label={`Edit ${edu.degree}`}><FiEdit2/></button><button onClick={()=>deleteEducation(i)} className="cf-delete-btn" aria-label={`Delete ${edu.degree}`}><FiTrash2/></button></div></div>))}{formData.education.length===0&&<p className="cf-empty-msg">No education added yet</p>}</div>
                  </div>
                </div>
              )}
              {activeSection==='skills'&&(
                <div className="cf-form-section-content">
                  <h3 className="cf-form-section-title"><FiTool/> Trade Skills & Certifications</h3>
                  <div className="cf-form-card">
                    <h4 className="cf-sub-section-title">Trade Skills & Expertise</h4><p className="cf-section-desc">List your construction skills and areas of expertise</p>
                    <div className="cf-skills-input">
                      <div className="cf-form-group"><label className="cf-form-label">Skill Name*<input value={currentSkill.name} onChange={e=>setCurrentSkill({...currentSkill,name:e.target.value})} placeholder="Blueprint Reading, Concrete Forming" className="cf-form-input"/></label><label className="cf-form-label">Category<input value={currentSkill.category} onChange={e=>setCurrentSkill({...currentSkill,category:e.target.value})} placeholder="Carpentry, Electrical, Plumbing" className="cf-form-input"/></label></div>
                      <label className="cf-form-label">Years of Experience<input value={currentSkill.yearsExperience} onChange={e=>setCurrentSkill({...currentSkill,yearsExperience:e.target.value})} placeholder="5+ years" className="cf-form-input"/></label>
                    ...
                    <div className="cf-items-list">{formData.skills.map((s,i)=>(<div key={i} className="cf-list-item"><div><strong>{s.name}</strong>{s.category&&` (${s.category})`}{s.yearsExperience&&` – ${s.yearsExperience} years`}</div><div className="cf-item-actions"><button onClick={()=>editSkill(i)} className="cf-edit-btn"><FiEdit2/></button><button onClick={()=>deleteSkill(i)} className="cf-delete-btn"><FiTrash2/></button></div></div>))}{formData.skills.length===0&&<p className="cf-empty-msg">No skills added yet</p>}</div>
                  </div>
                  <div className="cf-form-card">
                    <h4 className="cf-sub-section-title"><FiShield/> Safety Training & Qualifications</h4><p className="cf-section-desc">Add your construction safety certifications and training</p>
                    <div className="cf-skills-input">
                      <div className="cf-form-group"><label className="cf-form-label">Safety Certification*<input value={currentSafety.name} onChange={e=>setCurrentSafety({...currentSafety,name:e.target.value})} placeholder="OSHA 30-Hour, First Aid/CPR" className="cf-form-input"/></label><label className="cf-form-label">Level / Type<input value={currentSafety.level} onChange={e=>setCurrentSafety({...currentSafety,level:e.target.value})} placeholder="Certified, Trained, Licensed" className="cf-form-input"/></label></div>
                      <div className="cf-form-actions"><button onClick={addSafety} className="cf-add-btn" disabled={!currentSafety.name.trim()}><FiPlus/> {currentSafety.isEditing?'Update Safety Training':'Add Safety Training'}</button>{currentSafety.isEditing&&<button onClick={()=>setCurrentSafety(defaultSafety())} className="cf-cancel-btn"><FiX/> Cancel</button>}</div>
                    </div>
                    <div className="cf-items-list">{formData.safetyTraining.map((s,i)=>(<div key={i} className="cf-list-item"><div><strong>{s.name}</strong>{s.level&&` – ${s.level}`}</div><div className="cf-item-actions"><button onClick={()=>editSafety(i)} className="cf-edit-btn"><FiEdit2/></button><button onClick={()=>deleteSafety(i)} className="cf-delete-btn"><FiTrash2/></button></div></div>))}{formData.safetyTraining.length===0&&<p className="cf-empty-msg">No safety training added yet</p>}</div>
                  </div>
                  <div className="cf-form-card">
                    <h4 className="cf-sub-section-title"><FiTruck/> Equipment Operation</h4><p className="cf-section-desc">List construction equipment you can operate</p>
                    <div className="cf-skills-input">
                      <div className="cf-form-group"><label className="cf-form-label">Equipment Name*<input value={currentEquipment.name} onChange={e=>setCurrentEquipment({...currentEquipment,name:e.target.value})} placeholder="Excavator, Forklift, Bulldozer" className="cf-form-input"/></label><label className="cf-form-label">Years of Experience<input value={currentEquipment.yearsExperience} onChange={e=>setCurrentEquipment({...currentEquipment,yearsExperience:e.target.value})} placeholder="3+ years" className="cf-form-input"/></label></div>
                      <div className="cf-form-actions"><button onClick={addEquipment} className="cf-add-btn" disabled={!currentEquipment.name.trim()}><FiPlus/> {currentEquipment.isEditing?'Update Equipment':'Add Equipment'}</button>{currentEquipment.isEditing&&<button onClick={()=>setCurrentEquipment(defaultEquipment())} className="cf-cancel-btn"><FiX/> Cancel</button>}</div>
                    </div>
                    <div className="cf-items-list">{formData.equipment.map((e,i)=>(<div key={i} className="cf-list-item"><div><strong>{e.name}</strong>{e.yearsExperience&&` – ${e.yearsExperience} years`}</div><div className="cf-item-actions"><button onClick={()=>editEquipment(i)} className="cf-edit-btn"><FiEdit2/></button><button onClick={()=>deleteEquipment(i)} className="cf-delete-btn"><FiTrash2/></button></div></div>))}{formData.equipment.length===0&&<p className="cf-empty-msg">No equipment added yet</p>}</div>
                  </div>
                  <div className="cf-form-card">
                    <h4 className="cf-sub-section-title">Languages</h4><p className="cf-section-desc">List languages you speak (important for international work)</p>
                    <div className="cf-skills-input">
                      <div className="cf-form-group"><label className="cf-form-label">Language*<input value={currentLanguage.name} onChange={e=>setCurrentLanguage({...currentLanguage,name:e.target.value})} placeholder="Spanish, French, Arabic" className="cf-form-input"/></label><label className="cf-form-label">Proficiency Level<input value={currentLanguage.proficiency} onChange={e=>setCurrentLanguage({...currentLanguage,proficiency:e.target.value})} placeholder="Fluent, Conversational, Basic" className="cf-form-input"/></label></div>
              ...
              {activeSection==='certifications'&&(
                <div className="cf-form-section-content">
                  <h3 className="cf-form-section-title"><FiShield/> Certifications & Licenses</h3>
                  <div className="cf-form-card">
                    <h4 className="cf-sub-section-title">Professional Certifications & Licenses</h4><p className="cf-section-desc">Add your professional construction licenses and trade certifications</p>
                    <div className="cf-skills-input">
                      <div className="cf-form-group"><label className="cf-form-label">License/Certification Name*<input value={currentCertification.name} onChange={e=>setCurrentCertification({...currentCertification,name:e.target.value})} placeholder="Journeyman Electrician License, OSHA 500" className="cf-form-input"/></label><label className="cf-form-label">Issuing Authority<input value={currentCertification.issuingAuthority} onChange={e=>setCurrentCertification({...currentCertification,issuingAuthority:e.target.value})} placeholder="State Licensing Board, OSHA" className="cf-form-input"/></label></div>
                      <div className="cf-form-group"><label className="cf-form-label">License Number<input value={currentCertification.licenseNumber} onChange={e=>setCurrentCertification({...currentCertification,licenseNumber:e.target.value})} placeholder="EL1234567" className="cf-form-input"/></label><label className="cf-form-label">Expiry Date<input value={currentCertification.expiryDate} onChange={e=>setCurrentCertification({...currentCertification,expiryDate:e.target.value})} placeholder="Month Year" className="cf-form-input"/></label></div>
                ...
                <div className="cf-form-section-content">
                  ...
                  <div className="cf-form-card">
                    <div className="cf-font-grid">
                      {[{key:'name',label:'Name'},{key:'sectionTitle',label:'Section Titles'},{key:'jobTitle',label:'Job Titles'},{key:'degree',label:'Degrees/Certifications'},{key:'institution',label:'Company/Institution Names'},{key:'institutionDate',label:'Institution Dates'},{key:'regularText',label:'Regular Text'},{key:'bulletText',label:'Bullet Points'},{key:'contactInfo',label:'Contact Info'},{key:'skillText',label:'Skills Text'},{key:'licenseText',label:'Certification Text'}].map(({key,label})=>(
        ...
        <section className="cf-faq-section" aria-labelledby="faq-title">
          <div className="cf-container">
            <div className="cf-section-header">
              ...
              <p className="cf-section-subtitle">Everything you need to know about creating professional construction resumes with our tool.</p>
            </div>
        ...
        <section className="cf-cta-section" aria-labelledby="cta-title">
          <div className="cf-container">
            <div className="cf-cta-content">
              <h2 className="cf-cta-title" id="cta-title">Ready to Build Your Construction Career?</h2>
              <p className="cf-cta-subtitle">Join 3 million+ construction professionals who landed their dream jobs with our free ATS-friendly construction resume builder.</p>
              <div className="cf-cta-btn-wrap">
                <button onClick={()=>setActiveSection('personal')} className="cf-cta-btn" aria-label="Create your free construction resume now—no sign-up required">
                  <span className="cf-cta-btn-text">Create Your Free Construction Resume Now</span>
                  <FiArrowRight className="cf-cta-btn-icon"/>
                </button>
              </div>
              <div className="cf-cta-guarantee"><FiCheck className="cf-guarantee-icon"/><span className="cf-guarantee-text">No credit card required • Free forever • Download in minutes • ATS Optimized for Construction • Works Worldwide</span></div>
            </div>
          </div>
        </section>

        {showFullPreview&&(<div className="cf-modal" onClick={()=>setShowFullPreview(false)}><div className="cf-modal-content" onClick={e=>e.stopPropagation()}><div className="cf-modal-header"><h3>Full Construction Resume Preview</h3><button className="cf-close-btn" onClick={()=>setShowFullPreview(false)}><FiX/></button></div><div className="cf-modal-pages"><div className="cf-modal-page"><ConstructionTemplate formData={formData}/></div></div></div></div>)}
...
export default ConstructionResume;