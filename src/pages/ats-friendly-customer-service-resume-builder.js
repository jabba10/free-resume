...
const careerFlowStyles = `
  ...
  * { box-sizing: border-box; }
  ...
  .cf-cs-template { color: #000000; line-height: 1.25; height: 100%; }
  ...
  .cf-modal-page .cf-cs-template { width: 100%; height: 100%; padding: 10mm 15mm; box-sizing: border-box; overflow-y: auto; }
  ...
  @media (max-width: 768px) {
    ...
    .cf-modal-page .cf-cs-template { padding: 8mm 12mm; }
  ...
  @media (max-width: 480px) {
    ...
    .cf-modal-page .cf-cs-template { padding: 6mm 8mm; }
  ...
  @media print {
    ...
    .cf-cs-template, .cf-section { page-break-inside: avoid; }
  }
`;

const CustomerServiceResume = ({ seoData, buildTimestamp }) => {
  const {
    currentDate,
    lastModifiedDate,
    reviewDates,
  ...
  const [formData, setFormData] = useState({ fullName: '', email: '', phone: '', address: '', summary: '', experience: [], education: [], skills: [], certifications: [], achievements: [], languages: [], socialLinks: [] });
  ...
  const faqs = [
    { question: "Is this customer service resume builder really free with no hidden costs?", answer: "Yes, our customer service resume builder is completely free with no hidden costs or watermarks. Create, edit, and download your professional customer service resume in PDF format without any payment required." },
    { question: "What does ATS-friendly mean for customer service resumes?", answer: "ATS-friendly means our customer service resume templates are optimized to pass through Applicant Tracking Systems used by 95% of companies worldwide. This ensures your customer service experience, metrics, and skills are properly scanned and recognized." },
    { question: "Can I download my customer service resume as PDF without creating an account?", answer: "Absolutely! Download your professional customer service resume in PDF format without creating an account. Everything is completely free and accessible immediately for customer service professionals worldwide." },
    { question: "How many customer service resume templates are available for free?", answer: "We offer professionally designed ATS-friendly customer service resume templates for call center agents, customer success managers, technical support, customer experience specialists, and all customer service roles. All templates are completely free and optimized for global hiring." },
    { question: "How does your customer service resume builder work?", answer: "Our builder uses ATS-optimized customer service templates with proper industry terminology formatting. We guide you to highlight customer service metrics, multilingual abilities, and technical skills that global employers look for." },
    { question: "Can I edit my customer service resume after downloading it?", answer: "Yes, you can always come back and edit your customer service resume. Your work saves automatically, and you can download updated versions as many times as needed—completely free." }
  ];
  ...
  const hasContent = () => formData.fullName || formData.email || formData.summary || formData.socialLinks.length > 0 || formData.experience.length > 0 || formData.education.length > 0 || formData.skills.length > 0 || formData.certifications.length > 0 || formData.achievements.length > 0 || formData.languages.length > 0;
  ...
  const CSTemplate = ({ formData }) => {
    ...
    return (
      <div className="cf-cs-template">
        ...
        {hasSkills && <section className="cf-section"><h2 className="cf-section-title" style={{ fontSize: `${fontSizes.sectionTitle}pt`, fontWeight: 700 }}>CORE COMPETENCIES</h2><ul className="cf-skills-list">{formData.skills.map((s, i) => <li key={i} style={{ fontSize: `${fontSizes.skillText}pt`, fontWeight: 400 }}>{s.name}{s.category && ` (${s.category})`}</li>)}</ul></section>}
      ...
      <div className="cf-resume-builder" lang="en-US">
        ...
        <nav className="cf-breadcrumb" aria-label="Breadcrumb">
          <ol>
            ...
            <li><Link href="/ats-friendly-customer-service-resume-builder" className="cf-breadcrumb-link" prefetch={false}><span className="cf-breadcrumb-text">Free Global Templates</span></Link></li>
          </ol>
        </nav>
        ...
        <section className="cf-hero">
          <div className="cf-container">
            <div className="cf-hero-content">
              <div className="cf-trust-badge"><FiStar className="cf-trust-icon"/><span className="cf-trust-text">Best Free Customer Service Resume Builder 2026</span></div>
              <h1 className="cf-hero-title">Free Customer Service Resume <span className="cf-gradient-text">Builder 2026: Start Now</span></h1>
              <p className="cf-hero-subtitle">Create a <strong className="cf-hero-highlight">professional, ATS-optimized customer service resume for free in minutes.</strong> Our global customer service resume builder ensures your experience and metrics get noticed by employers worldwide.</p>
              <div className="cf-cta-buttons">
                <button onClick={() => setActiveSection('personal')} className="cf-btn-primary" aria-label="Start building your free customer service resume now—no sign-up required">
                  <span className="cf-btn-text">Start Building Your Customer Service Resume Now</span>
                  <FiArrowRight className="cf-btn-icon" />
                  <div className="cf-btn-pulse"></div>
                </button>
                <button onClick={generatePDF} className="cf-btn-secondary" aria-label="Download customer service resume as PDF" disabled={isGeneratingPDF || !hasContent()}><FiDownload className="cf-btn-icon"/><span className="cf-btn-text">Download Customer Service Resume PDF</span></button>
              </div>
              <div className="cf-hero-stats">
                <div className="cf-stat-item"><span className="cf-stat-number">5M+</span><span className="cf-stat-label">CS Professionals</span></div>
              ...
              <div className="cf-industry-badges">
                <div className="cf-badge-grid">
                  <span className="cf-badge-item"><FiPhoneCall/> Call Center Agent</span>
                  <span className="cf-badge-item"><FiHeadphones/> Technical Support</span>
                  <span className="cf-badge-item"><FiUsers/> Customer Success Manager</span>
                  <span className="cf-badge-item"><FiMessageSquare/> Chat Support</span>
                  <span className="cf-badge-item"><FiGlobe/> Multilingual Support</span>
                  <span className="cf-badge-item"><FiThumbsUp/> Customer Experience</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="cf-layout">
          <div className="cf-preview-section">
            ...
            <div className={`cf-preview-container ${showFullPreview ? 'cf-full-preview' : ''}`}><div className="cf-resume-card"><div className="cf-preview-content"><div className="cf-resume-preview" ref={resumeRef}><CSTemplate formData={formData}/></div></div></div></div>
          </div>
          <div className="cf-form-section">
            <div className="cf-form-nav">
              {[{id:'personal',label:'Personal',icon:<FiUser/>},{id:'experience',label:'Experience',icon:<FiHeadphones/>},{id:'education',label:'Education',icon:<FiBook/>},{id:'skills',label:'Skills & Lang.',icon:<FiGlobe/>},{id:'achievements',label:'Achievements',icon:<FiAward/>},{id:'settings',label:'Font Settings',icon:<FiSettings/>}].map(item => (<button key={item.id} className={`cf-nav-btn ${activeSection===item.id?'cf-nav-btn-active':''}`} onClick={()=>setActiveSection(item.id)}>{item.icon}<span>{item.label}</span></button>))}
            </div>
            <div className="cf-form-content">
              {activeSection==='personal'&&(
                <div className="cf-form-section-content">
                  ...
                  <div className="cf-form-card">
                    <div className="cf-form-group"><label className="cf-form-label">Full Name*<input type="text" name="fullName" value={formData.fullName} onChange={handleInputChange} placeholder="John Smith, Customer Service Professional" required className="cf-form-input"/></label><label className="cf-form-label">Email*<input type="email" name="email" value={formData.email} onChange={handleInputChange} placeholder="john.smith@example.com" required className="cf-form-input"/></label></div>
                    <div className="cf-form-group"><label className="cf-form-label">Phone (International Format)<input type="tel" name="phone" value={formData.phone} onChange={handleInputChange} placeholder="+1 (555) 123-4567" className="cf-form-input"/></label><label className="cf-form-label">Location (City, Country)<input type="text" name="address" value={formData.address} onChange={handleInputChange} placeholder="New York, USA | London, UK | Sydney, Australia" className="cf-form-input"/></label></div>
                  </div>
                  <div className="cf-form-card"><label className="cf-form-label">Professional Customer Service Summary*<textarea name="summary" value={formData.summary} onChange={handleInputChange} placeholder="Dedicated Customer Service Professional with 5+ years of experience in global customer support and call center operations. Proven track record of achieving 95%+ customer satisfaction scores across multiple channels (phone, email, chat). Multilingual support in English and Spanish. Expertise in CRM systems (Salesforce, Zendesk) and technical troubleshooting. Committed to delivering exceptional customer experiences and driving customer loyalty." required className="cf-form-textarea" rows="6"/><div className="cf-char-count">{formData.summary.length}/500 characters</div></label></div>
                  <div className="cf-form-card">
                    ...
                    <div className="cf-social-input">
                      <select value={currentSocialLink.platform} onChange={e=>setCurrentSocialLink({...currentSocialLink,platform:e.target.value})} className="cf-form-select"><option value="">Select Platform</option><option value="LinkedIn">LinkedIn</option><option value="GitHub">GitHub (Technical Support)</option><option value="Portfolio">Professional Portfolio</option><option value="Website">Personal Website</option><option value="Twitter">Twitter (Customer Service)</option></select>
              ...
              {activeSection==='experience'&&(
                <div className="cf-form-section-content">
                  <h3 className="cf-form-section-title"><FiHeadphones/> Customer Service Experience</h3>
                  <p className="cf-section-desc">List your customer service positions in reverse chronological order (most recent first)</p>
                  <div className="cf-form-card">
                    <div className="cf-form-group"><label className="cf-form-label">Position Title*<input value={currentExperience.position} onChange={e=>setCurrentExperience({...currentExperience,position:e.target.value})} placeholder="Customer Service Representative, Technical Support Specialist" required className="cf-form-input"/></label><label className="cf-form-label">Company/Organization*<input value={currentExperience.employer} onChange={e=>setCurrentExperience({...currentExperience,employer:e.target.value})} placeholder="Global Tech Solutions Inc." required className="cf-form-input"/></label></div>
                    <label className="cf-form-label">Department / Team / Specialization<input value={currentExperience.department} onChange={e=>setCurrentExperience({...currentExperience,department:e.target.value})} placeholder="Enterprise Customer Support, Bilingual Support Team" className="cf-form-input"/></label>
                    ...
                    <label className="cf-form-label">Key Responsibilities & Customer Service Metrics*<textarea value={currentExperience.description} onChange={e=>setCurrentExperience({...currentExperience,description:e.target.value})} placeholder="• Managed 50+ daily customer inquiries via phone, email, and live chat with 95%+ satisfaction rate
...
• Maintained 98% accuracy in documentation and case resolution" required className="cf-form-textarea" rows="8"/><div className="cf-char-count">{currentExperience.description.length}/2000 characters</div></label>
              ...
              {activeSection==='education'&&(
                <div className="cf-form-section-content">
                  ...
                  <div className="cf-form-card">
                    ...
                    <label className="cf-form-label">Program / Specialization / Concentration<input value={currentEducation.program} onChange={e=>setCurrentEducation({...currentEducation,program:e.target.value})} placeholder="Customer Relationship Management, Communications" className="cf-form-input"/></label>
              ...
              {activeSection==='skills'&&(
                <div className="cf-form-section-content">
                  <h3 className="cf-form-section-title"><FiGlobe/> Core Competencies & Languages</h3>
                  <div className="cf-form-card">
                    <h4 className="cf-sub-section-title">Core Customer Service Skills</h4><p className="cf-section-desc">List your customer service skills and categorize them</p>
                    <div className="cf-skills-input">
                      <div className="cf-form-group"><label className="cf-form-label">Skill Name*<input value={currentSkill.name} onChange={e=>setCurrentSkill({...currentSkill,name:e.target.value})} placeholder="Conflict Resolution, CRM Software, Technical Troubleshooting" className="cf-form-input"/></label><label className="cf-form-label">Category<select value={currentSkill.category} onChange={e=>setCurrentSkill({...currentSkill,category:e.target.value})} className="cf-form-select"><option value="">Select Category</option><option value="Communication">Communication</option><option value="Technical">Technical Skills</option><option value="Software">Software/CRM</option><option value="Problem Solving">Problem Solving</option><option value="Multilingual">Multilingual</option><option value="Metrics">Performance Metrics</option></select></label></div>
                    ...
                    <div className="cf-items-list">{formData.skills.map((s,i)=>(<div key={i} className="cf-list-item"><div><strong>{s.name}</strong>{s.category&&<span className="cf-skill-cat"> – {s.category}</span>}</div><div className="cf-item-actions"><button onClick={()=>editSkill(i)} className="cf-edit-btn"><FiEdit2/></button><button onClick={()=>deleteSkill(i)} className="cf-delete-btn"><FiTrash2/></button></div></div>))}{formData.skills.length===0&&<p className="cf-empty-msg">No skills added yet</p>}</div>
                  </div>
                  <div className="cf-form-card">
                    <h4 className="cf-sub-section-title"><FiShield/> Professional Certifications</h4><p className="cf-section-desc">Add your customer service and support certifications</p>
                    <div className="cf-skills-input">
                      <div className="cf-form-group"><label className="cf-form-label">Certification Name*<input value={currentCertification.name} onChange={e=>setCurrentCertification({...currentCertification,name:e.target.value})} placeholder="Customer Service Professional (CSP) Certification" className="cf-form-input"/></label><label className="cf-form-label">Issuing Organization<input value={currentCertification.issuingAuthority} onChange={e=>setCurrentCertification({...currentCertification,issuingAuthority:e.target.value})} placeholder="International Customer Service Association" className="cf-form-input"/></label></div>
                      <div className="cf-form-group"><label className="cf-form-label">Certification Number<input value={currentCertification.certificationNumber} onChange={e=>setCurrentCertification({...currentCertification,certificationNumber:e.target.value})} placeholder="CSP1234567" className="cf-form-input"/></label><label className="cf-form-label">Expiry Date<input value={currentCertification.expiryDate} onChange={e=>setCurrentCertification({...currentCertification,expiryDate:e.target.value})} placeholder="Month Year" className="cf-form-input"/></label></div>
                  ...
                  <div className="cf-form-card">
                    <h4 className="cf-sub-section-title"><FiGlobe/> Languages</h4><p className="cf-section-desc">List languages you speak and your proficiency level (crucial for global customer service roles)</p>
                    <div className="cf-skills-input">
                      <div className="cf-form-group"><label className="cf-form-label">Language*<input value={currentLanguage.name} onChange={e=>setCurrentLanguage({...currentLanguage,name:e.target.value})} placeholder="English, Spanish, French, German, Mandarin" className="cf-form-input"/></label><label className="cf-form-label">Proficiency Level<select value={currentLanguage.proficiency} onChange={e=>setCurrentLanguage({...currentLanguage,proficiency:e.target.value})} className="cf-form-select"><option value="">Select Proficiency</option><option value="Native">Native</option><option value="Fluent">Fluent</option><option value="Professional">Professional</option><option value="Intermediate">Intermediate</option><option value="Basic">Basic</option></select></label></div>
                    ...
                    <div className="cf-items-list">{formData.languages.map((l,i)=>(<div key={i} className="cf-list-item"><div><strong>{l.name}</strong>{l.proficiency&&` (${l.proficiency})`}</div><div className="cf-item-actions"><button onClick={()=>editLanguage(i)} className="cf-edit-btn"><FiEdit2/></button><button onClick={()=>deleteLanguage(i)} className="cf-delete-btn"><FiTrash2/></button></div></div>))}{formData.languages.length===0&&<p className="cf-empty-msg">No languages added yet</p>}</div>
                  </div>
                </div>
              )}
              {activeSection==='achievements'&&(
                <div className="cf-form-section-content">
                  <h3 className="cf-form-section-title"><FiAward/> Key Achievements</h3>
                  <p className="cf-section-desc">Highlight your customer service achievements with measurable results</p>
                  <div className="cf-form-card">
                    <label className="cf-form-label">Achievement Title*<input value={currentAchievement.title} onChange={e=>setCurrentAchievement({...currentAchievement,title:e.target.value})} placeholder="Reduced Customer Complaint Resolution Time" required className="cf-form-input"/></label>
                    <label className="cf-form-label">Achievement Description (Include metrics)*<textarea value={currentAchievement.description} onChange={e=>setCurrentAchievement({...currentAchievement,description:e.target.value})} placeholder="Decreased average complaint resolution time from 48 hours to 24 hours, resulting in 30% improvement in customer satisfaction scores and reducing escalations by 45%." required className="cf-form-textarea" rows="4"/><div className="cf-char-count">{currentAchievement.description.length}/500 characters</div></label>
                  ...
                  <div className="cf-form-card"><h4 className="cf-sub-section-title">Your Achievements</h4>{formData.achievements.length===0?<p className="cf-empty-msg">No achievements added yet</p>:<div className="cf-items-list">{formData.achievements.map((a,i)=>(<div key={i} className="cf-list-item"><div className="cf-item-content"><div className="cf-item-header"><strong className="cf-item-title">{a.title}</strong></div><div className="cf-item-desc">{a.description}</div></div><div className="cf-item-actions"><button onClick={()=>editAchievement(i)} className="cf-edit-btn" aria-label={`Edit ${a.title}`}><FiEdit2/></button><button onClick={()=>deleteAchievement(i)} className="cf-delete-btn" aria-label={`Delete ${a.title}`}><FiTrash2/></button></div></div>))}</div>}</div>
                </div>
              )}
        ...
        <section className="cf-faq-section" aria-labelledby="faq-title">
          <div className="cf-container">
            <div className="cf-section-header">
              ...
              <p className="cf-section-subtitle">Everything you need to know about creating professional customer service resumes with our global builder.</p>
            </div>
        ...
        <section className="cf-cta-section" aria-labelledby="cta-title">
          <div className="cf-container">
            <div className="cf-cta-content">
              <h2 className="cf-cta-title" id="cta-title">Ready to Advance Your Customer Service Career?</h2>
              <p className="cf-cta-subtitle">Join 5 million+ customer service professionals worldwide who landed their dream jobs with our free ATS-friendly customer service resume builder.</p>
              <div className="cf-cta-btn-wrap">
                <button onClick={()=>setActiveSection('personal')} className="cf-cta-btn" aria-label="Create your free customer service resume now—no sign-up required">
                  <span className="cf-cta-btn-text">Create Your Free Customer Service Resume Now</span>
                  <FiArrowRight className="cf-cta-btn-icon"/>
                </button>
              </div>
              <div className="cf-cta-guarantee"><FiCheck className="cf-guarantee-icon"/><span className="cf-guarantee-text">No credit card required • Free forever • Download in minutes • ATS Optimized for Global Customer Service Roles</span></div>
            </div>
          </div>
        </section>

        {showFullPreview&&(<div className="cf-modal" onClick={()=>setShowFullPreview(false)}><div className="cf-modal-content" onClick={e=>e.stopPropagation()}><div className="cf-modal-header"><h3>Full Customer Service Resume Preview</h3><button className="cf-close-btn" onClick={()=>setShowFullPreview(false)}><FiX/></button></div><div className="cf-modal-pages"><div className="cf-modal-page"><CSTemplate formData={formData}/></div></div></div></div>)}
