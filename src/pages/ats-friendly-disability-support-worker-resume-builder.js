...
const careerFlowStyles = `
  ...
  * { box-sizing: border-box; }
  ...
  .cf-ndis-badge { text-align: center; margin-top: 2pt; color: #495057; font-weight: 500; }
  ...
  .cf-organization { color: #495057; margin: 0 0 3pt; font-style: italic; }
  ...
  .cf-skills-grid { display: grid; grid-template-columns: 1fr; gap: 4pt; }
  .cf-skill-category h4 { font-weight: 700; margin: 0 0 2pt; color: #000000; }
  .cf-cert-item, .cf-availability-item { margin-bottom: 3pt; line-height: 1.25; }
...
const DisabilitySupportResume = ({ seoData, buildTimestamp }) => {
  ...
  const defaultEducation = () => ({ institution: '', qualification: '', certNumber: '', startDate: '', endDate: '', isEditing: false, editIndex: null });
  const defaultCertification = () => ({ name: '', issuingBody: '', certNumber: '', expiryDate: '', isEditing: false, editIndex: null });
  const defaultSkill = () => ({ name: '', category: '', isEditing: false, editIndex: null });
  const defaultAchievement = () => ({ description: '', isEditing: false, editIndex: null });
  const defaultAvailability = () => ({ days: '', hours: '', type: '', isEditing: false, editIndex: null });
  ...
  const [formData, setFormData] = useState({ fullName: '', email: '', phone: '', location: '', ndisWorkerNumber: '', summary: '', experience: [], education: [], certifications: [], skills: [], achievements: [], availability: [], languages: [], socialLinks: [] });
  const [fontSizes, setFontSizes] = useState({ name: 14, sectionTitle: 10, contactInfo: 7, jobTitle: 9, organization: 7, qualification: 9, institution: 7, institutionDate: 6, regularText: 8, bulletText: 8, skillText: 7, certText: 8 });
  ...
  const faqs = [
    { question: "Is this disability support resume builder free for Australian job seekers?", answer: "Yes, our disability support resume builder is completely free with no hidden costs. Created specifically for Australian support workers, NDIS providers, and disability services employers." },
    { question: "What makes this builder specific to Australian Disability Support Workers?", answer: "Our templates include NDIS-specific sections, Australian qualification formats, NDIS Worker Screening Check, and industry-standard terminology used by Australian disability service providers." },
    { question: "Can I include my NDIS Worker Number and certifications?", answer: "Absolutely! We have dedicated sections for NDIS Worker Screening Check, First Aid certificates, manual handling, and other essential Australian disability support qualifications." },
    { question: "Are the templates ATS-friendly for Australian disability providers?", answer: "Yes, our templates are optimized for Australian ATS systems used by major disability service providers like Scope, Aruma, Life Without Barriers, and NDIS-registered organisations." },
    { question: "What Australian-specific sections does the builder include?", answer: "NDIS Worker Details, Australian Qualifications Framework (AQF) certifications, Disability Services Act compliance, Client Rights & Dignity of Risk principles, and Australian working rights." },
    { question: "Can I download my resume as PDF for Australian job applications?", answer: "Yes, download your professionally formatted disability support resume in PDF format - completely free, no watermarks, ready for Australian job applications." }
  ];

  const handleFontSizeChange = (key, value) => setFontSizes(prev => ({ ...prev, [key]: Math.max(4, Math.min(24, parseInt(value) || prev[key])) }));
  const resetFontSizes = () => setFontSizes({ name: 14, sectionTitle: 10, contactInfo: 7, jobTitle: 9, organization: 7, qualification: 9, institution: 7, institutionDate: 6, regularText: 8, bulletText: 8, skillText: 7, certText: 8 });
  const getSocialIcon = () => <FiGlobe />;
  const formatSocialUrl = (url) => url ? url.replace(/(https?:\/\/)?(www\.)?/, '').replace(/\/$/, '') : '';
  const handleInputChange = (e) => setFormData({ ...formData, [e.target.name]: e.target.value });
  const hasContent = () => formData.fullName || formData.email || formData.summary || formData.socialLinks.length > 0 || formData.experience.length > 0 || formData.education.length > 0 || formData.certifications.length > 0 || formData.skills.length > 0 || formData.achievements.length > 0 || formData.availability.length > 0 || formData.languages.length > 0;
    ...
    return (
      <div className="cf-universal-template">
        <header className="cf-resume-header">
          ...
          {formData.ndisWorkerNumber && <div className="cf-ndis-badge" style={{ fontSize: `${fontSizes.contactInfo}pt` }}>NDIS Worker Number: {formData.ndisWorkerNumber}</div>}
        ...
        {hasExperience && <section className="cf-section"><h2 className="cf-section-title" style={{ fontSize: `${fontSizes.sectionTitle}pt` }}>DISABILITY SUPPORT EXPERIENCE</h2>{formData.experience.map((exp, i) => (<div key={i} className="cf-experience-item"><div className="cf-experience-header"><h3 style={{ fontSize: `${fontSizes.jobTitle}pt` }}>{exp.position}</h3><p className="cf-organization" style={{ fontSize: `${fontSizes.organization}pt` }}>{exp.employer}{exp.clientType && ` • ${exp.clientType}`} | {exp.startDate} – {exp.endDate || 'Present'}</p></div><ul className="cf-bullet-list">{exp.description.split('\n').filter(l => l.trim()).map((line, j) => <li key={j} style={{ fontSize: `${fontSizes.bulletText}pt` }}>{line}</li>)}</ul></div>))}</section>}
        {hasEducation && <section className="cf-section"><h2 className="cf-section-title" style={{ fontSize: `${fontSizes.sectionTitle}pt` }}>QUALIFICATIONS & TRAINING</h2>{formData.education.map((edu, i) => (<div key={i} className="cf-education-item"><h3 style={{ fontSize: `${fontSizes.qualification}pt` }}>{edu.qualification}{edu.certNumber && ` (${edu.certNumber})`}</h3><p className="cf-institution-text" style={{ fontSize: `${fontSizes.institution}pt` }}>{edu.institution} | <span className="cf-institution-date" style={{ fontSize: `${fontSizes.institutionDate}pt` }}>{edu.startDate} – {edu.endDate || 'Current'}</span></p></div>))}</section>}
        {hasCertifications && <section className="cf-section"><h2 className="cf-section-title" style={{ fontSize: `${fontSizes.sectionTitle}pt` }}>CERTIFICATIONS & LICENSES</h2>{formData.certifications.map((cert, i) => (<div key={i} className="cf-cert-item" style={{ fontSize: `${fontSizes.certText}pt` }}><strong style={{ fontWeight: 700 }}>{cert.name}</strong>{cert.issuingBody && ` – ${cert.issuingBody}`}{cert.certNumber && ` (#${cert.certNumber})`}{cert.expiryDate && ` – Expires: ${cert.expiryDate}`}</div>))}</section>}
        {hasSkills && <section className="cf-section"><h2 className="cf-section-title" style={{ fontSize: `${fontSizes.sectionTitle}pt` }}>SPECIALISED SKILLS</h2><div className="cf-skills-grid">{formData.skills.map((skill, i) => (<div key={i} className="cf-skill-category">{skill.category && <h4 style={{ fontSize: `${fontSizes.skillText}pt` }}>{skill.category}</h4>}<ul className="cf-bullet-list"><li style={{ fontSize: `${fontSizes.skillText}pt` }}>{skill.name}</li></ul></div>))}</div></section>}
        {hasAchievements && <section className="cf-section"><h2 className="cf-section-title" style={{ fontSize: `${fontSizes.sectionTitle}pt` }}>KEY ACHIEVEMENTS</h2><ul className="cf-bullet-list">{formData.achievements.map((ach, i) => (<li key={i} style={{ fontSize: `${fontSizes.bulletText}pt` }}>{ach.description}</li>))}</ul></section>}
        {hasAvailability && <section className="cf-section"><h2 className="cf-section-title" style={{ fontSize: `${fontSizes.sectionTitle}pt` }}>AVAILABILITY</h2>{formData.availability.map((avail, i) => (<div key={i} className="cf-availability-item" style={{ fontSize: `${fontSizes.regularText}pt` }}><strong style={{ fontWeight: 700 }}>{avail.days}</strong>{avail.hours && ` – ${avail.hours}`}{avail.type && ` (${avail.type})`}</div>))}</section>}
  ...
  return (
    <>
      ...
      <div className="cf-resume-builder" lang="en-AU">
        <Head>
        ...
        <nav className="cf-breadcrumb" aria-label="Breadcrumb">
          <ol>
            ...
            <li><Link href="/ats-friendly-disability-support-worker-resume-builder" className="cf-breadcrumb-link" prefetch={false}><span className="cf-breadcrumb-text">Free NDIS Resume Template</span></Link></li>
          </ol>
        </nav>
        ...
        <section className="cf-hero">
          <div className="cf-container">
            <div className="cf-hero-content">
              <div className="cf-trust-badge"><FiStar className="cf-trust-icon"/><span className="cf-trust-text">Best Free NDIS Resume Builder 2026</span></div>
              <h1 className="cf-hero-title">Free NDIS <span className="cf-gradient-text">Resume Builder 2026: Create Your Australian Support Worker CV & Get Hired</span></h1>
              <p className="cf-hero-subtitle">Create a <strong className="cf-hero-highlight">professional, NDIS-optimized disability support resume for free in minutes.</strong> Our Australian resume builder ensures your experience gets noticed by disability service providers and NDIS organisations.</p>
              <div className="cf-cta-buttons">
                <button onClick={() => setActiveSection('personal')} className="cf-btn-primary" aria-label="Start building your free disability support resume now—no sign-up required">
                  <span className="cf-btn-text">Start Building Your NDIS Resume Now</span>
                  <FiArrowRight className="cf-btn-icon" />
                  <div className="cf-btn-pulse"></div>
                </button>
                <button onClick={generatePDF} className="cf-btn-secondary" aria-label="Download disability support resume as PDF" disabled={isGeneratingPDF || !hasContent()}><FiDownload className="cf-btn-icon"/><span className="cf-btn-text">Download NDIS Resume PDF</span></button>
              </div>
              <div className="cf-hero-stats">
                <div className="cf-stat-item"><span className="cf-stat-number">150K+</span><span className="cf-stat-label">Support Workers</span></div>
              ...
              <div className="cf-industry-badges">
                <div className="cf-badge-grid">
                  <span className="cf-badge-item"><FiCheckCircle/> NDIS Worker Screening</span>
                  <span className="cf-badge-item"><FiCheckCircle/> First Aid & CPR</span>
                  <span className="cf-badge-item"><FiCheckCircle/> Manual Handling</span>
                  <span className="cf-badge-item"><FiCheckCircle/> Cert III/IV Disability</span>
                  <span className="cf-badge-item"><FiCheckCircle/> Mental Health First Aid</span>
                  <span className="cf-badge-item"><FiCheckCircle/> Medication Administration</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="cf-layout">
          <div className="cf-preview-section">
            ...
            <div className={`cf-preview-container ${showFullPreview ? 'cf-full-preview' : ''}`}><div className="cf-resume-card"><div className="cf-preview-content"><div className="cf-resume-preview" ref={resumeRef}><DisabilitySupportTemplate formData={formData}/></div></div></div></div>
          </div>

          <div className="cf-form-section">
            <div className="cf-form-nav">
              {[{id:'personal',label:'Personal',icon:<FiUser/>},{id:'experience',label:'Support Exp.',icon:<FiUsers/>},{id:'education',label:'Qualifications',icon:<FiBook/>},{id:'certifications',label:'Certifications',icon:<FiShield/>},{id:'skills',label:'Skills',icon:<FiActivity/>},{id:'settings',label:'Font Settings',icon:<FiSettings/>}].map(item => (<button key={item.id} className={`cf-nav-btn ${activeSection===item.id?'cf-nav-btn-active':''}`} onClick={()=>setActiveSection(item.id)}>{item.icon}<span>{item.label}</span></button>))}
            </div>
            ...
            <div className="cf-form-content">
              {activeSection==='personal'&&(
                <div className="cf-form-section-content">
                  ...
                  <div className="cf-form-card">
                    <div className="cf-form-group"><label className="cf-form-label">Full Name*<input type="text" name="fullName" value={formData.fullName} onChange={handleInputChange} placeholder="Sarah Johnson, Disability Support Worker" required className="cf-form-input"/></label><label className="cf-form-label">Email*<input type="email" name="email" value={formData.email} onChange={handleInputChange} placeholder="sarah.johnson@email.com" required className="cf-form-input"/></label></div>
                    <div className="cf-form-group"><label className="cf-form-label">Phone*<input type="tel" name="phone" value={formData.phone} onChange={handleInputChange} placeholder="04XX XXX XXX" required className="cf-form-input"/></label><label className="cf-form-label">Location (Australia)*<input type="text" name="location" value={formData.location} onChange={handleInputChange} placeholder="Melbourne, VIC" required className="cf-form-input"/></label></div>
                    <label className="cf-form-label">NDIS Worker Screening Number<input type="text" name="ndisWorkerNumber" value={formData.ndisWorkerNumber} onChange={handleInputChange} placeholder="NDISXXXXXXXX" className="cf-form-input"/></label>
                  </div>
                  <div className="cf-form-card"><label className="cf-form-label">Professional Disability Support Profile*<textarea name="summary" value={formData.summary} onChange={handleInputChange} placeholder="Compassionate and dedicated Disability Support Worker with 5+ years of experience supporting individuals with diverse abilities. Skilled in implementing NDIS plans, providing personal care, and promoting independence. Strong understanding of Disability Services Act, person-centered approaches, and dignity of risk principles. Committed to enhancing quality of life for participants through respectful, individualised support." required className="cf-form-textarea" rows="6"/><div className="cf-char-count">{formData.summary.length}/500 characters</div></label></div>
                  <div className="cf-form-card">
                    <h4 className="cf-sub-section-title"><FiGlobe/> Professional Links</h4><p className="cf-section-desc">Add your professional profiles (LinkedIn, SEEK, etc.)</p>
                    <div className="cf-social-input">
                      <select value={currentSocialLink.platform} onChange={e=>setCurrentSocialLink({...currentSocialLink,platform:e.target.value})} className="cf-form-select"><option value="">Select Platform</option><option value="LinkedIn">LinkedIn</option><option value="SEEK">SEEK Profile</option><option value="Indeed">Indeed Profile</option><option value="Portfolio">Professional Portfolio</option></select>
              ...
              {activeSection==='experience'&&(
                <div className="cf-form-section-content">
                  <h3 className="cf-form-section-title"><FiUsers/> Disability Support Experience</h3><p className="cf-section-desc">List your disability support positions in reverse chronological order (most recent first)</p>
                  <div className="cf-form-card">
                    <div className="cf-form-group"><label className="cf-form-label">Position Title*<input value={currentExperience.position} onChange={e=>setCurrentExperience({...currentExperience,position:e.target.value})} placeholder="Disability Support Worker, Senior Support Worker" required className="cf-form-input"/></label><label className="cf-form-label">Organisation/Provider*<input value={currentExperience.employer} onChange={e=>setCurrentExperience({...currentExperience,employer:e.target.value})} placeholder="National Disability Services, Scope Australia" required className="cf-form-input"/></label></div>
                    <label className="cf-form-label">Client Type / Support Area<input value={currentExperience.clientType} onChange={e=>setCurrentExperience({...currentExperience,clientType:e.target.value})} placeholder="Complex Needs, SIL, Mental Health, Aged Care with Disability" className="cf-form-input"/></label>
                    ...
                    <label className="cf-form-label">Key Responsibilities & Achievements*<textarea value={currentExperience.description} onChange={e=>setCurrentExperience({...currentExperience,description:e.target.value})} placeholder="• Provided person-centred support to 4 participants with complex needs in SIL accommodation
...
• Collaborated with multidisciplinary team including OT's and support coordinators" required className="cf-form-textarea" rows="8"/><div className="cf-char-count">{currentExperience.description.length}/2000 characters</div></label>
                    <div className="cf-form-actions"><button onClick={addExperience} className="cf-add-btn" disabled={!currentExperience.position||!currentExperience.employer||!currentExperience.startDate}><FiPlus/> {currentExperience.isEditing?'Update Support Experience':'Add Support Experience'}</button>{currentExperience.isEditing&&<button onClick={()=>setCurrentExperience(defaultExperience())} className="cf-cancel-btn"><FiX/> Cancel</button>}</div>
                  </div>
                  <div className="cf-form-card"><h4 className="cf-sub-section-title">Your Disability Support Experience</h4>{formData.experience.length===0?<p className="cf-empty-msg">No support experience added yet</p>:<div className="cf-items-list">{formData.experience.map((exp,i)=>(<div key={i} className="cf-list-item"><div className="cf-item-content"><div className="cf-item-header"><strong className="cf-item-title">{exp.position}</strong><span className="cf-item-subtitle">at {exp.employer}</span></div><div className="cf-item-meta"><span>{exp.startDate} – {exp.endDate||'Present'}</span>{exp.clientType&&<span>{exp.clientType}</span>}</div><div className="cf-item-desc">{exp.description.split('\n').filter(l=>l.trim()).map((line,j)=><p key={j} className="cf-bullet-point">• {line}</p>)}</div></div><div className="cf-item-actions"><button onClick={()=>editExperience(i)} className="cf-edit-btn" aria-label={`Edit ${exp.position} experience`}><FiEdit2/></button><button onClick={()=>deleteExperience(i)} className="cf-delete-btn" aria-label={`Delete ${exp.position} experience`}><FiTrash2/></button></div></div>))}</div>}</div>
                </div>
              )}

              {activeSection==='education'&&(
                <div className="cf-form-section-content">
                  <h3 className="cf-form-section-title"><FiBook/> Australian Qualifications & Training</h3><p className="cf-section-desc">List your disability support qualifications, training, and Australian certifications</p>
                  <div className="cf-form-card">
                    <div className="cf-form-group"><label className="cf-form-label">Institution/RTO*<input value={currentEducation.institution} onChange={e=>setCurrentEducation({...currentEducation,institution:e.target.value})} placeholder="TAFE NSW, Open Colleges, Australian Red Cross" required className="cf-form-input"/></label><label className="cf-form-label">Qualification/Certificate*<input value={currentEducation.qualification} onChange={e=>setCurrentEducation({...currentEducation,qualification:e.target.value})} placeholder="Certificate III in Individual Support (Disability)" required className="cf-form-input"/></label></div>
                    <label className="cf-form-label">Certificate Number<input value={currentEducation.certNumber} onChange={e=>setCurrentEducation({...currentEducation,certNumber:e.target.value})} placeholder="NRT1234567" className="cf-form-input"/></label>
                    <div className="cf-form-group"><label className="cf-form-label">Start Date<input type="text" placeholder="Month Year" value={currentEducation.startDate} onChange={e=>setCurrentEducation({...currentEducation,startDate:e.target.value})} className="cf-form-input"/></label><label className="cf-form-label">End Date / Expected<input type="text" placeholder="Month Year or Current" value={currentEducation.endDate} onChange={e=>setCurrentEducation({...currentEducation,endDate:e.target.value})} className="cf-form-input"/></label></div>
                    <div className="cf-form-actions"><button onClick={addEducation} className="cf-add-btn" disabled={!currentEducation.institution||!currentEducation.qualification}><FiPlus/> {currentEducation.isEditing?'Update Qualification':'Add Qualification'}</button>{currentEducation.isEditing&&<button onClick={()=>setCurrentEducation(defaultEducation())} className="cf-cancel-btn"><FiX/> Cancel</button>}</div>
                  </div>
                  <div className="cf-form-card"><h4 className="cf-sub-section-title">Your Qualifications</h4>{formData.education.length===0?<p className="cf-empty-msg">No qualifications added yet</p>:<div className="cf-items-list">{formData.education.map((edu,i)=>(<div key={i} className="cf-list-item"><div className="cf-item-content"><div className="cf-item-header"><strong className="cf-item-title">{edu.qualification}</strong>{edu.certNumber&&<span className="cf-item-subtitle"> – #{edu.certNumber}</span>}</div><div className="cf-item-meta"><span>{edu.institution}</span><span>{edu.startDate} – {edu.endDate||'Current'}</span></div></div><div className="cf-item-actions"><button onClick={()=>editEducation(i)} className="cf-edit-btn" aria-label={`Edit ${edu.qualification}`}><FiEdit2/></button><button onClick={()=>deleteEducation(i)} className="cf-delete-btn" aria-label={`Delete ${edu.qualification}`}><FiTrash2/></button></div></div>))}</div>}</div>
                </div>
              )}

              {activeSection==='certifications'&&(
                <div className="cf-form-section-content">
                  <h3 className="cf-form-section-title"><FiShield/> Australian Certifications & Licenses</h3>
                  <div className="cf-form-card">
                    <h4 className="cf-sub-section-title">Essential Australian Certifications</h4><p className="cf-section-desc">Add your essential Australian disability support certifications and licenses</p>
                    <div className="cf-skills-input">
                      <div className="cf-form-group"><label className="cf-form-label">Certification/License Name*<input value={currentCertification.name} onChange={e=>setCurrentCertification({...currentCertification,name:e.target.value})} placeholder="First Aid & CPR Certificate" className="cf-form-input"/></label><label className="cf-form-label">Issuing Body<input value={currentCertification.issuingBody} onChange={e=>setCurrentCertification({...currentCertification,issuingBody:e.target.value})} placeholder="Australian Red Cross, St John Ambulance" className="cf-form-input"/></label></div>
                      <div className="cf-form-group"><label className="cf-form-label">Certificate Number<input value={currentCertification.certNumber} onChange={e=>setCurrentCertification({...currentCertification,certNumber:e.target.value})} placeholder="FA1234567" className="cf-form-input"/></label><label className="cf-form-label">Expiry Date<input value={currentCertification.expiryDate} onChange={e=>setCurrentCertification({...currentCertification,expiryDate:e.target.value})} placeholder="Month Year" className="cf-form-input"/></label></div>
                    ...
                    <div className="cf-items-list">{formData.certifications.map((cert,i)=>(<div key={i} className="cf-list-item"><div><strong>{cert.name}</strong>{cert.issuingBody&&` – ${cert.issuingBody}`}{cert.certNumber&&` (#${cert.certNumber})`}{cert.expiryDate&&` – Expires: ${cert.expiryDate}`}</div><div className="cf-item-actions"><button onClick={()=>editCertification(i)} className="cf-edit-btn"><FiEdit2/></button><button onClick={()=>deleteCertification(i)} className="cf-delete-btn"><FiTrash2/></button></div></div>))}{formData.certifications.length===0&&<p className="cf-empty-msg">No certifications added yet</p>}</div>
                  </div>
                  <div className="cf-form-card">
                    <h4 className="cf-sub-section-title">Key Achievements</h4><p className="cf-section-desc">Highlight your significant achievements in disability support</p>
                    <div className="cf-skills-input"><textarea value={currentAchievement.description} onChange={e=>setCurrentAchievement({...currentAchievement,description:e.target.value})} placeholder="Successfully implemented positive behaviour support plan reducing incidents by 60%" className="cf-form-textarea" rows="3"/><div className="cf-form-actions"><button onClick={addAchievement} className="cf-add-btn" disabled={!currentAchievement.description.trim()}><FiPlus/> {currentAchievement.isEditing?'Update Achievement':'Add Achievement'}</button>{currentAchievement.isEditing&&<button onClick={()=>setCurrentAchievement(defaultAchievement())} className="cf-cancel-btn"><FiX/> Cancel</button>}</div></div>
                    <div className="cf-items-list">{formData.achievements.map((ach,i)=>(<div key={i} className="cf-list-item"><span>{ach.description}</span><div className="cf-item-actions"><button onClick={()=>editAchievement(i)} className="cf-edit-btn"><FiEdit2/></button><button onClick={()=>deleteAchievement(i)} className="cf-delete-btn"><FiTrash2/></button></div></div>))}{formData.achievements.length===0&&<p className="cf-empty-msg">No achievements added yet</p>}</div>
                  </div>
                  <div className="cf-form-card">
                    <h4 className="cf-sub-section-title"><FiCalendar/> Availability</h4><p className="cf-section-desc">Specify your working availability for disability support roles</p>
                    <div className="cf-skills-input">
                      <div className="cf-form-group"><label className="cf-form-label">Days Available*<input value={currentAvailability.days} onChange={e=>setCurrentAvailability({...currentAvailability,days:e.target.value})} placeholder="Monday - Friday, Weekends, 24/7 Roster" className="cf-form-input"/></label><label className="cf-form-label">Hours<input value={currentAvailability.hours} onChange={e=>setCurrentAvailability({...currentAvailability,hours:e.target.value})} placeholder="7am - 3pm, Night Shifts, Flexible" className="cf-form-input"/></label></div>
                      <label className="cf-form-label">Employment Type<input value={currentAvailability.type} onChange={e=>setCurrentAvailability({...currentAvailability,type:e.target.value})} placeholder="Full-time, Part-time, Casual, Contract" className="cf-form-input"/></label>
                      <div className="cf-form-actions"><button onClick={addAvailability} className="cf-add-btn" disabled={!currentAvailability.days.trim()}><FiPlus/> {currentAvailability.isEditing?'Update Availability':'Add Availability'}</button>{currentAvailability.isEditing&&<button onClick={()=>setCurrentAvailability(defaultAvailability())} className="cf-cancel-btn"><FiX/> Cancel</button>}</div>
                    </div>
                    <div className="cf-items-list">{formData.availability.map((avail,i)=>(<div key={i} className="cf-list-item"><div><strong>{avail.days}</strong>{avail.hours&&` – ${avail.hours}`}{avail.type&&` (${avail.type})`}</div><div className="cf-item-actions"><button onClick={()=>editAvailability(i)} className="cf-edit-btn"><FiEdit2/></button><button onClick={()=>deleteAvailability(i)} className="cf-delete-btn"><FiTrash2/></button></div></div>))}{formData.availability.length===0&&<p className="cf-empty-msg">No availability specified yet</p>}</div>
                  </div>
                  <div className="cf-form-card">
                    <h4 className="cf-sub-section-title">Languages</h4><p className="cf-section-desc">List languages you speak (important for diverse Australian communities)</p>
                    <div className="cf-skills-input">
                      <div className="cf-form-group"><label className="cf-form-label">Language*<input value={currentLanguage.name} onChange={e=>setCurrentLanguage({...currentLanguage,name:e.target.value})} placeholder="Arabic, Vietnamese, Mandarin" className="cf-form-input"/></label><label className="cf-form-label">Proficiency Level<input value={currentLanguage.proficiency} onChange={e=>setCurrentLanguage({...currentLanguage,proficiency:e.target.value})} placeholder="Fluent, Conversational, Basic" className="cf-form-input"/></label></div>
              ...
              {activeSection==='skills'&&(
                <div className="cf-form-section-content">
                  <h3 className="cf-form-section-title"><FiActivity/> Disability Support Skills</h3>
                  <div className="cf-form-card">
                    <h4 className="cf-sub-section-title">Specialised Disability Support Skills</h4><p className="cf-section-desc">List your specialised skills in disability support categories</p>
                    <div className="cf-skills-input">
                      <div className="cf-form-group"><label className="cf-form-label">Skill Name*<input value={currentSkill.name} onChange={e=>setCurrentSkill({...currentSkill,name:e.target.value})} placeholder="Personal Care Assistance" className="cf-form-input"/></label><label className="cf-form-label">Category<input value={currentSkill.category} onChange={e=>setCurrentSkill({...currentSkill,category:e.target.value})} placeholder="Personal Care, Community Access, Behaviour Support" className="cf-form-input"/></label></div>
                ...
                <div className="cf-form-section-content">
                  ...
                  <div className="cf-form-card">
                    <div className="cf-font-grid">
                      {[{key:'name',label:'Name'},{key:'sectionTitle',label:'Section Titles'},{key:'jobTitle',label:'Job Titles'},{key:'qualification',label:'Qualifications'},{key:'institution',label:'Organisation/Institution'},{key:'institutionDate',label:'Dates'},{key:'regularText',label:'Regular Text'},{key:'bulletText',label:'Bullet Points'},{key:'contactInfo',label:'Contact Info'},{key:'skillText',label:'Skills Text'},{key:'certText',label:'Certification Text'}].map(({key,label})=>(<div key={key} className="cf-font-control"><label className="cf-font-label"><span>{label}</span><span className="cf-font-value">{fontSizes[key]}pt</span></label><input type="range" min={key==='institutionDate'?4:6} max={key==='name'?24:key==='sectionTitle'?18:key==='jobTitle'||key==='qualification'?16:14} value={fontSizes[key]} onChange={e=>handleFontSizeChange(key,e.target.value)} className="cf-font-slider"/></div>))}
                    </div>
        ...
        <section className="cf-faq-section" aria-labelledby="faq-title">
          <div className="cf-container">
            <div className="cf-section-header">
              ...
              <p className="cf-section-subtitle">Everything you need to know about creating professional disability support resumes for the Australian market.</p>
            </div>
        ...
        <section className="cf-cta-section" aria-labelledby="cta-title">
          <div className="cf-container">
            <div className="cf-cta-content">
              <h2 className="cf-cta-title" id="cta-title">Ready to Advance Your Disability Support Career in Australia?</h2>
              <p className="cf-cta-subtitle">Join 150,000+ Australian disability support professionals who landed their dream jobs with our free NDIS-optimized resume builder.</p>
              <div className="cf-cta-btn-wrap">
                <button onClick={()=>setActiveSection('personal')} className="cf-cta-btn" aria-label="Create your free disability support resume now—no sign-up required">
                  <span className="cf-cta-btn-text">Create Your Free NDIS Resume Now</span>
                  <FiArrowRight className="cf-cta-btn-icon"/>
                </button>
              </div>
              <div className="cf-cta-guarantee"><FiCheck className="cf-guarantee-icon"/><span className="cf-guarantee-text">No credit card required • Free forever • Download in minutes • NDIS Optimized for Australia</span></div>
            </div>
          </div>
        </section>

        {showFullPreview&&(<div className="cf-modal" onClick={()=>setShowFullPreview(false)}><div className="cf-modal-content" onClick={e=>e.stopPropagation()}><div className="cf-modal-header"><h3>Full Disability Support Resume Preview</h3><button className="cf-close-btn" onClick={()=>setShowFullPreview(false)}><FiX/></button></div><div className="cf-modal-pages"><div className="cf-modal-page"><DisabilitySupportTemplate formData={formData}/></div></div></div></div>)}
