...
const careerFlowStyles = `
  ...
  * { box-sizing: border-box; }
  ...
  .cf-electrician-template { color: #000000; line-height: 1.25; height: 100%; }
  ...
  .cf-modal-page .cf-electrician-template { width: 100%; height: 100%; padding: 10mm 15mm; box-sizing: border-box; overflow-y: auto; }
  ...
  @media (max-width: 768px) {
    ...
    .cf-modal-page .cf-electrician-template { padding: 8mm 12mm; }
  ...
  @media print {
    ...
    .cf-electrician-template, .cf-section { page-break-inside: avoid; }
  }
`;

const ElectricianResume = ({ seoData, buildTimestamp }) => {
  const {
    currentDate,
    lastModifiedDate,
    reviewDates,
  ...
  const defaultLicense = () => ({ name: '', issuingAuthority: '', licenseNumber: '', expiryDate: '', country: '', isEditing: false, editIndex: null });
  ...
  const defaultCertification = () => ({ name: '', issuingBody: '', dateEarned: '', expiryDate: '', isEditing: false, editIndex: null });

  const [formData, setFormData] = useState({ fullName: '', email: '', phone: '', address: '', summary: '', experience: [], education: [], specialties: [], licenses: [], certifications: [], affiliations: [], skills: [], languages: [], socialLinks: [] });
  const [fontSizes, setFontSizes] = useState({ name: 14, sectionTitle: 10, contactInfo: 7, jobTitle: 9, company: 7, degree: 9, institution: 7, institutionDate: 6, regularText: 8, bulletText: 8, skillText: 7, licenseText: 8, certificationText: 8 });
  ...
  const testimonials = [
    {
      quote: "Created my electrician resume in 10 minutes and landed interviews the same week. The industry-specific templates are incredible!",
      metric: "Found Master Electrician Position in 2 Weeks",
      name: "Michael R.",
      role: "Master Electrician",
      company: "Global Electrical Contractors"
    },
    {
      quote: "Finally a resume builder that understands electrical certifications. The ATS templates helped me pass screening systems.",
    ...
    {
      quote: "As a recent electrical trade school graduate, the entry-level templates were perfect. Landed my first commercial job using this builder.",
      metric: "First Job After Graduation",
      name: "James P.",
      role: "Apprentice Electrician",
      company: "Industrial Electrical Services"
    },
    {
      quote: "The electrician resume builder saved me - could update my CV between jobs. Professional results without the cost.",
      metric: "Career Advancement Success",
      name: "Robert L.",
      role: "Journeyman Electrician",
      company: "Commercial Electrical"
    },
    {
      quote: "Electrical ATS-friendly templates actually work! Got callbacks from companies that previously ignored my applications.",
    ...
    {
      quote: "Free PDF download with proper electrical formatting? Unbeatable value. Best electrician resume builder I've found.",
  ...
  const resetFontSizes = () => setFontSizes({ name: 14, sectionTitle: 10, contactInfo: 7, jobTitle: 9, company: 7, degree: 9, institution: 7, institutionDate: 6, regularText: 8, bulletText: 8, skillText: 7, licenseText: 8, certificationText: 8 });
  const getSocialIcon = () => <FiGlobe />;
  const formatSocialUrl = (url) => url ? url.replace(/(https?:\/\/)?(www\.)?/, '').replace(/\/$/, '') : '';
  const handleInputChange = (e) => setFormData({ ...formData, [e.target.name]: e.target.value });
  const hasContent = () => formData.fullName || formData.email || formData.summary || formData.socialLinks.length > 0 || formData.experience.length > 0 || formData.education.length > 0 || formData.specialties.length > 0 || formData.licenses.length > 0 || formData.certifications.length > 0 || formData.affiliations.length > 0 || formData.skills.length > 0 || formData.languages.length > 0;
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      pdf.save(`${formData.fullName || 'electrician_resume'}_resume.pdf`);
      setShowSharePopup(true);
    } catch (err) { console.error("PDF Error:", err); alert("Failed to generate PDF."); }
    finally { setIsGeneratingPDF(false); }
  };

  const ElectricianTemplate = ({ formData }) => {
    ...
    return (
      <div className="cf-electrician-template">
        ...
        {hasLicenses && <section className="cf-section"><h2 className="cf-section-title" style={{ fontSize: `${fontSizes.sectionTitle}pt`, fontWeight: 700 }}>LICENSES & REGISTRATIONS</h2>{formData.licenses.map((l, i) => (<div key={i} className="cf-license-item" style={{ fontSize: `${fontSizes.licenseText}pt`, fontWeight: 400 }}><strong style={{ fontWeight: 700 }}>{l.name}</strong>{l.issuingAuthority && ` – ${l.issuingAuthority}`}{l.country && ` (${l.country})`}{l.licenseNumber && ` (License #: ${l.licenseNumber})`}{l.expiryDate && ` – Expires: ${l.expiryDate}`}</div>))}</section>}
        {hasCertifications && <section className="cf-section"><h2 className="cf-section-title" style={{ fontSize: `${fontSizes.sectionTitle}pt`, fontWeight: 700 }}>CERTIFICATIONS</h2>{formData.certifications.map((c, i) => (<div key={i} className="cf-cert-item" style={{ fontSize: `${fontSizes.certificationText}pt`, fontWeight: 400 }}><strong style={{ fontWeight: 700 }}>{c.name}</strong>{c.issuingBody && ` – ${c.issuingBody}`}{c.dateEarned && ` (Earned: ${c.dateEarned})`}{c.expiryDate && ` – Expires: ${c.expiryDate}`}</div>))}</section>}
        {hasSpecialties && <section className="cf-section"><h2 className="cf-section-title" style={{ fontSize: `${fontSizes.sectionTitle}pt`, fontWeight: 700 }}>AREAS OF EXPERTISE</h2><ul className="cf-skills-list">{formData.specialties.map((s, i) => <li key={i} style={{ fontSize: `${fontSizes.skillText}pt`, fontWeight: 400 }}>{s.name}</li>)}</ul></section>}
        {hasSkills && <section className="cf-section"><h2 className="cf-section-title" style={{ fontSize: `${fontSizes.sectionTitle}pt`, fontWeight: 700 }}>TECHNICAL SKILLS</h2><ul className="cf-bullet-list">{formData.skills.map((s, i) => (<li key={i} style={{ fontSize: `${fontSizes.bulletText}pt`, fontWeight: 400 }}>{s.name}{s.proficiency && ` (${s.proficiency})`}</li>))}</ul></section>}
      ...
      <div className="cf-resume-builder" lang="en-US">
                ...
                "@graph": [
                  {
                    ...
                    "mainEntity": {
                      ...
                      "name": "Electrician Resume Builder - ATS Optimized Electrical Resume Maker",
                      ...
                      "description": "Free online ATS-friendly electrician resume builder for industrial, commercial, residential, and specialized electricians globally.",
                      "featureList": [
                        "Electrical ATS-Optimized Templates",
                        "International Standards Compliance",
                        "One-Click PDF Download",
                        "Electrical Experience Formatting",
                  ...
                  {
                    "@type": "Service",
                    "serviceType": "Online Electrician Resume Building Service",
                    "provider": {
                      "@type": "Organization",
                      "name": "Professional Electrician Resume Free",
                      "url": "https://professionalresumefree.com",
                    ...
                    "hasOfferCatalog": {
                      "@type": "OfferCatalog",
                      "name": "Free Electrician Resume Building Services",
                      "itemListElement": [
                        {
                          "@type": "Offer",
                          "itemOffered": {
                            "@type": "Service",
                            "name": "Electrical ATS Resume Templates"
                          }
                        },
                        {
                          "@type": "Offer",
                          "itemOffered": {
                            "@type": "Service",
                            "name": "Electrician Resume Editing"
                          }
                        }
                      ]
                    },
                    "description": "Free ATS-friendly electrician resume builder for electrical professionals worldwide",
                    "offers": {
                      "@type": "Offer",
                      "price": "0",
                      "priceCurrency": "USD"
        ...
        <nav className="cf-breadcrumb" aria-label="Breadcrumb">
          <ol>
            ...
            <li><Link href="/ats-friendly-electrician-resume-builder" className="cf-breadcrumb-link" prefetch={false}><span className="cf-breadcrumb-text">Free Electrical Template</span></Link></li>
          </ol>
        </nav>
            ...
            <div className="cf-hero-content">
              ...
              <div className="cf-cta-buttons">
                <button onClick={() => setActiveSection('personal')} className="cf-btn-primary" aria-label="Start building your free electrician resume now—no sign-up required">
                  <span className="cf-btn-text">Start Building Your Electrician Resume Now</span>
                  <FiArrowRight className="cf-btn-icon" />
                  <div className="cf-btn-pulse"></div>
                </button>
                <button onClick={generatePDF} className="cf-btn-secondary" aria-label="Download electrician resume as PDF" disabled={isGeneratingPDF || !hasContent()}><FiDownload className="cf-btn-icon"/><span className="cf-btn-text">Download Electrician Resume PDF</span></button>
              </div>
              <div className="cf-hero-stats">
                <div className="cf-stat-item"><span className="cf-stat-number">2M+</span><span className="cf-stat-label">Electricians</span></div>
              ...
              <div className="cf-industry-badges">
                <div className="cf-badge-grid">
                  <span className="cf-badge-item"><FiZap/> Industrial Electrician</span>
                  <span className="cf-badge-item"><FiGrid/> Commercial Electrician</span>
                  <span className="cf-badge-item"><FiHome/> Residential Electrician</span>
                  <span className="cf-badge-item"><FiTool/> Maintenance Electrician</span>
                  <span className="cf-badge-item"><FiWifi/> Data & Telecom</span>
                  <span className="cf-badge-item"><FiBattery/> Renewable Energy</span>
                  <span className="cf-badge-item"><FiCpu/> Automation & Controls</span>
                  <span className="cf-badge-item"><FiCloud/> Smart Grid Systems</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="cf-layout">
          <div className="cf-preview-section">
            ...
            <div className={`cf-preview-container ${showFullPreview ? 'cf-full-preview' : ''}`}><div className="cf-resume-card"><div className="cf-preview-content"><div className="cf-resume-preview" ref={resumeRef}><ElectricianTemplate formData={formData}/></div></div></div></div>
          </div>
          <div className="cf-form-section">
            <div className="cf-form-nav">
              {[{id:'personal',label:'Personal',icon:<FiUser/>},{id:'experience',label:'Electrical Exp.',icon:<FiTool/>},{id:'education',label:'Education',icon:<FiBook/>},{id:'certifications',label:'Licenses & Certs',icon:<FiShield/>},{id:'skills',label:'Skills & Specialties',icon:<FiZap/>},{id:'settings',label:'Font Settings',icon:<FiSettings/>}].map(item => (<button key={item.id} className={`cf-nav-btn ${activeSection===item.id?'cf-nav-btn-active':''}`} onClick={()=>setActiveSection(item.id)}>{item.icon}<span>{item.label}</span></button>))}
            </div>
            <div className="cf-form-content">
              {activeSection==='personal'&&(
                <div className="cf-form-section-content">
                  ...
                  <div className="cf-form-card">
                    <div className="cf-form-group"><label className="cf-form-label">Full Name*<input type="text" name="fullName" value={formData.fullName} onChange={handleInputChange} placeholder="John Smith, Master Electrician" required className="cf-form-input"/></label><label className="cf-form-label">Email*<input type="email" name="email" value={formData.email} onChange={handleInputChange} placeholder="john.smith@electrical.com" required className="cf-form-input"/></label></div>
                  ...
                  <div className="cf-form-card"><label className="cf-form-label">Professional Electrical Summary*<textarea name="summary" value={formData.summary} onChange={handleInputChange} placeholder="Certified Master Electrician with 10+ years of experience in industrial, commercial, and residential electrical systems. Expertise in NEC/IEC compliance, automation controls, and renewable energy installations. Proven track record of managing large-scale electrical projects with zero safety incidents. Skilled in PLC programming, power distribution, and energy management systems." required className="cf-form-textarea" rows="6"/><div className="cf-char-count">{formData.summary.length}/500 characters</div></label></div>
                  <div className="cf-form-card">
                    <h4 className="cf-sub-section-title"><FiGlobe/> Professional Links</h4><p className="cf-section-desc">Add your professional electrical profiles (LinkedIn, portfolio, etc.)</p>
                    <div className="cf-social-input">
                      <select value={currentSocialLink.platform} onChange={e=>setCurrentSocialLink({...currentSocialLink,platform:e.target.value})} className="cf-form-select"><option value="">Select Platform</option><option value="LinkedIn">LinkedIn</option><option value="Portfolio">Professional Portfolio</option><option value="Website">Personal Website</option><option value="GitHub">GitHub (for automation/code)</option><option value="Upwork">Upwork/Freelance</option></select>
              ...
              {activeSection==='experience'&&(
                <div className="cf-form-section-content">
                  <h3 className="cf-form-section-title"><FiTool/> Electrical Experience</h3>
                  <p className="cf-section-desc">List your electrical positions in reverse chronological order (most recent first)</p>
                  <div className="cf-form-card">
                    <div className="cf-form-group"><label className="cf-form-label">Position Title*<input value={currentExperience.position} onChange={e=>setCurrentExperience({...currentExperience,position:e.target.value})} placeholder="Master Electrician, Project Lead" required className="cf-form-input"/></label><label className="cf-form-label">Company/Contractor*<input value={currentExperience.employer} onChange={e=>setCurrentExperience({...currentExperience,employer:e.target.value})} placeholder="Global Electrical Contractors Inc." required className="cf-form-input"/></label></div>
                    <label className="cf-form-label">Project Type / Sector<input value={currentExperience.projectType} onChange={e=>setCurrentExperience({...currentExperience,projectType:e.target.value})} placeholder="Industrial Manufacturing / Commercial Construction" className="cf-form-input"/></label>
                    ...
                    <label className="cf-form-label">Key Electrical Responsibilities & Achievements*<textarea value={currentExperience.description} onChange={e=>setCurrentExperience({...currentExperience,description:e.target.value})} placeholder="• Led electrical installation for $15M industrial manufacturing facility
• Designed and implemented PLC control systems for automation
• Managed team of 8 electricians on commercial construction projects
• Reduced energy consumption by 25% through efficient system design
• Ensured 100% compliance with NEC/IEC electrical codes
• Maintained zero safety incidents over 5-year period" required className="cf-form-textarea" rows="8"/><div className="cf-char-count">{currentExperience.description.length}/2000 characters</div></label>
                    <div className="cf-form-actions"><button onClick={addExperience} className="cf-add-btn" disabled={!currentExperience.position||!currentExperience.employer||!currentExperience.startDate}><FiPlus/> {currentExperience.isEditing?'Update Electrical Experience':'Add Electrical Experience'}</button>{currentExperience.isEditing&&<button onClick={()=>setCurrentExperience(defaultExperience())} className="cf-cancel-btn"><FiX/> Cancel</button>}</div>
                  </div>
                  <div className="cf-form-card"><h4 className="cf-sub-section-title">Your Electrical Experience</h4>{formData.experience.length===0?<p className="cf-empty-msg">No electrical experience added yet</p>:<div className="cf-items-list">{formData.experience.map((exp,i)=>(<div key={i} className="cf-list-item"><div className="cf-item-content"><div className="cf-item-header"><strong className="cf-item-title">{exp.position}</strong><span className="cf-item-subtitle">at {exp.employer}</span></div><div className="cf-item-meta"><span>{exp.startDate} – {exp.endDate||'Present'}</span>{exp.projectType&&<span>{exp.projectType}</span>}</div><div className="cf-item-desc">{exp.description.split('\n').filter(l=>l.trim()).map((line,j)=><p key={j} className="cf-bullet-point">• {line}</p>)}</div></div><div className="cf-item-actions"><button onClick={()=>editExperience(i)} className="cf-edit-btn" aria-label={`Edit ${exp.position} experience`}><FiEdit2/></button><button onClick={()=>deleteExperience(i)} className="cf-delete-btn" aria-label={`Delete ${exp.position} experience`}><FiTrash2/></button></div></div>))}</div>}</div>
                </div>
              )}
              {activeSection==='education'&&(
                <div className="cf-form-section-content">
                  <h3 className="cf-form-section-title"><FiBook/> Education & Training</h3>
                  <p className="cf-section-desc">List your electrical education, degrees, apprenticeships, and relevant training</p>
                  <div className="cf-form-card">
                    <div className="cf-form-group"><label className="cf-form-label">Institution/Trade School*<input value={currentEducation.institution} onChange={e=>setCurrentEducation({...currentEducation,institution:e.target.value})} placeholder="National Electrical Training Institute" required className="cf-form-input"/></label><label className="cf-form-label">Degree/Certification*<input value={currentEducation.degree} onChange={e=>setCurrentEducation({...currentEducation,degree:e.target.value})} placeholder="Electrical Apprenticeship, Associate Degree in Electrical Technology" required className="cf-form-input"/></label></div>
                    <label className="cf-form-label">Program / Specialization / Focus<input value={currentEducation.program} onChange={e=>setCurrentEducation({...currentEducation,program:e.target.value})} placeholder="Industrial Electrical Systems, Renewable Energy" className="cf-form-input"/></label>
              ...
              {activeSection==='certifications'&&(
                <div className="cf-form-section-content">
                  ...
                  <div className="cf-form-card">
                    <h4 className="cf-sub-section-title">Electrical Licenses & Registrations</h4><p className="cf-section-desc">Add your professional electrical licenses (Master Electrician, Journeyman, etc.)</p>
                    <div className="cf-form-group"><label className="cf-form-label">License Name*<input value={currentLicense.name} onChange={e=>setCurrentLicense({...currentLicense,name:e.target.value})} placeholder="Master Electrician License" className="cf-form-input"/></label><label className="cf-form-label">Issuing Authority<input value={currentLicense.issuingAuthority} onChange={e=>setCurrentLicense({...currentLicense,issuingAuthority:e.target.value})} placeholder="State Electrical Board" className="cf-form-input"/></label></div>
                    <div className="cf-form-group"><label className="cf-form-label">Country/Region<input value={currentLicense.country} onChange={e=>setCurrentLicense({...currentLicense,country:e.target.value})} placeholder="USA, UK, Canada, Australia, EU" className="cf-form-input"/></label><label className="cf-form-label">License Number<input value={currentLicense.licenseNumber} onChange={e=>setCurrentLicense({...currentLicense,licenseNumber:e.target.value})} placeholder="ME123456" className="cf-form-input"/></label></div>
                    <label className="cf-form-label">Expiry Date<input value={currentLicense.expiryDate} onChange={e=>setCurrentLicense({...currentLicense,expiryDate:e.target.value})} placeholder="Month Year" className="cf-form-input"/></label>
                    ...
                    <div className="cf-items-list">{formData.licenses.map((l,i)=>(<div key={i} className="cf-list-item"><div><strong>{l.name}</strong>{l.issuingAuthority&&` – ${l.issuingAuthority}`}{l.country&&` (${l.country})`}{l.licenseNumber&&` (#${l.licenseNumber})`}{l.expiryDate&&` – Expires: ${l.expiryDate}`}</div><div className="cf-item-actions"><button onClick={()=>editLicense(i)} className="cf-edit-btn"><FiEdit2/></button><button onClick={()=>deleteLicense(i)} className="cf-delete-btn"><FiTrash2/></button></div></div>))}{formData.licenses.length===0&&<p className="cf-empty-msg">No licenses added yet</p>}</div>
                  </div>
                  <div className="cf-form-card">
                    <h4 className="cf-sub-section-title">Professional Certifications</h4><p className="cf-section-desc">Add your electrical certifications (OSHA, NFPA, manufacturer-specific, etc.)</p>
                    <div className="cf-form-group"><label className="cf-form-label">Certification Name*<input value={currentCertification.name} onChange={e=>setCurrentCertification({...currentCertification,name:e.target.value})} placeholder="OSHA 30-Hour, NFPA 70E, Siemens PLC Certified" className="cf-form-input"/></label><label className="cf-form-label">Issuing Body<input value={currentCertification.issuingBody} onChange={e=>setCurrentCertification({...currentCertification,issuingBody:e.target.value})} placeholder="OSHA, NFPA, Siemens, ABB" className="cf-form-input"/></label></div>
                    ...
                    <div className="cf-items-list">{formData.certifications.map((c,i)=>(<div key={i} className="cf-list-item"><div><strong>{c.name}</strong>{c.issuingBody&&` – ${c.issuingBody}`}{c.dateEarned&&` (Earned: ${c.dateEarned})`}{c.expiryDate&&` – Expires: ${c.expiryDate}`}</div><div className="cf-item-actions"><button onClick={()=>editCertification(i)} className="cf-edit-btn"><FiEdit2/></button><button onClick={()=>deleteCertification(i)} className="cf-delete-btn"><FiTrash2/></button></div></div>))}{formData.certifications.length===0&&<p className="cf-empty-msg">No certifications added yet</p>}</div>
                  </div>
                  <div className="cf-form-card">
                    <h4 className="cf-sub-section-title">Professional Affiliations</h4><p className="cf-section-desc">Add your professional electrical organization memberships</p>
                    <div className="cf-form-group"><label className="cf-form-label">Organization*<input value={currentAffiliation.organization} onChange={e=>setCurrentAffiliation({...currentAffiliation,organization:e.target.value})} placeholder="International Brotherhood of Electrical Workers (IBEW)" className="cf-form-input"/></label><label className="cf-form-label">Role / Membership Type<input value={currentAffiliation.role} onChange={e=>setCurrentAffiliation({...currentAffiliation,role:e.target.value})} placeholder="Member, Committee Chair" className="cf-form-input"/></label></div>
              ...
              {activeSection==='skills'&&(
                <div className="cf-form-section-content">
                  <h3 className="cf-form-section-title"><FiZap/> Skills & Specialties</h3>
                  <div className="cf-form-card">
                    <h4 className="cf-sub-section-title">Areas of Electrical Expertise</h4><p className="cf-section-desc">List your electrical specialties and areas of expertise</p>
                    <div className="cf-skills-input">
                      <input value={currentSpecialty.name} onChange={e=>setCurrentSpecialty({...currentSpecialty,name:e.target.value})} placeholder="Industrial Automation, Renewable Energy Systems, PLC Programming" className="cf-form-input"/>
                  ...
                  <div className="cf-form-card">
                    <h4 className="cf-sub-section-title">Technical Skills</h4><p className="cf-section-desc">List your specific technical skills and proficiencies</p>
                    <div className="cf-form-group"><label className="cf-form-label">Skill*<input value={currentSkill.name} onChange={e=>setCurrentSkill({...currentSkill,name:e.target.value})} placeholder="PLC Programming (Siemens, Allen-Bradley)" className="cf-form-input"/></label><label className="cf-form-label">Proficiency Level<input value={currentSkill.proficiency} onChange={e=>setCurrentSkill({...currentSkill,proficiency:e.target.value})} placeholder="Expert, Advanced, Proficient" className="cf-form-input"/></label></div>
        ...
        <section className="cf-faq-section" aria-labelledby="faq-title">
          <div className="cf-container">
            <div className="cf-section-header">
              ...
              <p className="cf-section-subtitle">Everything you need to know about creating professional electrician resumes with our tool.</p>
            </div>
        ...
        <section className="cf-cta-section" aria-labelledby="cta-title">
          <div className="cf-container">
            <div className="cf-cta-content">
              <h2 className="cf-cta-title" id="cta-title">Ready to Advance Your Electrical Career?</h2>
              <p className="cf-cta-subtitle">Join 2 million+ electricians who landed their dream jobs with our free ATS-friendly electrician resume builder.</p>
              <div className="cf-cta-btn-wrap">
                <button onClick={()=>setActiveSection('personal')} className="cf-cta-btn" aria-label="Create your free electrician resume now—no sign-up required">
                  <span className="cf-cta-btn-text">Create Your Free Electrician Resume Now</span>
        ...
        {showFullPreview&&(<div className="cf-modal" onClick={()=>setShowFullPreview(false)}><div className="cf-modal-content" onClick={e=>e.stopPropagation()}><div className="cf-modal-header"><h3>Full Electrician Resume Preview</h3><button className="cf-close-btn" onClick={()=>setShowFullPreview(false)}><FiX/></button></div><div className="cf-modal-pages"><div className="cf-modal-page"><ElectricianTemplate formData={formData}/></div></div></div></div>)}
...
export default ElectricianResume;