...
const careerFlowStyles = `
  ...
  * { box-sizing: border-box; }
  ...
  .cf-eng-template { color: #000000; line-height: 1.25; height: 100%; }
  ...
  .cf-modal-page .cf-eng-template { width: 100%; height: 100%; padding: 10mm 15mm; box-sizing: border-box; overflow-y: auto; }
  ...
  @media (max-width: 768px) {
    ...
    .cf-modal-page .cf-eng-template { padding: 8mm 12mm; }
  ...
  @media (max-width: 480px) {
    ...
    .cf-modal-page .cf-eng-template { padding: 6mm 8mm; }
  ...
  @media print {
    ...
    .cf-eng-template, .cf-section { page-break-inside: avoid; }
  }
`;

const EngineeringResume = ({ seoData, buildTimestamp }) => {
  const {
    currentDate,
    lastModifiedDate,
    reviewDates,
  ...
  const defaultEducation = () => ({ institution: '', degree: '', major: '', gpa: '', startDate: '', endDate: '', isEditing: false, editIndex: null });
  ...
  const defaultProject = () => ({ name: '', technologies: '', description: '', isEditing: false, editIndex: null });
  const defaultPublication = () => ({ title: '', conference: '', year: '', isEditing: false, editIndex: null });
  ...
  const [formData, setFormData] = useState({ fullName: '', email: '', phone: '', address: '', title: '', summary: '', experience: [], education: [], technicalSkills: [], certifications: [], projects: [], publications: [], languages: [], socialLinks: [] });
  ...
  const faqs = [
    { question: "Is this engineering resume builder really free with no hidden costs?", answer: "Yes, our engineering resume builder is completely free with no hidden costs or watermarks. Create, edit, and download your professional engineering resume in PDF format without any payment required." },
    { question: "What does ATS-friendly mean for engineering resumes?", answer: "ATS-friendly means our engineering resume templates are optimized to pass through Applicant Tracking Systems used by 95% of tech companies and engineering firms. This ensures your technical skills and experience are properly scanned and recognized." },
    { question: "Can I download my engineering resume as PDF without creating an account?", answer: "Absolutely! Download your professional engineering resume in PDF format without creating an account. Everything is completely free and accessible immediately for all engineering disciplines." },
    { question: "How many engineering resume templates are available for free?", answer: "We offer professionally designed ATS-friendly engineering resume templates for software, mechanical, electrical, civil, chemical engineers and all technical specialties. All templates are completely free and optimized for engineering hiring." },
    { question: "How does your engineering resume builder work?", answer: "Our builder uses ATS-optimized engineering templates with proper technical terminology formatting. We guide you to highlight technical experience, projects, certifications, and specialized skills that engineering employers look for." },
    { question: "Can I include engineering projects and GitHub links in my resume?", answer: "Yes! Our builder has dedicated sections for engineering projects, technical skills, and professional links including GitHub, LinkedIn, and portfolio websites." }
  ];
  ...
  const hasContent = () => formData.fullName || formData.email || formData.summary || formData.title || formData.socialLinks.length > 0 || formData.experience.length > 0 || formData.education.length > 0 || formData.technicalSkills.length > 0 || formData.certifications.length > 0 || formData.projects.length > 0 || formData.publications.length > 0 || formData.languages.length > 0;
    ...
    return (
      <div className="cf-eng-template">
        <header className="cf-resume-header">
          ...
          {formData.title && <p className="cf-professional-title" style={{ fontSize: `${fontSizes.degree}pt`, fontWeight: 400 }}>{formData.title}</p>}
        ...
        {hasExperience && <section className="cf-section"><h2 className="cf-section-title" style={{ fontSize: `${fontSizes.sectionTitle}pt`, fontWeight: 700 }}>PROFESSIONAL EXPERIENCE</h2>{formData.experience.map((exp, i) => (<div key={i} className="cf-experience-item"><div className="cf-experience-header"><h3 style={{ fontSize: `${fontSizes.jobTitle}pt`, fontWeight: 700 }}>{exp.position}</h3><p className="cf-company" style={{ fontSize: `${fontSizes.company}pt`, fontWeight: 400 }}>{exp.company} | {exp.startDate} – {exp.endDate || 'Present'}</p>{exp.technologies && <p style={{ fontSize: `${fontSizes.skillText}pt`, fontWeight: 400 }}><strong>Technologies:</strong> {exp.technologies}</p>}</div><ul className="cf-bullet-list">{exp.description.split('\n').filter(l => l.trim()).map((line, j) => <li key={j} style={{ fontSize: `${fontSizes.bulletText}pt`, fontWeight: 400 }}>{line}</li>)}</ul></div>))}</section>}
        {hasTechnicalSkills && <section className="cf-section"><h2 className="cf-section-title" style={{ fontSize: `${fontSizes.sectionTitle}pt`, fontWeight: 700 }}>TECHNICAL SKILLS</h2>{formData.technicalSkills.map((skill, i) => (<div key={i} style={{ marginBottom: '3pt' }}><strong style={{ fontSize: `${fontSizes.skillText}pt`, fontWeight: 700 }}>{skill.category}:</strong><span style={{ fontSize: `${fontSizes.skillText}pt`, fontWeight: 400 }}> {skill.skills}</span></div>))}</section>}
        {hasProjects && <section className="cf-section"><h2 className="cf-section-title" style={{ fontSize: `${fontSizes.sectionTitle}pt`, fontWeight: 700 }}>ENGINEERING PROJECTS</h2>{formData.projects.map((project, i) => (<div key={i} className="cf-project-item"><h3 style={{ fontSize: `${fontSizes.jobTitle}pt`, fontWeight: 700 }}>{project.name}</h3>{project.technologies && <p style={{ fontSize: `${fontSizes.company}pt`, fontWeight: 400 }}><strong>Technologies:</strong> {project.technologies}</p>}<ul className="cf-bullet-list">{project.description.split('\n').filter(l => l.trim()).map((line, j) => <li key={j} style={{ fontSize: `${fontSizes.bulletText}pt`, fontWeight: 400 }}>{line}</li>)}</ul></div>))}</section>}
        {hasEducation && <section className="cf-section"><h2 className="cf-section-title" style={{ fontSize: `${fontSizes.sectionTitle}pt`, fontWeight: 700 }}>EDUCATION</h2>{formData.education.map((edu, i) => (<div key={i} className="cf-education-item"><h3 style={{ fontSize: `${fontSizes.degree}pt`, fontWeight: 700 }}>{edu.degree}{edu.major && `, ${edu.major}`}</h3><p className="cf-institution-text" style={{ fontSize: `${fontSizes.institution}pt`, fontWeight: 400 }}>{edu.institution} | <span className="cf-institution-date" style={{ fontSize: `${fontSizes.institutionDate}pt`, fontWeight: 400 }}>{edu.startDate} – {edu.endDate || 'Present'}{edu.gpa && ` | GPA: ${edu.gpa}`}</span></p></div>))}</section>}
        {hasCertifications && <section className="cf-section"><h2 className="cf-section-title" style={{ fontSize: `${fontSizes.sectionTitle}pt`, fontWeight: 700 }}>CERTIFICATIONS</h2>{formData.certifications.map((cert, i) => (<div key={i} className="cf-cert-item" style={{ fontSize: `${fontSizes.certificationText}pt`, fontWeight: 400 }}><strong style={{ fontWeight: 700 }}>{cert.name}</strong>{cert.issuer && ` – ${cert.issuer}`}{cert.date && ` (${cert.date})`}{cert.credentialId && ` | ID: ${cert.credentialId}`}</div>))}</section>}
        {hasPublications && <section className="cf-section"><h2 className="cf-section-title" style={{ fontSize: `${fontSizes.sectionTitle}pt`, fontWeight: 700 }}>PUBLICATIONS</h2>{formData.publications.map((pub, i) => (<div key={i} className="cf-publication-item" style={{ fontSize: `${fontSizes.regularText}pt`, fontWeight: 400 }}>"{pub.title}" – {pub.conference}{pub.year && `, ${pub.year}`}</div>))}</section>}
      ...
      <div className="cf-resume-builder" lang="en-US">
        ...
        <nav className="cf-breadcrumb" aria-label="Breadcrumb">
          <ol>
            ...
            <li><span className="cf-breadcrumb-text">Free ATS-Friendly Engineering Template</span></li>
          </ol>
        </nav>
        ...
        <section className="cf-hero">
          <div className="cf-container">
            <div className="cf-hero-content">
              <div className="cf-trust-badge"><FiStar className="cf-trust-icon"/><span className="cf-trust-text">Best Free Engineering Resume Builder 2026</span></div>
              <h1 className="cf-hero-title">Free Engineering <span className="cf-gradient-text">Resume Builder 2026</span></h1>
              <p className="cf-hero-subtitle">Create a <strong className="cf-hero-highlight">professional, ATS-optimized engineering resume for free in minutes.</strong> Our technical resume builder ensures your engineering experience and skills get noticed by tech companies and engineering firms.</p>
              <div className="cf-cta-buttons">
                <button onClick={() => setActiveSection('personal')} className="cf-btn-primary" aria-label="Start building your free engineering resume now—no sign-up required">
                  <span className="cf-btn-text">Start Building Your Engineering Resume Now</span>
                  <FiArrowRight className="cf-btn-icon" />
                  <div className="cf-btn-pulse"></div>
                </button>
                <button onClick={generatePDF} className="cf-btn-secondary" aria-label="Download engineering resume as PDF" disabled={isGeneratingPDF || !hasContent()}><FiDownload className="cf-btn-icon"/><span className="cf-btn-text">Download Engineering Resume PDF</span></button>
              </div>
              <div className="cf-hero-stats">
                <div className="cf-stat-item"><span className="cf-stat-number">5M+</span><span className="cf-stat-label">Engineers</span></div>
              ...
              <div className="cf-industry-badges">
                <div className="cf-badge-grid">
                  <span className="cf-badge-item"><FiCode/> Software Engineer</span>
                  <span className="cf-badge-item"><FiTool/> Mechanical Engineer</span>
                  <span className="cf-badge-item"><FiCpu/> Electrical Engineer</span>
                  <span className="cf-badge-item"><FiLayers/> Civil Engineer</span>
                  <span className="cf-badge-item"><FiDatabase/> Data Engineer</span>
                  <span className="cf-badge-item"><FiCloud/> Cloud Engineer</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="cf-layout">
          <div className="cf-preview-section">
            ...
            <div className={`cf-preview-container ${showFullPreview ? 'cf-full-preview' : ''}`}><div className="cf-resume-card"><div className="cf-preview-content"><div className="cf-resume-preview" ref={resumeRef}><EngTemplate formData={formData}/></div></div></div></div>
          </div>
          <div className="cf-form-section">
            <div className="cf-form-nav">
              {[{id:'personal',label:'Personal',icon:<FiUser/>},{id:'experience',label:'Experience',icon:<FiBriefcase/>},{id:'skills',label:'Tech Skills',icon:<FiCode/>},{id:'projects',label:'Projects',icon:<FiTool/>},{id:'education',label:'Education',icon:<FiBook/>},{id:'settings',label:'Font Settings',icon:<FiSettings/>}].map(item => (<button key={item.id} className={`cf-nav-btn ${activeSection===item.id?'cf-nav-btn-active':''}`} onClick={()=>setActiveSection(item.id)}>{item.icon}<span>{item.label}</span></button>))}
            </div>
            <div className="cf-form-content">
              {activeSection==='personal'&&(
                <div className="cf-form-section-content">
                  ...
                  <div className="cf-form-card">
                    <div className="cf-form-group"><label className="cf-form-label">Full Name*<input type="text" name="fullName" value={formData.fullName} onChange={handleInputChange} placeholder="John Smith, Senior Software Engineer" required className="cf-form-input"/></label><label className="cf-form-label">Professional Title<input type="text" name="title" value={formData.title} onChange={handleInputChange} placeholder="Senior Software Engineer | Full Stack Developer" className="cf-form-input"/></label></div>
                    <div className="cf-form-group"><label className="cf-form-label">Email*<input type="email" name="email" value={formData.email} onChange={handleInputChange} placeholder="john.smith@engineer.com" required className="cf-form-input"/></label><label className="cf-form-label">Phone<input type="tel" name="phone" value={formData.phone} onChange={handleInputChange} placeholder="(555) 123-4567" className="cf-form-input"/></label></div>
                    <label className="cf-form-label">Location<input type="text" name="address" value={formData.address} onChange={handleInputChange} placeholder="San Francisco, CA" className="cf-form-input"/></label>
                  </div>
                  <div className="cf-form-card"><label className="cf-form-label">Professional Engineering Summary*<textarea name="summary" value={formData.summary} onChange={handleInputChange} placeholder="Results-driven Software Engineer with 8+ years of experience in full-stack development and cloud architecture. Specialized in designing scalable microservices and leading agile development teams. Proven track record of delivering high-impact projects that improve system performance by 40% and reduce operational costs by 30%." required className="cf-form-textarea" rows="6"/><div className="cf-char-count">{formData.summary.length}/500 characters</div></label></div>
                  <div className="cf-form-card">
                    ...
                    <div className="cf-social-input">
                      <select value={currentSocialLink.platform} onChange={e=>setCurrentSocialLink({...currentSocialLink,platform:e.target.value})} className="cf-form-select"><option value="">Select Platform</option><option value="LinkedIn">LinkedIn</option><option value="GitHub">GitHub</option><option value="GitLab">GitLab</option><option value="Portfolio">Portfolio</option><option value="Website">Personal Website</option><option value="StackOverflow">Stack Overflow</option></select>
                      <input type="url" placeholder="https://github.com/yourusername" value={currentSocialLink.url} onChange={e=>setCurrentSocialLink({...currentSocialLink,url:e.target.value})} className="cf-form-input"/>
              ...
              {activeSection==='experience'&&(
                <div className="cf-form-section-content">
                  <h3 className="cf-form-section-title"><FiBriefcase/> Professional Experience</h3>
                  <p className="cf-section-desc">List your engineering positions in reverse chronological order (most recent first)</p>
                  <div className="cf-form-card">
                    <div className="cf-form-group"><label className="cf-form-label">Position Title*<input value={currentExperience.position} onChange={e=>setCurrentExperience({...currentExperience,position:e.target.value})} placeholder="Senior Software Engineer" required className="cf-form-input"/></label><label className="cf-form-label">Company*<input value={currentExperience.company} onChange={e=>setCurrentExperience({...currentExperience,company:e.target.value})} placeholder="Tech Corporation Inc." required className="cf-form-input"/></label></div>
                    <label className="cf-form-label">Technologies Used<input value={currentExperience.technologies} onChange={e=>setCurrentExperience({...currentExperience,technologies:e.target.value})} placeholder="React, Node.js, AWS, Python, Docker, Kubernetes" className="cf-form-input"/></label>
                    ...
                    <label className="cf-form-label">Key Responsibilities & Engineering Achievements*<textarea value={currentExperience.description} onChange={e=>setCurrentExperience({...currentExperience,description:e.target.value})} placeholder="• Led development of microservices architecture that improved system scalability by 200%
...
• Collaborated with cross-functional teams to deliver features on-time and within budget" required className="cf-form-textarea" rows="8"/><div className="cf-char-count">{currentExperience.description.length}/2000 characters</div></label>
                  ...
                  <div className="cf-form-card"><h4 className="cf-sub-section-title">Your Engineering Experience</h4>{formData.experience.length===0?<p className="cf-empty-msg">No experience added yet</p>:<div className="cf-items-list">{formData.experience.map((exp,i)=>(<div key={i} className="cf-list-item"><div className="cf-item-content"><div className="cf-item-header"><strong className="cf-item-title">{exp.position}</strong><span className="cf-item-subtitle">at {exp.company}</span></div><div className="cf-item-meta"><span>{exp.startDate} – {exp.endDate||'Present'}</span>{exp.technologies&&<span><strong>Tech:</strong> {exp.technologies}</span>}</div><div className="cf-item-desc">{exp.description.split('\n').filter(l=>l.trim()).map((line,j)=><p key={j} className="cf-bullet-point">• {line}</p>)}</div></div><div className="cf-item-actions"><button onClick={()=>editExperience(i)} className="cf-edit-btn" aria-label={`Edit ${exp.position} experience`}><FiEdit2/></button><button onClick={()=>deleteExperience(i)} className="cf-delete-btn" aria-label={`Delete ${exp.position} experience`}><FiTrash2/></button></div></div>))}</div>}</div>
                </div>
              )}
              {activeSection==='skills'&&(
                <div className="cf-form-section-content">
                  ...
                  <div className="cf-form-card">
                    <h4 className="cf-sub-section-title">Technical Skills by Category</h4><p className="cf-section-desc">Organize your technical skills by categories (Programming Languages, Frameworks, Tools, etc.)</p>
                    <div className="cf-skills-input">
                      <div className="cf-form-group"><label className="cf-form-label">Category*<input value={currentTechnicalSkill.category} onChange={e=>setCurrentTechnicalSkill({...currentTechnicalSkill,category:e.target.value})} placeholder="Programming Languages, Frameworks, Databases, Tools" className="cf-form-input"/></label><label className="cf-form-label">Skills*<input value={currentTechnicalSkill.skills} onChange={e=>setCurrentTechnicalSkill({...currentTechnicalSkill,skills:e.target.value})} placeholder="Python, JavaScript, Java, C++, Go" className="cf-form-input"/></label></div>
                      <div className="cf-form-actions"><button onClick={addTechnicalSkill} className="cf-add-btn" disabled={!currentTechnicalSkill.category.trim()||!currentTechnicalSkill.skills.trim()}><FiPlus/> {currentTechnicalSkill.isEditing?'Update Skills':'Add Skills Category'}</button>{currentTechnicalSkill.isEditing&&<button onClick={()=>setCurrentTechnicalSkill(defaultTechnicalSkill())} className="cf-cancel-btn"><FiX/> Cancel</button>}</div>
                    </div>
                    <div className="cf-items-list">{formData.technicalSkills.map((skill,i)=>(<div key={i} className="cf-list-item"><div><strong>{skill.category}:</strong> {skill.skills}</div><div className="cf-item-actions"><button onClick={()=>editTechnicalSkill(i)} className="cf-edit-btn"><FiEdit2/></button><button onClick={()=>deleteTechnicalSkill(i)} className="cf-delete-btn"><FiTrash2/></button></div></div>))}{formData.technicalSkills.length===0&&<p className="cf-empty-msg">No technical skills added yet</p>}</div>
                  </div>
                  <div className="cf-form-card">
                    <h4 className="cf-sub-section-title"><FiAward/> Certifications</h4><p className="cf-section-desc">Add your professional engineering certifications</p>
                    <div className="cf-skills-input">
              ...
              {activeSection==='projects'&&(
                <div className="cf-form-section-content">
                  <h3 className="cf-form-section-title"><FiTool/> Engineering Projects</h3>
                  <div className="cf-form-card">
                    <h4 className="cf-sub-section-title">Engineering Projects</h4><p className="cf-section-desc">List your significant engineering projects, including personal, academic, or professional projects</p>
                    <div className="cf-skills-input">
                      <label className="cf-form-label">Project Name*<input value={currentProject.name} onChange={e=>setCurrentProject({...currentProject,name:e.target.value})} placeholder="Real-Time Analytics Dashboard" className="cf-form-input"/></label>
                      <label className="cf-form-label">Technologies Used<input value={currentProject.technologies} onChange={e=>setCurrentProject({...currentProject,technologies:e.target.value})} placeholder="React, Node.js, MongoDB, Docker" className="cf-form-input"/></label>
                      <label className="cf-form-label">Project Description & Achievements*<textarea value={currentProject.description} onChange={e=>setCurrentProject({...currentProject,description:e.target.value})} placeholder="• Designed and developed a real-time analytics dashboard for monitoring system performance
...
• Used by 500+ internal users to make data-driven decisions" className="cf-form-textarea" rows="6"/><div className="cf-char-count">{currentProject.description.length}/1500 characters</div></label>
                    ...
                    <div className="cf-items-list">{formData.projects.map((project,i)=>(<div key={i} className="cf-list-item"><div className="cf-item-content"><div className="cf-item-header"><strong className="cf-item-title">{project.name}</strong>{project.technologies&&<span className="cf-item-subtitle"> – {project.technologies}</span>}</div><div className="cf-item-desc">{project.description.split('\n').filter(l=>l.trim()).map((line,j)=><p key={j} className="cf-bullet-point">• {line}</p>)}</div></div><div className="cf-item-actions"><button onClick={()=>editProject(i)} className="cf-edit-btn"><FiEdit2/></button><button onClick={()=>deleteProject(i)} className="cf-delete-btn"><FiTrash2/></button></div></div>))}{formData.projects.length===0&&<p className="cf-empty-msg">No projects added yet</p>}</div>
                  </div>
                  <div className="cf-form-card">
                    <h4 className="cf-sub-section-title">Publications & Research</h4><p className="cf-section-desc">Add your engineering publications, research papers, or conference presentations</p>
                    <div className="cf-skills-input">
                      <div className="cf-form-group"><label className="cf-form-label">Publication Title*<input value={currentPublication.title} onChange={e=>setCurrentPublication({...currentPublication,title:e.target.value})} placeholder="Optimizing Machine Learning Models for Edge Devices" className="cf-form-input"/></label><label className="cf-form-label">Conference/Journal<input value={currentPublication.conference} onChange={e=>setCurrentPublication({...currentPublication,conference:e.target.value})} placeholder="IEEE International Conference on Machine Learning" className="cf-form-input"/></label></div>
              ...
              {activeSection==='education'&&(
                <div className="cf-form-section-content">
                  ...
                  <p className="cf-section-desc">List your engineering education, degrees, and relevant coursework</p>
                  <div className="cf-form-card">
                    <div className="cf-form-group"><label className="cf-form-label">Institution*<input value={currentEducation.institution} onChange={e=>setCurrentEducation({...currentEducation,institution:e.target.value})} placeholder="Massachusetts Institute of Technology" required className="cf-form-input"/></label><label className="cf-form-label">Degree*<input value={currentEducation.degree} onChange={e=>setCurrentEducation({...currentEducation,degree:e.target.value})} placeholder="Master of Science in Computer Science" required className="cf-form-input"/></label></div>
                    <div className="cf-form-group"><label className="cf-form-label">Major / Specialization<input value={currentEducation.major} onChange={e=>setCurrentEducation({...currentEducation,major:e.target.value})} placeholder="Artificial Intelligence, Software Engineering" className="cf-form-input"/></label><label className="cf-form-label">GPA<input value={currentEducation.gpa} onChange={e=>setCurrentEducation({...currentEducation,gpa:e.target.value})} placeholder="3.8/4.0" className="cf-form-input"/></label></div>
                  ...
                  <div className="cf-form-card"><h4 className="cf-sub-section-title">Your Education</h4>{formData.education.length===0?<p className="cf-empty-msg">No education added yet</p>:<div className="cf-items-list">{formData.education.map((edu,i)=>(<div key={i} className="cf-list-item"><div className="cf-item-content"><div className="cf-item-header"><strong className="cf-item-title">{edu.degree}</strong>{edu.major&&<span className="cf-item-subtitle"> – {edu.major}</span>}</div><div className="cf-item-meta"><span>{edu.institution}</span><span>{edu.startDate} – {edu.endDate||'Present'}{edu.gpa&&` | GPA: ${edu.gpa}`}</span></div></div><div className="cf-item-actions"><button onClick={()=>editEducation(i)} className="cf-edit-btn" aria-label={`Edit ${edu.degree}`}><FiEdit2/></button><button onClick={()=>deleteEducation(i)} className="cf-delete-btn" aria-label={`Delete ${edu.degree}`}><FiTrash2/></button></div></div>))}</div>}</div>
                </div>
              )}
        ...
        <section className="cf-faq-section" aria-labelledby="faq-title">
          <div className="cf-container">
            <div className="cf-section-header">
              ...
              <p className="cf-section-subtitle">Everything you need to know about creating professional engineering resumes with our tool.</p>
            </div>
        ...
        <section className="cf-cta-section" aria-labelledby="cta-title">
          <div className="cf-container">
            <div className="cf-cta-content">
              <h2 className="cf-cta-title" id="cta-title">Ready to Advance Your Engineering Career?</h2>
              <p className="cf-cta-subtitle">Join 5 million+ engineering professionals who landed their dream jobs with our free ATS-friendly engineering resume builder.</p>
              <div className="cf-cta-btn-wrap">
                <button onClick={()=>setActiveSection('personal')} className="cf-cta-btn" aria-label="Create your free engineering resume now—no sign-up required">
                  <span className="cf-cta-btn-text">Create Your Free Engineering Resume Now</span>
                  <FiArrowRight className="cf-cta-btn-icon"/>
                </button>
              </div>
              <div className="cf-cta-guarantee"><FiCheck className="cf-guarantee-icon"/><span className="cf-guarantee-text">No credit card required • Free forever • Download in minutes • ATS Optimized for Engineering</span></div>
            </div>
          </div>
        </section>

        {showFullPreview&&(<div className="cf-modal" onClick={()=>setShowFullPreview(false)}><div className="cf-modal-content" onClick={e=>e.stopPropagation()}><div className="cf-modal-header"><h3>Full Engineering Resume Preview</h3><button className="cf-close-btn" onClick={()=>setShowFullPreview(false)}><FiX/></button></div><div className="cf-modal-pages"><div className="cf-modal-page"><EngTemplate formData={formData}/></div></div></div></div>)}
