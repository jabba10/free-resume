...
const careerFlowStyles = `
  ...
  * { box-sizing: border-box; }
  ...
  .cf-cert-item, .cf-affiliation-item { margin-bottom: 3pt; line-height: 1.25; }
...
const Resume = ({ seoData, buildTimestamp }) => {
  ...
  const defaultTechnical = () => ({ name: '', isEditing: false, editIndex: null });
  ...
  const defaultIndustry = () => ({ name: '', isEditing: false, editIndex: null });
  ...
  const [formData, setFormData] = useState({ fullName: '', email: '', phone: '', address: '', summary: '', experience: [], education: [], technical: [], certifications: [], affiliations: [], industries: [], languages: [], socialLinks: [] });
  ...
  const hasContent = () => formData.fullName || formData.email || formData.summary || formData.socialLinks.length > 0 || formData.experience.length > 0 || formData.education.length > 0 || formData.technical.length > 0 || formData.certifications.length > 0 || formData.affiliations.length > 0 || formData.industries.length > 0 || formData.languages.length > 0;
  ...
  const FinanceTemplate = ({ formData }) => {
    ...
    return (
      <div className="cf-universal-template">
        ...
        {hasTechnical && <section className="cf-section"><h2 className="cf-section-title" style={{ fontSize: `${fontSizes.sectionTitle}pt` }}>TECHNICAL SKILLS</h2><ul className="cf-skills-list">{formData.technical.map((s, i) => <li key={i} style={{ fontSize: `${fontSizes.skillText}pt` }}>{s.name}</li>)}</ul></section>}
        {hasCertifications && <section className="cf-section"><h2 className="cf-section-title" style={{ fontSize: `${fontSizes.sectionTitle}pt` }}>CERTIFICATIONS & LICENSES</h2>{formData.certifications.map((l, i) => (<div key={i} className="cf-cert-item" style={{ fontSize: `${fontSizes.certificationText}pt` }}><strong style={{ fontWeight: 700 }}>{l.name}</strong>{l.issuingAuthority && ` – ${l.issuingAuthority}`}{l.licenseNumber && ` (License #: ${l.licenseNumber})`}{l.expiryDate && ` – Expires: ${l.expiryDate}`}</div>))}</section>}
        {hasIndustries && <section className="cf-section"><h2 className="cf-section-title" style={{ fontSize: `${fontSizes.sectionTitle}pt` }}>INDUSTRY EXPERTISE</h2><ul className="cf-bullet-list">{formData.industries.map((p, i) => <li key={i} style={{ fontSize: `${fontSizes.bulletText}pt` }}>{p.name}</li>)}</ul></section>}
      ...
      <div className="cf-resume-builder" lang="en-US">
        ...
        <nav className="cf-breadcrumb" aria-label="Breadcrumb">
          <ol>
            ...
            <li>
              <Link href="/ats-friendly-finance-resume-builder" className="cf-breadcrumb-link" prefetch={false}>
                <span className="cf-breadcrumb-text">Free Finance Resume Builder</span>
              </Link>
            </li>
          </ol>
        </nav>
          ...
          <div className="cf-container">
            <div className="cf-hero-content">
              ...
              <p className="cf-hero-subtitle">
                Create a <strong className="cf-hero-highlight">professional, ATS-optimized finance resume for free in minutes.</strong> Our finance resume builder ensures your deal experience and financial credentials get noticed by investment banks, hedge funds, and financial institutions.
              </p>
              ...
              <div className="cf-cta-buttons">
                ...
                >
                  <span className="cf-btn-text">Start Building Your Finance Resume Now</span>
                ...
                >
                  ...
                  <span className="cf-btn-text">Download Finance Resume PDF</span>
                </button>
              </div>

              <div className="cf-industry-badges">
                <div className="cf-badge-grid">
                  <span className="cf-badge-item"><FiTrendingUp /> Investment Banking</span>
                  <span className="cf-badge-item"><FiPieChart /> Private Equity</span>
                  <span className="cf-badge-item"><FiBarChart /> Hedge Funds</span>
                  <span className="cf-badge-item"><FiDollarSign /> Corporate Finance</span>
                  <span className="cf-badge-item"><FiPercent /> Financial Analysis</span>
                  <span className="cf-badge-item"><FiCreditCard /> Asset Management</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="cf-layout">
          <div className="cf-preview-section">
            ...
            <div className={`cf-preview-container ${showFullPreview ? 'cf-full-preview' : ''}`}><div className="cf-resume-card"><div className="cf-preview-content"><div className="cf-resume-preview" ref={resumeRef}><FinanceTemplate formData={formData}/></div></div></div></div>
          </div>

          <div className="cf-form-section">
            <div className="cf-form-nav">
              {[{id:'personal',label:'Personal',icon:<FiUser/>},{id:'experience',label:'Finance Exp',icon:<FiBriefcase/>},{id:'education',label:'Education',icon:<FiBook/>},{id:'technical',label:'Tech Skills',icon:<FiTrendingUp/>},{id:'certifications',label:'Certifications',icon:<FiShield/>},{id:'settings',label:'Font Settings',icon:<FiSettings/>}].map(item => (<button key={item.id} className={`cf-nav-btn ${activeSection===item.id?'cf-nav-btn-active':''}`} onClick={()=>setActiveSection(item.id)}>{item.icon}<span>{item.label}</span></button>))}
            </div>
            ...
            <div className="cf-form-content">
              {activeSection==='personal'&&(
                <div className="cf-form-section-content">
                  ...
                  <div className="cf-form-card">
                    <div className="cf-form-group"><label className="cf-form-label">Full Name*<input type="text" name="fullName" value={formData.fullName} onChange={handleInputChange} placeholder="John Smith, CFA" required className="cf-form-input"/></label><label className="cf-form-label">Email*<input type="email" name="email" value={formData.email} onChange={handleInputChange} placeholder="john.smith@finance.com" required className="cf-form-input"/></label></div>
                    <div className="cf-form-group"><label className="cf-form-label">Phone<input type="tel" name="phone" value={formData.phone} onChange={handleInputChange} placeholder="(555) 123-4567" className="cf-form-input"/></label><label className="cf-form-label">Location<input type="text" name="address" value={formData.address} onChange={handleInputChange} placeholder="New York, NY" className="cf-form-input"/></label></div>
                  </div>
                  <div className="cf-form-card"><label className="cf-form-label">Professional Finance Summary*<textarea name="summary" value={formData.summary} onChange={handleInputChange} placeholder="Results-driven Investment Banking Analyst with 5+ years of experience in M&A, capital markets, and financial modeling. Proven track record in executing $2B+ in transactions across technology and healthcare sectors. Strong proficiency in DCF, LBO, and comparable company analysis. CFA Level III candidate with exceptional quantitative and analytical skills." required className="cf-form-textarea" rows="6"/><div className="cf-char-count">{formData.summary.length}/500 characters</div></label></div>
                  <div className="cf-form-card">
                    <h3 className="cf-sub-section-title"><FiGlobe/> Professional Links</h3><p className="cf-section-desc">Add your professional finance profiles</p>
                    <div className="cf-social-input">
                      <select value={currentSocialLink.platform} onChange={e=>setCurrentSocialLink({...currentSocialLink,platform:e.target.value})} className="cf-form-select"><option value="">Select Platform</option><option value="LinkedIn">LinkedIn</option><option value="Bloomberg">Bloomberg Terminal</option><option value="Portfolio">Professional Portfolio</option><option value="Website">Personal Website</option><option value="GitHub">GitHub</option><option value="Research">Research Publications</option></select>
              ...
              {activeSection==='experience'&&(
                <div className="cf-form-section-content">
                  <h2 className="cf-form-section-title"><FiBriefcase/> Finance Experience</h2><p className="cf-section-desc">List your finance positions in reverse chronological order</p>
                  <div className="cf-form-card">
                    <div className="cf-form-group"><label className="cf-form-label">Position Title*<input value={currentExperience.position} onChange={e=>setCurrentExperience({...currentExperience,position:e.target.value})} placeholder="Investment Banking Analyst" required className="cf-form-input"/></label><label className="cf-form-label">Financial Institution*<input value={currentExperience.employer} onChange={e=>setCurrentExperience({...currentExperience,employer:e.target.value})} placeholder="Goldman Sachs & Co." required className="cf-form-input"/></label></div>
                    <label className="cf-form-label">Division / Department / Coverage Group<input value={currentExperience.department} onChange={e=>setCurrentExperience({...currentExperience,department:e.target.value})} placeholder="Technology, Media & Telecommunications (TMT) Investment Banking" className="cf-form-input"/></label>
                    <div className="cf-form-group"><label className="cf-form-label">Start Date*<input type="text" placeholder="Month Year (e.g., June 2020)" value={currentExperience.startDate} onChange={e=>setCurrentExperience({...currentExperience,startDate:e.target.value})} required className="cf-form-input"/></label><label className="cf-form-label">End Date<input type="text" placeholder="Month Year or Present" value={currentExperience.endDate} onChange={e=>setCurrentExperience({...currentExperience,endDate:e.target.value})} className="cf-form-input"/></label></div>
                    <label className="cf-form-label">Key Financial Responsibilities & Deal Experience*<textarea value={currentExperience.description} onChange={e=>setCurrentExperience({...currentExperience,description:e.target.value})} placeholder="• Executed $2.5B merger between TechCorp and DataSystems, creating synergy models with 15% cost savings
...
• Mentored 3 summer analysts in financial modeling and valuation techniques" required className="cf-form-textarea" rows="8"/><div className="cf-char-count">{currentExperience.description.length}/2000 characters</div></label>
              ...
              {activeSection==='education'&&(
                <div className="cf-form-section-content">
                  <h2 className="cf-form-section-title"><FiBook/> Education & Qualifications</h2><p className="cf-section-desc">List your education, degrees, and relevant finance qualifications</p>
                  <div className="cf-form-card">
                    <div className="cf-form-group"><label className="cf-form-label">Institution*<input value={currentEducation.institution} onChange={e=>setCurrentEducation({...currentEducation,institution:e.target.value})} placeholder="Wharton School, University of Pennsylvania" required className="cf-form-input"/></label><label className="cf-form-label">Degree*<input value={currentEducation.degree} onChange={e=>setCurrentEducation({...currentEducation,degree:e.target.value})} placeholder="Bachelor of Science in Economics" required className="cf-form-input"/></label></div>
                    <label className="cf-form-label">Program / Concentration / Major<input value={currentEducation.program} onChange={e=>setCurrentEducation({...currentEducation,program:e.target.value})} placeholder="Finance, Financial Engineering" className="cf-form-input"/></label>
                    ...
                    <label className="cf-form-label">GPA / Honors<input value={currentEducation.gpa} onChange={e=>setCurrentEducation({...currentEducation,gpa:e.target.value})} placeholder="3.8/4.0, Magna Cum Laude" className="cf-form-input"/></label>
              ...
              {activeSection==='technical'&&(
                <div className="cf-form-section-content">
                  <h2 className="cf-form-section-title"><FiTrendingUp/> Technical Skills & Certifications</h2>
                  <div className="cf-form-card">
                    <h3 className="cf-sub-section-title">Technical Finance Skills</h3><p className="cf-section-desc">List your technical skills and financial software proficiency</p>
                    <div className="cf-skills-input"><input value={currentTechnical.name} onChange={e=>setCurrentTechnical({...currentTechnical,name:e.target.value})} placeholder="Financial Modeling (DCF, LBO, M&A), Bloomberg Terminal, VBA, Python, SQL" className="cf-form-input"/><div className="cf-form-actions"><button onClick={addTechnical} className="cf-add-btn" disabled={!currentTechnical.name.trim()}><FiPlus/> {currentTechnical.isEditing?'Update':'Add Skill'}</button>{currentTechnical.isEditing&&<button onClick={()=>setCurrentTechnical(defaultTechnical())} className="cf-cancel-btn"><FiX/> Cancel</button>}</div></div>
                    <div className="cf-items-list">{formData.technical.map((s,i)=>(<div key={i} className="cf-list-item"><span>{s.name}</span><div className="cf-item-actions"><button onClick={()=>editTechnical(i)} className="cf-edit-btn"><FiEdit2/></button><button onClick={()=>deleteTechnical(i)} className="cf-delete-btn"><FiTrash2/></button></div></div>))}{formData.technical.length===0&&<p className="cf-empty-msg">No technical skills added yet</p>}</div>
                  </div>
                  <div className="cf-form-card">
                    <h3 className="cf-sub-section-title"><FiShield/> Finance Certifications & Licenses</h3><p className="cf-section-desc">Add your professional finance certifications and licenses</p>
                    <div className="cf-skills-input">
                      <div className="cf-form-group"><label className="cf-form-label">Certification/License Name*<input value={currentCertification.name} onChange={e=>setCurrentCertification({...currentCertification,name:e.target.value})} placeholder="Chartered Financial Analyst (CFA)" className="cf-form-input"/></label><label className="cf-form-label">Issuing Authority<input value={currentCertification.issuingAuthority} onChange={e=>setCurrentCertification({...currentCertification,issuingAuthority:e.target.value})} placeholder="CFA Institute" className="cf-form-input"/></label></div>
                      <div className="cf-form-group"><label className="cf-form-label">License Number<input value={currentCertification.licenseNumber} onChange={e=>setCurrentCertification({...currentCertification,licenseNumber:e.target.value})} placeholder="CFA1234567" className="cf-form-input"/></label><label className="cf-form-label">Expiry Date<input value={currentCertification.expiryDate} onChange={e=>setCurrentCertification({...currentCertification,expiryDate:e.target.value})} placeholder="Month Year" className="cf-form-input"/></label></div>
                    ...
                    <div className="cf-items-list">{formData.certifications.map((l,i)=>(<div key={i} className="cf-list-item"><div><strong>{l.name}</strong>{l.issuingAuthority&&` – ${l.issuingAuthority}`}{l.licenseNumber&&` (#${l.licenseNumber})`}{l.expiryDate&&` – Expires: ${l.expiryDate}`}</div><div className="cf-item-actions"><button onClick={()=>editCertification(i)} className="cf-edit-btn"><FiEdit2/></button><button onClick={()=>deleteCertification(i)} className="cf-delete-btn"><FiTrash2/></button></div></div>))}{formData.certifications.length===0&&<p className="cf-empty-msg">No certifications added yet</p>}</div>
                  </div>
                  <div className="cf-form-card">
                    <h3 className="cf-sub-section-title">Professional Affiliations</h3><p className="cf-section-desc">Add your professional finance organization memberships</p>
                    <div className="cf-skills-input">
                      <div className="cf-form-group"><label className="cf-form-label">Organization*<input value={currentAffiliation.organization} onChange={e=>setCurrentAffiliation({...currentAffiliation,organization:e.target.value})} placeholder="CFA Institute, Financial Analysts Federation" className="cf-form-input"/></label><label className="cf-form-label">Role / Membership Type<input value={currentAffiliation.role} onChange={e=>setCurrentAffiliation({...currentAffiliation,role:e.target.value})} placeholder="Member, Committee Chair" className="cf-form-input"/></label></div>
              ...
              {activeSection==='settings'&&(
                <div className="cf-form-section-content">
                  <h2 className="cf-form-section-title"><FiSettings/> Font Size Settings</h2><p className="cf-section-desc">Customize font sizes for your resume PDF. All sizes in points (pt).</p>
                  <div className="cf-form-card">
