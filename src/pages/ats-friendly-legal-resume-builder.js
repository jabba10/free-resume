...
const careerFlowStyles = `
  ...
  * { box-sizing: border-box; }
  ...
  .cf-legal-template { color: #000000; line-height: 1.25; height: 100%; }
  ...
  .cf-license-item, .cf-court-item, .cf-affiliation-item, .cf-pub-item { margin-bottom: 3pt; line-height: 1.25; }
  ...
  .cf-modal-page .cf-legal-template { width: 100%; height: 100%; padding: 10mm 15mm; box-sizing: border-box; overflow-y: auto; }
  ...
  @media (max-width: 768px) {
    ...
    .cf-modal-page .cf-legal-template { padding: 8mm 12mm; }
  ...
  @media print {
    ...
    .cf-legal-template, .cf-section { page-break-inside: avoid; }
  }
`;

const LegalResume = ({ seoData, buildTimestamp }) => {
  ...
  const defaultLicense = () => ({ jurisdiction: '', barNumber: '', admissionDate: '', status: 'Active', isEditing: false, editIndex: null });
  const defaultAffiliation = () => ({ organization: '', role: '', isEditing: false, editIndex: null });
  const defaultPublication = () => ({ title: '', publication: '', year: '', isEditing: false, editIndex: null });
  ...
  const defaultCourt = () => ({ name: '', jurisdiction: '', isEditing: false, editIndex: null });

  const [formData, setFormData] = useState({ fullName: '', email: '', phone: '', address: '', summary: '', experience: [], education: [], specialties: [], licenses: [], affiliations: [], publications: [], languages: [], socialLinks: [], courts: [] });
  ...
  const hasContent = () => formData.fullName || formData.email || formData.summary || formData.socialLinks.length > 0 || formData.experience.length > 0 || formData.education.length > 0 || formData.specialties.length > 0 || formData.licenses.length > 0 || formData.affiliations.length > 0 || formData.publications.length > 0 || formData.languages.length > 0 || formData.courts.length > 0;
  ...
  const LegalTemplate = ({ formData }) => {
    ...
    return (
      <div className="cf-legal-template">
        ...
        {hasExperience && <section className="cf-section"><h2 className="cf-section-title" style={{ fontSize: `${fontSizes.sectionTitle}pt`, fontWeight: 700 }}>LEGAL EXPERIENCE</h2>{formData.experience.map((exp, i) => (<div key={i} className="cf-experience-item"><div className="cf-experience-header"><h3 style={{ fontSize: `${fontSizes.jobTitle}pt`, fontWeight: 700 }}>{exp.position}</h3><p className="cf-company" style={{ fontSize: `${fontSizes.company}pt`, fontWeight: 400 }}>{exp.employer}{exp.practiceArea && ` – ${exp.practiceArea}`} | {exp.startDate} – {exp.endDate || 'Present'}</p></div><ul className="cf-bullet-list">{exp.description.split('\n').filter(l => l.trim()).map((line, j) => <li key={j} style={{ fontSize: `${fontSizes.bulletText}pt`, fontWeight: 400 }}>{line}</li>)}</ul></div>))}</section>}
        {hasEducation && <section className="cf-section"><h2 className="cf-section-title" style={{ fontSize: `${fontSizes.sectionTitle}pt`, fontWeight: 700 }}>EDUCATION</h2>{formData.education.map((edu, i) => (<div key={i} className="cf-education-item"><h3 style={{ fontSize: `${fontSizes.degree}pt`, fontWeight: 700 }}>{edu.degree}{edu.honors && `, ${edu.honors}`}</h3><p className="cf-institution-text" style={{ fontSize: `${fontSizes.institution}pt`, fontWeight: 400 }}>{edu.institution} | <span className="cf-institution-date" style={{ fontSize: `${fontSizes.institutionDate}pt`, fontWeight: 400 }}>{edu.startDate} – {edu.endDate || 'Expected'}</span></p></div>))}</section>}
        {hasLicenses && <section className="cf-section"><h2 className="cf-section-title" style={{ fontSize: `${fontSizes.sectionTitle}pt`, fontWeight: 700 }}>BAR ADMISSIONS & CREDENTIALS</h2>{formData.licenses.map((l, i) => (<div key={i} className="cf-license-item" style={{ fontSize: `${fontSizes.licenseText}pt`, fontWeight: 400 }}><strong style={{ fontWeight: 700 }}>{l.jurisdiction}</strong>{l.barNumber && ` (Bar #: ${l.barNumber})`}{l.admissionDate && ` – Admitted: ${l.admissionDate}`}{l.status && ` – Status: ${l.status}`}</div>))}</section>}
        {hasCourts && <section className="cf-section"><h2 className="cf-section-title" style={{ fontSize: `${fontSizes.sectionTitle}pt`, fontWeight: 700 }}>COURT ADMISSIONS</h2>{formData.courts.map((c, i) => (<div key={i} className="cf-court-item" style={{ fontSize: `${fontSizes.regularText}pt`, fontWeight: 400 }}>{c.name}{c.jurisdiction && ` – ${c.jurisdiction}`}</div>))}</section>}
        {hasSpecialties && <section className="cf-section"><h2 className="cf-section-title" style={{ fontSize: `${fontSizes.sectionTitle}pt`, fontWeight: 700 }}>PRACTICE AREAS</h2><ul className="cf-skills-list">{formData.specialties.map((s, i) => <li key={i} style={{ fontSize: `${fontSizes.skillText}pt`, fontWeight: 400 }}>{s.name}</li>)}</ul></section>}
        {hasPublications && <section className="cf-section"><h2 className="cf-section-title" style={{ fontSize: `${fontSizes.sectionTitle}pt`, fontWeight: 700 }}>PUBLICATIONS & SPEAKING ENGAGEMENTS</h2>{formData.publications.map((p, i) => (<div key={i} className="cf-pub-item" style={{ fontSize: `${fontSizes.regularText}pt`, fontWeight: 400 }}><strong style={{ fontWeight: 700 }}>"{p.title}"</strong>{p.publication && ` – ${p.publication}`}{p.year && ` (${p.year})`}</div>))}</section>}
      ...
      <div className="cf-resume-builder" lang="en-US">
          ...
          <ol>
            <li>
              <Link href="/" className="cf-breadcrumb-link" prefetch={false}>
                ...
                <span className="cf-breadcrumb-text">Legal Resume Builder</span>
              </Link>
            </li>
            <li className="cf-breadcrumb-separator"><FiChevronRightIcon/></li>
            <li>
              <Link href="/ats-friendly-legal-resume-builder" className="cf-breadcrumb-link" prefetch={false}>
                <span className="cf-breadcrumb-text">Free Legal Templates</span>
              </Link>
            </li>
          </ol>
        </nav>
          ...
          <div className="cf-container">
            <div className="cf-hero-content">
              ...
              <p className="cf-hero-subtitle">
                Create a <strong className="cf-hero-highlight">professional, ATS-optimized legal resume for free in minutes.</strong> Our legal resume builder ensures your experience and credentials get noticed by top law firms and legal employers globally.
              </p>
              <div className="cf-cta-buttons">
                ...
                >
                  <span className="cf-btn-text">Start Building Your Legal Resume Now</span>
                  <FiArrowRight className="cf-btn-icon" />
                  <div className="cf-btn-pulse"></div>
                </button>
                <button onClick={generatePDF} className="cf-btn-secondary" disabled={isGeneratingPDF || !hasContent()} aria-label="Download legal resume as PDF">
                  <FiDownload className="cf-btn-icon"/>
                  <span className="cf-btn-text">Download Legal Resume PDF</span>
                </button>
              </div>
              <div className="cf-hero-stats">
                <div className="cf-stat-item"><span className="cf-stat-number">500K+</span><span className="cf-stat-label">Legal Professionals</span></div>
              ...
              <div className="cf-industry-badges">
                <div className="cf-badge-grid">
                  <span className="cf-badge-item">Corporate Law</span>
                  <span className="cf-badge-item">Litigation</span>
                  <span className="cf-badge-item">International Law</span>
                  <span className="cf-badge-item">IP Law</span>
                  <span className="cf-badge-item">Compliance</span>
                  <span className="cf-badge-item">Maritime Law</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="cf-layout">
          <div className="cf-preview-section">
            ...
            <div className={`cf-preview-container ${showFullPreview ? 'cf-full-preview' : ''}`}><div className="cf-resume-card"><div className="cf-preview-content"><div className="cf-resume-preview" ref={resumeRef}><LegalTemplate formData={formData}/></div></div></div></div>
          </div>
          <div className="cf-form-section">
            <div className="cf-form-nav">
              {[{id:'personal',label:'Personal',icon:<FiUser/>},{id:'experience',label:'Legal Experience',icon:<FiBriefcase/>},{id:'education',label:'Education',icon:<FiBook/>},{id:'credentials',label:'Credentials',icon:<FiShield/>},{id:'specialties',label:'Practice Areas',icon:<FiTarget/>},{id:'settings',label:'Font Settings',icon:<FiSettings/>}].map(item => (<button key={item.id} className={`cf-nav-btn ${activeSection===item.id?'cf-nav-btn-active':''}`} onClick={()=>setActiveSection(item.id)}>{item.icon}<span>{item.label}</span></button>))}
            </div>
            <div className="cf-form-content">
              {activeSection==='personal'&&(
                <div className="cf-form-section-content">
                  ...
                  <div className="cf-form-card">
                    <div className="cf-form-group"><label className="cf-form-label">Full Name*<input type="text" name="fullName" value={formData.fullName} onChange={handleInputChange} placeholder="John Smith, Esq., LL.M." required className="cf-form-input"/></label><label className="cf-form-label">Email*<input type="email" name="email" value={formData.email} onChange={handleInputChange} placeholder="john.smith@lawfirm.com" required className="cf-form-input"/></label></div>
                    <div className="cf-form-group"><label className="cf-form-label">Phone<input type="tel" name="phone" value={formData.phone} onChange={handleInputChange} placeholder="(555) 123-4567" className="cf-form-input"/></label><label className="cf-form-label">Location<input type="text" name="address" value={formData.address} onChange={handleInputChange} placeholder="City, State/Province, Country" className="cf-form-input"/></label></div>
                  </div>
                  <div className="cf-form-card"><label className="cf-form-label">Professional Legal Profile*<textarea name="summary" value={formData.summary} onChange={handleInputChange} placeholder="Accomplished corporate attorney with 10+ years of experience in M&A, securities regulation, and international transactions..." required className="cf-form-textarea" rows="6"/><div className="cf-char-count">{formData.summary.length}/500 characters</div></label></div>
                  <div className="cf-form-card">
                    <h4 className="cf-sub-section-title"><FiGlobe/> Professional Links</h4><p className="cf-section-desc">Add your professional legal profiles</p>
                    <div className="cf-social-input">
                      <select value={currentSocialLink.platform} onChange={e=>setCurrentSocialLink({...currentSocialLink,platform:e.target.value})} className="cf-form-select"><option value="">Select Platform</option><option value="LinkedIn">LinkedIn</option><option value="Avvo">Avvo</option><option value="Martindale">Martindale-Hubbell</option><option value="Portfolio">Professional Portfolio</option><option value="Website">Law Firm Website</option><option value="SSRN">SSRN Publications</option></select>
              ...
              {activeSection==='experience'&&(
                <div className="cf-form-section-content">
                  <h3 className="cf-form-section-title"><FiBriefcase/> Legal Experience</h3>
                  <p className="cf-section-desc">List your legal positions in reverse chronological order (most recent first)</p>
                  <div className="cf-form-card">
                    <div className="cf-form-group"><label className="cf-form-label">Position Title*<input value={currentExperience.position} onChange={e=>setCurrentExperience({...currentExperience,position:e.target.value})} placeholder="Senior Associate, Corporate Law" required className="cf-form-input"/></label><label className="cf-form-label">Law Firm/Organization*<input value={currentExperience.employer} onChange={e=>setCurrentExperience({...currentExperience,employer:e.target.value})} placeholder="Doe & Partners LLP" required className="cf-form-input"/></label></div>
                    <label className="cf-form-label">Practice Area / Department<input value={currentExperience.practiceArea} onChange={e=>setCurrentExperience({...currentExperience,practiceArea:e.target.value})} placeholder="Mergers & Acquisitions / International Transactions" className="cf-form-input"/></label>
                    ...
                    <label className="cf-form-label">Key Legal Responsibilities & Achievements*<textarea value={currentExperience.description} onChange={e=>setCurrentExperience({...currentExperience,description:e.target.value})} placeholder="• Led due diligence for cross-border M&A transactions valued at $50M-$500M..." required className="cf-form-textarea" rows="8"/><div className="cf-char-count">{currentExperience.description.length}/2000 characters</div></label>
                    <div className="cf-form-actions"><button onClick={addExperience} className="cf-add-btn" disabled={!currentExperience.position||!currentExperience.employer||!currentExperience.startDate}><FiPlus/> {currentExperience.isEditing?'Update Legal Experience':'Add Legal Experience'}</button>{currentExperience.isEditing&&<button onClick={()=>setCurrentExperience(defaultExperience())} className="cf-cancel-btn"><FiX/> Cancel</button>}</div>
                  </div>
                  <div className="cf-form-card"><h4 className="cf-sub-section-title">Your Legal Experience</h4>{formData.experience.length===0?<p className="cf-empty-msg">No legal experience added yet</p>:<div className="cf-items-list">{formData.experience.map((exp,i)=>(<div key={i} className="cf-list-item"><div className="cf-item-content"><div className="cf-item-header"><strong className="cf-item-title">{exp.position}</strong><span className="cf-item-subtitle">at {exp.employer}</span></div><div className="cf-item-meta"><span>{exp.startDate} – {exp.endDate||'Present'}</span>{exp.practiceArea&&<span>{exp.practiceArea}</span>}</div><div className="cf-item-desc">{exp.description.split('\n').filter(l=>l.trim()).map((line,j)=><p key={j} className="cf-bullet-point">• {line}</p>)}</div></div><div className="cf-item-actions"><button onClick={()=>editExperience(i)} className="cf-edit-btn" aria-label={`Edit ${exp.position} experience`}><FiEdit2/></button><button onClick={()=>deleteExperience(i)} className="cf-delete-btn" aria-label={`Delete ${exp.position} experience`}><FiTrash2/></button></div></div>))}</div>}</div>
                </div>
              )}
              {activeSection==='education'&&(
                <div className="cf-form-section-content">
                  <h3 className="cf-form-section-title"><FiBook/> Legal Education</h3>
                  <p className="cf-section-desc">List your law degrees, certifications, and relevant legal training</p>
                  <div className="cf-form-card">
                    <div className="cf-form-group"><label className="cf-form-label">Institution*<input value={currentEducation.institution} onChange={e=>setCurrentEducation({...currentEducation,institution:e.target.value})} placeholder="Harvard Law School" required className="cf-form-input"/></label><label className="cf-form-label">Degree*<input value={currentEducation.degree} onChange={e=>setCurrentEducation({...currentEducation,degree:e.target.value})} placeholder="Juris Doctor (J.D.)" required className="cf-form-input"/></label></div>
                    <label className="cf-form-label">Honors / Awards / Concentration<input value={currentEducation.honors} onChange={e=>setCurrentEducation({...currentEducation,honors:e.target.value})} placeholder="Cum Laude, Law Review Editor-in-Chief" className="cf-form-input"/></label>
                  ...
                  <div className="cf-form-card"><h4 className="cf-sub-section-title">Your Education</h4>{formData.education.length===0?<p className="cf-empty-msg">No education added yet</p>:<div className="cf-items-list">{formData.education.map((edu,i)=>(<div key={i} className="cf-list-item"><div className="cf-item-content"><div className="cf-item-header"><strong className="cf-item-title">{edu.degree}</strong>{edu.honors&&<span className="cf-item-subtitle"> – {edu.honors}</span>}</div><div className="cf-item-meta"><span>{edu.institution}</span><span>{edu.startDate} – {edu.endDate||'Present'}</span></div></div><div className="cf-item-actions"><button onClick={()=>editEducation(i)} className="cf-edit-btn" aria-label={`Edit ${edu.degree}`}><FiEdit2/></button><button onClick={()=>deleteEducation(i)} className="cf-delete-btn" aria-label={`Delete ${edu.degree}`}><FiTrash2/></button></div></div>))}</div>}</div>
                </div>
              )}
              {activeSection==='credentials'&&(
                <div className="cf-form-section-content">
                  <h3 className="cf-form-section-title"><FiShield/> Bar Admissions & Credentials</h3>
                  <div className="cf-form-card">
                    <h4 className="cf-sub-section-title">Bar Admissions</h4><p className="cf-section-desc">Add your bar admissions by jurisdiction</p>
                    <div className="cf-skills-input">
                      <div className="cf-form-group"><label className="cf-form-label">Jurisdiction*<input value={currentLicense.jurisdiction} onChange={e=>setCurrentLicense({...currentLicense,jurisdiction:e.target.value})} placeholder="State of New York" className="cf-form-input"/></label><label className="cf-form-label">Bar Number<input value={currentLicense.barNumber} onChange={e=>setCurrentLicense({...currentLicense,barNumber:e.target.value})} placeholder="1234567" className="cf-form-input"/></label></div>
                      <div className="cf-form-group"><label className="cf-form-label">Admission Date<input value={currentLicense.admissionDate} onChange={e=>setCurrentLicense({...currentLicense,admissionDate:e.target.value})} placeholder="Month Year" className="cf-form-input"/></label><label className="cf-form-label">Status<select value={currentLicense.status} onChange={e=>setCurrentLicense({...currentLicense,status:e.target.value})} className="cf-form-select"><option value="Active">Active</option><option value="Inactive">Inactive</option><option value="Suspended">Suspended</option><option value="Retired">Retired</option></select></label></div>
                      <div className="cf-form-actions"><button onClick={addLicense} className="cf-add-btn" disabled={!currentLicense.jurisdiction.trim()}><FiPlus/> {currentLicense.isEditing?'Update Admission':'Add Admission'}</button>{currentLicense.isEditing&&<button onClick={()=>setCurrentLicense(defaultLicense())} className="cf-cancel-btn"><FiX/> Cancel</button>}</div>
                    </div>
                    <div className="cf-items-list">{formData.licenses.map((l,i)=>(<div key={i} className="cf-list-item"><div><strong>{l.jurisdiction}</strong>{l.barNumber&&` (Bar #${l.barNumber})`}{l.admissionDate&&` – Admitted: ${l.admissionDate}`}{l.status&&` – Status: ${l.status}`}</div><div className="cf-item-actions"><button onClick={()=>editLicense(i)} className="cf-edit-btn"><FiEdit2/></button><button onClick={()=>deleteLicense(i)} className="cf-delete-btn"><FiTrash2/></button></div></div>))}{formData.licenses.length===0&&<p className="cf-empty-msg">No bar admissions added yet</p>}</div>
                  </div>
                  <div className="cf-form-card">
                    <h4 className="cf-sub-section-title">Court Admissions</h4><p className="cf-section-desc">Add court admissions and tribunal memberships</p>
                    <div className="cf-skills-input">
                      <div className="cf-form-group"><label className="cf-form-label">Court/Tribunal Name*<input value={currentCourt.name} onChange={e=>setCurrentCourt({...currentCourt,name:e.target.value})} placeholder="U.S. Supreme Court" className="cf-form-input"/></label><label className="cf-form-label">Jurisdiction<input value={currentCourt.jurisdiction} onChange={e=>setCurrentCourt({...currentCourt,jurisdiction:e.target.value})} placeholder="United States" className="cf-form-input"/></label></div>
                      <div className="cf-form-actions"><button onClick={addCourt} className="cf-add-btn" disabled={!currentCourt.name.trim()}><FiPlus/> {currentCourt.isEditing?'Update Court':'Add Court'}</button>{currentCourt.isEditing&&<button onClick={()=>setCurrentCourt(defaultCourt())} className="cf-cancel-btn"><FiX/> Cancel</button>}</div>
                    </div>
                    <div className="cf-items-list">{formData.courts.map((c,i)=>(<div key={i} className="cf-list-item"><div>{c.name}{c.jurisdiction&&` – ${c.jurisdiction}`}</div><div className="cf-item-actions"><button onClick={()=>editCourt(i)} className="cf-edit-btn"><FiEdit2/></button><button onClick={()=>deleteCourt(i)} className="cf-delete-btn"><FiTrash2/></button></div></div>))}{formData.courts.length===0&&<p className="cf-empty-msg">No court admissions added yet</p>}</div>
                  </div>
                  <div className="cf-form-card">
                    <h4 className="cf-sub-section-title">Languages</h4><p className="cf-section-desc">List languages you speak (important for international law)</p>
                    <div className="cf-skills-input">
                      <div className="cf-form-group"><label className="cf-form-label">Language*<input value={currentLanguage.name} onChange={e=>setCurrentLanguage({...currentLanguage,name:e.target.value})} placeholder="Spanish" className="cf-form-input"/></label><label className="cf-form-label">Proficiency Level<select value={currentLanguage.proficiency} onChange={e=>setCurrentLanguage({...currentLanguage,proficiency:e.target.value})} className="cf-form-select"><option value="">Select Proficiency</option><option value="Native">Native</option><option value="Fluent">Fluent</option><option value="Professional">Professional Working</option><option value="Limited">Limited Working</option><option value="Elementary">Elementary</option></select></label></div>
              ...
              {activeSection==='specialties'&&(
                <div className="cf-form-section-content">
                  <h3 className="cf-form-section-title"><FiTarget/> Practice Areas & Publications</h3>
                  <div className="cf-form-card">
                    <h4 className="cf-sub-section-title">Practice Areas & Specializations</h4><p className="cf-section-desc">List your legal practice areas and specializations</p>
                    <div className="cf-skills-input">
                      <input value={currentSpecialty.name} onChange={e=>setCurrentSpecialty({...currentSpecialty,name:e.target.value})} placeholder="Mergers & Acquisitions, International Arbitration" className="cf-form-input"/>
                      <div className="cf-form-actions"><button onClick={addSpecialty} className="cf-add-btn" disabled={!currentSpecialty.name.trim()}><FiPlus/> {currentSpecialty.isEditing?'Update Practice Area':'Add Practice Area'}</button>{currentSpecialty.isEditing&&<button onClick={()=>setCurrentSpecialty(defaultSpecialty())} className="cf-cancel-btn"><FiX/> Cancel</button>}</div>
                    </div>
                    <div className="cf-items-list">{formData.specialties.map((s,i)=>(<div key={i} className="cf-list-item"><span>{s.name}</span><div className="cf-item-actions"><button onClick={()=>editSpecialty(i)} className="cf-edit-btn"><FiEdit2/></button><button onClick={()=>deleteSpecialty(i)} className="cf-delete-btn"><FiTrash2/></button></div></div>))}{formData.specialties.length===0&&<p className="cf-empty-msg">No practice areas added yet</p>}</div>
                  </div>
                  <div className="cf-form-card">
                    <h4 className="cf-sub-section-title"><FiBookOpen/> Publications & Speaking Engagements</h4><p className="cf-section-desc">Add your legal publications, articles, and speaking engagements</p>
                    <div className="cf-skills-input">
                      <div className="cf-form-group"><label className="cf-form-label">Title*<input value={currentPublication.title} onChange={e=>setCurrentPublication({...currentPublication,title:e.target.value})} placeholder="'Cross-Border M&A in the Digital Age'" className="cf-form-input"/></label><label className="cf-form-label">Publication/Venue<input value={currentPublication.publication} onChange={e=>setCurrentPublication({...currentPublication,publication:e.target.value})} placeholder="Harvard Law Review" className="cf-form-input"/></label></div>
                    ...
                    <div className="cf-items-list">{formData.publications.map((p,i)=>(<div key={i} className="cf-list-item"><div><strong>"{p.title}"</strong>{p.publication&&` – ${p.publication}`}{p.year&&` (${p.year})`}</div><div className="cf-item-actions"><button onClick={()=>editPublication(i)} className="cf-edit-btn"><FiEdit2/></button><button onClick={()=>deletePublication(i)} className="cf-delete-btn"><FiTrash2/></button></div></div>))}{formData.publications.length===0&&<p className="cf-empty-msg">No publications added yet</p>}</div>
                  </div>
                  <div className="cf-form-card">
                    <h4 className="cf-sub-section-title">Professional Affiliations</h4><p className="cf-section-desc">Add your legal professional organization memberships</p>
                    <div className="cf-skills-input">
                      <div className="cf-form-group"><label className="cf-form-label">Organization*<input value={currentAffiliation.organization} onChange={e=>setCurrentAffiliation({...currentAffiliation,organization:e.target.value})} placeholder="American Bar Association (ABA)" className="cf-form-input"/></label><label className="cf-form-label">Role / Committee<input value={currentAffiliation.role} onChange={e=>setCurrentAffiliation({...currentAffiliation,role:e.target.value})} placeholder="Member, International Law Section" className="cf-form-input"/></label></div>
              ...
              {activeSection==='settings'&&(
                <div className="cf-form-section-content">
                  ...
                  <p className="cf-section-desc">Customize font sizes for your legal resume PDF. All sizes are in points (pt).</p>
                  <div className="cf-form-card">
        ...
        <section className="cf-faq-section" aria-labelledby="faq-title">
          <div className="cf-container">
            <div className="cf-section-header">
              ...
              <p>Everything you need to know about creating professional legal resumes with our tool.</p>
            </div>
        ...
        <section className="cf-cta-section" aria-labelledby="cta-title">
          <div className="cf-container">
            <div className="cf-cta-content">
              <h2 className="cf-cta-title" id="cta-title">Ready to Advance Your Legal Career?</h2>
              <p className="cf-cta-subtitle">Join 500,000+ legal professionals who landed positions at top law firms worldwide with our free ATS-friendly legal resume builder.</p>
              <div className="cf-cta-btn-wrap">
                ...
                >
                  <span className="cf-cta-btn-text">Create Your Free Legal Resume Now</span>
        ...
        {showFullPreview&&(<div className="cf-modal" onClick={()=>setShowFullPreview(false)}><div className="cf-modal-content" onClick={e=>e.stopPropagation()}><div className="cf-modal-header"><h3>Full Legal Resume Preview</h3><button className="cf-close-btn" onClick={()=>setShowFullPreview(false)}><FiX/></button></div><div className="cf-modal-pages"><div className="cf-modal-page"><LegalTemplate formData={formData}/></div></div></div></div>)}
