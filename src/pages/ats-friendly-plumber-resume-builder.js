...
const careerFlowStyles = `
  ...
  * { box-sizing: border-box; }
  ...
  .cf-plumber-template { color: #000000; line-height: 1.25; height: 100%; }
  ...
  .cf-license-item, .cf-cert-item { margin-bottom: 3pt; line-height: 1.25; }
  ...
  .cf-modal-page .cf-plumber-template { width: 100%; height: 100%; padding: 10mm 15mm; box-sizing: border-box; overflow-y: auto; }
  ...
  @media (max-width: 768px) {
    ...
    .cf-modal-page .cf-plumber-template { padding: 8mm 12mm; }
  ...
  @media print {
    ...
    .cf-plumber-template, .cf-section { page-break-inside: avoid; }
  }
`;

const PlumberResume = ({ seoData, buildTimestamp }) => {
  const {
    currentDate,
    lastModifiedDate,
    reviewDates,
  ...
  const defaultCertification = () => ({ organization: '', certification: '', isEditing: false, editIndex: null });
  const defaultSkill = () => ({ name: '', isEditing: false, editIndex: null });
  ...
  const [formData, setFormData] = useState({ fullName: '', email: '', phone: '', address: '', summary: '', experience: [], education: [], specialties: [], licenses: [], certifications: [], skills: [], languages: [], socialLinks: [] });
  ...
  const hasContent = () => formData.fullName || formData.email || formData.summary || formData.socialLinks.length > 0 || formData.experience.length > 0 || formData.education.length > 0 || formData.specialties.length > 0 || formData.licenses.length > 0 || formData.certifications.length > 0 || formData.skills.length > 0 || formData.languages.length > 0;
  ...
  const PlumberTemplate = ({ formData }) => {
    ...
    return (
      <div className="cf-plumber-template">
        ...
        {hasSkills && <section className="cf-section"><h2 className="cf-section-title" style={{ fontSize: `${fontSizes.sectionTitle}pt`, fontWeight: 700 }}>TECHNICAL SKILLS</h2><ul className="cf-skills-list">{formData.skills.map((s, i) => <li key={i} style={{ fontSize: `${fontSizes.skillText}pt`, fontWeight: 400 }}>{s.name}</li>)}</ul></section>}
        {(hasLicenses || hasCertifications) && <section className="cf-section"><h2 className="cf-section-title" style={{ fontSize: `${fontSizes.sectionTitle}pt`, fontWeight: 700 }}>LICENSES & CERTIFICATIONS</h2>{formData.licenses.map((l, i) => (<div key={i} className="cf-license-item" style={{ fontSize: `${fontSizes.licenseText}pt`, fontWeight: 400 }}><strong style={{ fontWeight: 700 }}>{l.name}</strong>{l.issuingAuthority && ` – ${l.issuingAuthority}`}{l.licenseNumber && ` (License #: ${l.licenseNumber})`}{l.expiryDate && ` – Expires: ${l.expiryDate}`}</div>))}{formData.certifications.map((c, i) => (<div key={`cert-${i}`} className="cf-cert-item" style={{ fontSize: `${fontSizes.licenseText}pt`, fontWeight: 400 }}><strong style={{ fontWeight: 700 }}>{c.certification}</strong>{c.organization && ` – ${c.organization}`}</div>))}</section>}
        {hasSpecialties && <section className="cf-section"><h2 className="cf-section-title" style={{ fontSize: `${fontSizes.sectionTitle}pt`, fontWeight: 700 }}>PLUMBING SPECIALTIES</h2><ul className="cf-bullet-list">{formData.specialties.map((s, i) => <li key={i} style={{ fontSize: `${fontSizes.bulletText}pt`, fontWeight: 400 }}>{s.name}</li>)}</ul></section>}
  ...
  const canonicalUrl = "https://professionalresumefree.com/ats-friendly-plumber-resume-builder";
      ...
      <div className="cf-resume-builder" lang="en-US">
        ...
        <nav className="cf-breadcrumb" aria-label="Breadcrumb">
          <ol>
            ...
            <li>
              <Link href="/ats-friendly-plumber-resume-builder" className="cf-breadcrumb-link" prefetch={false}>
                <span className="cf-breadcrumb-text">Free Plumbing Template</span>
              </Link>
            </li>
          </ol>
        </nav>
          ...
          <div className="cf-container">
            <div className="cf-hero-content">
              ...
              <p className="cf-hero-subtitle">
                Create a <strong className="cf-hero-highlight">professional, ATS-optimized plumbing resume for free in minutes.</strong> Our plumber resume builder ensures your technical skills and certifications get noticed by contractors and plumbing companies.
              </p>
              ...
              <div className="cf-cta-buttons">
                ...
                >
                  <span className="cf-btn-text">Start Building Your Plumber Resume Now</span>
                ...
                >
                  ...
                  <span className="cf-btn-text">Download Plumber Resume PDF</span>
                </button>
              </div>
              ...
              <div className="cf-hero-stats">
                <div className="cf-stat-item">
                  ...
                  <span className="cf-stat-label">Plumbing Professionals</span>
                </div>
                <div className="cf-stat-item">
              ...
              <div className="cf-industry-badges">
                <div className="cf-badge-grid">
                  <span className="cf-badge-item"><FiTool /> Apprentice Templates</span>
                  <span className="cf-badge-item"><FiTool /> Journeyman Templates</span>
                  <span className="cf-badge-item"><FiKey /> Master Plumber CVs</span>
                  <span className="cf-badge-item"><FiDroplet /> Pipefitter Resumes</span>
                  <span className="cf-badge-item"><FiThermometer /> Service Plumber</span>
                  <span className="cf-badge-item"><FiHardDrive /> Plumbing Supervisor</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="cf-layout">
          <div className="cf-preview-section">
            ...
            <div className={`cf-preview-container ${showFullPreview ? 'cf-full-preview' : ''}`}><div className="cf-resume-card"><div className="cf-preview-content"><div className="cf-resume-preview" ref={resumeRef}><PlumberTemplate formData={formData}/></div></div></div></div>
          </div>
          <div className="cf-form-section">
            <div className="cf-form-nav">
              {[{id:'personal',label:'Personal',icon:<FiUser/>},{id:'experience',label:'Plumbing Exp.',icon:<FiTool/>},{id:'education',label:'Education',icon:<FiBook/>},{id:'skills',label:'Skills & Specialties',icon:<FiActivity/>},{id:'licenses',label:'Licenses & Certs',icon:<FiShield/>},{id:'settings',label:'Font Settings',icon:<FiSettings/>}].map(item => (<button key={item.id} className={`cf-nav-btn ${activeSection===item.id?'cf-nav-btn-active':''}`} onClick={()=>setActiveSection(item.id)}>{item.icon}<span>{item.label}</span></button>))}
            </div>
            <div className="cf-form-content">
              {activeSection==='personal'&&(
                <div className="cf-form-section-content">
                  ...
                  <div className="cf-form-card">
                    <div className="cf-form-group"><label className="cf-form-label">Full Name*<input type="text" name="fullName" value={formData.fullName} onChange={handleInputChange} placeholder="John Smith, Master Plumber" required className="cf-form-input"/></label><label className="cf-form-label">Email*<input type="email" name="email" value={formData.email} onChange={handleInputChange} placeholder="john.smith@plumbing.com" required className="cf-form-input"/></label></div>
                    <div className="cf-form-group"><label className="cf-form-label">Phone*<input type="tel" name="phone" value={formData.phone} onChange={handleInputChange} placeholder="(555) 123-4567" required className="cf-form-input"/></label><label className="cf-form-label">Location<input type="text" name="address" value={formData.address} onChange={handleInputChange} placeholder="City, State, Country" className="cf-form-input"/></label></div>
                  </div>
                  <div className="cf-form-card"><label className="cf-form-label">Professional Plumbing Summary*<textarea name="summary" value={formData.summary} onChange={handleInputChange} placeholder="Licensed Master Plumber with 10+ years of experience in residential and commercial plumbing..." required className="cf-form-textarea" rows="6"/><div className="cf-char-count">{formData.summary.length}/500 characters</div></label></div>
                  <div className="cf-form-card">
                    <h3 className="cf-sub-section-title"><FiGlobe/> Professional Links</h3><p className="cf-section-desc">Add your professional plumbing profiles</p>
                    <div className="cf-social-input">
                      <select value={currentSocialLink.platform} onChange={e=>setCurrentSocialLink({...currentSocialLink,platform:e.target.value})} className="cf-form-select"><option value="">Select Platform</option><option value="LinkedIn">LinkedIn</option><option value="Portfolio">Professional Portfolio</option><option value="Website">Business Website</option><option value="Facebook">Facebook Business</option></select>
              ...
              {activeSection==='experience'&&(
                <div className="cf-form-section-content">
                  <h2 className="cf-form-section-title"><FiTool/> Plumbing Experience</h2>
                  <p className="cf-section-desc">List your plumbing positions in reverse chronological order (most recent first)</p>
                  <div className="cf-form-card">
                    <div className="cf-form-group"><label className="cf-form-label">Position Title*<input value={currentExperience.position} onChange={e=>setCurrentExperience({...currentExperience,position:e.target.value})} placeholder="Journeyman Plumber, Master Plumber" required className="cf-form-input"/></label><label className="cf-form-label">Plumbing Company/Contractor*<input value={currentExperience.employer} onChange={e=>setCurrentExperience({...currentExperience,employer:e.target.value})} placeholder="Quality Plumbing Services Inc." required className="cf-form-input"/></label></div>
                    <label className="cf-form-label">Project Type / Specialty<input value={currentExperience.projectType} onChange={e=>setCurrentExperience({...currentExperience,projectType:e.target.value})} placeholder="Residential Plumbing, Commercial Construction" className="cf-form-input"/></label>
                    ...
                    <label className="cf-form-label">Key Plumbing Responsibilities & Achievements*<textarea value={currentExperience.description} onChange={e=>setCurrentExperience({...currentExperience,description:e.target.value})} placeholder="• Installed and repaired plumbing systems in 50+ residential buildings..." required className="cf-form-textarea" rows="8"/><div className="cf-char-count">{currentExperience.description.length}/2000 characters</div></label>
                    <div className="cf-form-actions"><button onClick={addExperience} className="cf-add-btn" disabled={!currentExperience.position||!currentExperience.employer||!currentExperience.startDate}><FiPlus/> {currentExperience.isEditing?'Update Plumbing Experience':'Add Plumbing Experience'}</button>{currentExperience.isEditing&&<button onClick={()=>setCurrentExperience(defaultExperience())} className="cf-cancel-btn"><FiX/> Cancel</button>}</div>
                  </div>
                  <div className="cf-form-card"><h3 className="cf-sub-section-title">Your Plumbing Experience</h3>{formData.experience.length===0?<p className="cf-empty-msg">No plumbing experience added yet</p>:<div className="cf-items-list">{formData.experience.map((exp,i)=>(<div key={i} className="cf-list-item"><div className="cf-item-content"><div className="cf-item-header"><strong className="cf-item-title">{exp.position}</strong><span className="cf-item-subtitle">at {exp.employer}</span></div><div className="cf-item-meta"><span>{exp.startDate} – {exp.endDate||'Present'}</span>{exp.projectType&&<span>{exp.projectType}</span>}</div><div className="cf-item-desc">{exp.description.split('\n').filter(l=>l.trim()).map((line,j)=><p key={j} className="cf-bullet-point">• {line}</p>)}</div></div><div className="cf-item-actions"><button onClick={()=>editExperience(i)} className="cf-edit-btn"><FiEdit2/></button><button onClick={()=>deleteExperience(i)} className="cf-delete-btn"><FiTrash2/></button></div></div>))}</div>}</div>
                </div>
              )}
              {activeSection==='education'&&(
                <div className="cf-form-section-content">
                  <h2 className="cf-form-section-title"><FiBook/> Education & Training</h2>
                  <p className="cf-section-desc">List your plumbing education, apprenticeships, and relevant training</p>
                  <div className="cf-form-card">
                    <div className="cf-form-group"><label className="cf-form-label">Institution/Training Center*<input value={currentEducation.institution} onChange={e=>setCurrentEducation({...currentEducation,institution:e.target.value})} placeholder="United Association Plumbing Apprenticeship Program" required className="cf-form-input"/></label><label className="cf-form-label">Degree/Certification/Program*<input value={currentEducation.degree} onChange={e=>setCurrentEducation({...currentEducation,degree:e.target.value})} placeholder="Plumbing Apprenticeship" required className="cf-form-input"/></label></div>
                    <label className="cf-form-label">Specialization / Focus Area<input value={currentEducation.program} onChange={e=>setCurrentEducation({...currentEducation,program:e.target.value})} placeholder="Commercial Plumbing, Pipefitting" className="cf-form-input"/></label>
              ...
              {activeSection==='skills'&&(
                <div className="cf-form-section-content">
                  ...
                  <div className="cf-form-card">
                    <h3 className="cf-sub-section-title">Technical Plumbing Skills</h3><p className="cf-section-desc">List your technical skills and equipment proficiency</p>
                    <div className="cf-skills-input">
                      <input value={currentSkill.name} onChange={e=>setCurrentSkill({...currentSkill,name:e.target.value})} placeholder="Pipe Threading, Soldering, Blueprint Reading" className="cf-form-input"/>
              ...
              {activeSection==='licenses'&&(
                <div className="cf-form-section-content">
                  ...
                  <div className="cf-form-card">
                    <h3 className="cf-sub-section-title">Professional Plumbing Licenses</h3><p className="cf-section-desc">Add your professional plumbing licenses</p>
                    <div className="cf-skills-input">
                      <div className="cf-form-group"><label className="cf-form-label">License Name*<input value={currentLicense.name} onChange={e=>setCurrentLicense({...currentLicense,name:e.target.value})} placeholder="Master Plumber License" className="cf-form-input"/></label><label className="cf-form-label">Issuing Authority<input value={currentLicense.issuingAuthority} onChange={e=>setCurrentLicense({...currentLicense,issuingAuthority:e.target.value})} placeholder="State Plumbing Board" className="cf-form-input"/></label></div>
                      <div className="cf-form-group"><label className="cf-form-label">License Number<input value={currentLicense.licenseNumber} onChange={e=>setCurrentLicense({...currentLicense,licenseNumber:e.target.value})} placeholder="MP1234567" className="cf-form-input"/></label><label className="cf-form-label">Expiry Date<input value={currentLicense.expiryDate} onChange={e=>setCurrentLicense({...currentLicense,expiryDate:e.target.value})} placeholder="Month Year" className="cf-form-input"/></label></div>
                  ...
                  <div className="cf-form-card">
                    <h3 className="cf-sub-section-title">Professional Certifications</h3><p className="cf-section-desc">Add your plumbing certifications and training certificates</p>
                    <div className="cf-skills-input">
                      <div className="cf-form-group"><label className="cf-form-label">Certification Name*<input value={currentCertification.certification} onChange={e=>setCurrentCertification({...currentCertification,certification:e.target.value})} placeholder="Backflow Prevention Certification" className="cf-form-input"/></label><label className="cf-form-label">Issuing Organization<input value={currentCertification.organization} onChange={e=>setCurrentCertification({...currentCertification,organization:e.target.value})} placeholder="American Society of Plumbing Engineers" className="cf-form-input"/></label></div>
                      <div className="cf-form-actions"><button onClick={addCertification} className="cf-add-btn" disabled={!currentCertification.certification.trim()}><FiPlus/> {currentCertification.isEditing?'Update Certification':'Add Certification'}</button>{currentCertification.isEditing&&<button onClick={()=>setCurrentCertification(defaultCertification())} className="cf-cancel-btn"><FiX/> Cancel</button>}</div>
                    </div>
                    <div className="cf-items-list">{formData.certifications.map((c,i)=>(<div key={i} className="cf-list-item"><div><strong>{c.certification}</strong>{c.organization&&` – ${c.organization}`}</div><div className="cf-item-actions"><button onClick={()=>editCertification(i)} className="cf-edit-btn"><FiEdit2/></button><button onClick={()=>deleteCertification(i)} className="cf-delete-btn"><FiTrash2/></button></div></div>))}{formData.certifications.length===0&&<p className="cf-empty-msg">No certifications added yet</p>}</div>
                  </div>
                </div>
              )}
        ...
        <section className="cf-cta-section" aria-labelledby="cta-title">
          <div className="cf-container">
            <div className="cf-cta-content">
              <h2 className="cf-cta-title" id="cta-title">Ready to Advance Your Plumbing Career?</h2>
              ...
              <div className="cf-cta-btn-wrap">
                ...
                >
                  <span className="cf-cta-btn-text">Create Your Free Plumber Resume Now</span>
        ...
        {/* Full Preview Modal */}
        {showFullPreview&&(<div className="cf-modal" onClick={()=>setShowFullPreview(false)}><div className="cf-modal-content" onClick={e=>e.stopPropagation()}><div className="cf-modal-header"><h3>Full Plumber Resume Preview</h3><button className="cf-close-btn" onClick={()=>setShowFullPreview(false)}><FiX/></button></div><div className="cf-modal-pages"><div className="cf-modal-page"><PlumberTemplate formData={formData}/></div></div></div></div>)}
