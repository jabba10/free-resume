...
const warehouseResumeStyles = `
  ...
  * { box-sizing: border-box; }
  ...
  .cf-warehouse-template { color: #000000; line-height: 1.25; height: 100%; }
  ...
  .cf-certification-item, .cf-safety-item { margin-bottom: 3pt; line-height: 1.25; }
  ...
  .cf-modal-page .cf-warehouse-template { width: 100%; height: 100%; padding: 10mm 15mm; box-sizing: border-box; overflow-y: auto; }
  ...
  @media (max-width: 768px) {
    ...
    .cf-modal-page .cf-warehouse-template { padding: 8mm 12mm; }
  ...
  @media (max-width: 480px) {
    ...
    .cf-modal-page .cf-warehouse-template { padding: 6mm 8mm; }
  ...
  @media print {
    ...
    .cf-warehouse-template, .cf-section { page-break-inside: avoid; }
  }
`;
...
const Resume = ({ seoData, buildTimestamp }) => {
  ...
  const defaultExperience = () => ({ employer: '', position: '', warehouseType: '', startDate: '', endDate: '', description: '', isEditing: false, editIndex: null });
  const defaultEducation = () => ({ institution: '', degree: '', program: '', startDate: '', endDate: '', isEditing: false, editIndex: null });
  const defaultSkill = () => ({ name: '', level: '', isEditing: false, editIndex: null });
  ...
  const defaultSafety = () => ({ training: '', certification: '', isEditing: false, editIndex: null });
  ...
  const [formData, setFormData] = useState({ fullName: '', email: '', phone: '', address: '', summary: '', experience: [], education: [], skills: [], certifications: [], equipment: [], safetyTrainings: [], languages: [], socialLinks: [] });
  ...
  const faqs = [
    { question: "Is this warehouse resume builder really free with no hidden costs?", answer: "Yes, our warehouse resume builder is completely free with no hidden costs or watermarks. Create, edit, and download your professional warehouse resume in PDF format without any payment required." },
    { question: "What does ATS-friendly mean for warehouse resumes?", answer: "ATS-friendly means our warehouse resume templates are optimized to pass through Applicant Tracking Systems used by 95% of logistics companies and warehouses. This ensures your warehouse experience and skills are properly scanned and recognized." },
    { question: "Can I download my warehouse resume as PDF without creating an account?", answer: "Absolutely! Download your professional warehouse resume in PDF format without creating an account. Everything is completely free and accessible immediately for forklift operators, warehouse associates, and logistics professionals." }
  ];
  ...
  const hasContent = () => formData.fullName || formData.email || formData.summary || formData.socialLinks.length > 0 || formData.experience.length > 0 || formData.education.length > 0 || formData.skills.length > 0 || formData.certifications.length > 0 || formData.equipment.length > 0 || formData.safetyTrainings.length > 0 || formData.languages.length > 0;
  ...
  const addSafety = createAddFunction('safetyTrainings', currentSafety, setCurrentSafety, defaultSafety, () => currentSafety.training.trim());
  const editSafety = createEditFunction('safetyTrainings', setCurrentSafety);
  const deleteSafety = createDeleteFunction('safetyTrainings');
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      pdf.save(`${formData.fullName || 'warehouse_resume'}_resume.pdf`);
      setShowSharePopup(true);
    } catch (err) { console.error("PDF Error:", err); alert("Failed to generate PDF."); }
    finally { setIsGeneratingPDF(false); }
  };

  const WarehouseTemplate = ({ formData }) => {
    ...
    const hasSafety = formData.safetyTrainings.length > 0;
    const hasLanguages = formData.languages.length > 0;
    return (
      <div className="cf-warehouse-template">
        ...
        {hasSummary && <section className="cf-section"><h2 className="cf-section-title" style={{ fontSize: `${fontSizes.sectionTitle}pt`, fontWeight: 700 }}>WAREHOUSE PROFESSIONAL SUMMARY</h2><p className="cf-summary-text" style={{ fontSize: `${fontSizes.regularText}pt`, fontWeight: 400 }}>{formData.summary}</p></section>}
        {hasExperience && <section className="cf-section"><h2 className="cf-section-title" style={{ fontSize: `${fontSizes.sectionTitle}pt`, fontWeight: 700 }}>WAREHOUSE EXPERIENCE</h2>{formData.experience.map((exp, i) => (<div key={i} className="cf-experience-item"><div className="cf-experience-header"><h3 style={{ fontSize: `${fontSizes.jobTitle}pt`, fontWeight: 700 }}>{exp.position}</h3><p className="cf-company" style={{ fontSize: `${fontSizes.company}pt`, fontWeight: 400 }}>{exp.employer}{exp.warehouseType && ` – ${exp.warehouseType}`} | {exp.startDate} – {exp.endDate || 'Present'}</p></div><ul className="cf-bullet-list">{exp.description.split('\n').filter(l => l.trim()).map((line, j) => <li key={j} style={{ fontSize: `${fontSizes.bulletText}pt`, fontWeight: 400 }}>{line}</li>)}</ul></div>))}</section>}
        ...
        {hasSkills && <section className="cf-section"><h2 className="cf-section-title" style={{ fontSize: `${fontSizes.sectionTitle}pt`, fontWeight: 700 }}>WAREHOUSE SKILLS</h2><ul className="cf-skills-list">{formData.skills.map((s, i) => <li key={i} style={{ fontSize: `${fontSizes.skillText}pt`, fontWeight: 400 }}>{s.name}{s.level && ` (${s.level})`}</li>)}</ul></section>}
        {hasEquipment && <section className="cf-section"><h2 className="cf-section-title" style={{ fontSize: `${fontSizes.sectionTitle}pt`, fontWeight: 700 }}>EQUIPMENT & MACHINERY</h2><ul className="cf-skills-list">{formData.equipment.map((eq, i) => <li key={i} style={{ fontSize: `${fontSizes.skillText}pt`, fontWeight: 400 }}>{eq.name}{eq.yearsExperience && ` – ${eq.yearsExperience} years`}</li>)}</ul></section>}
        {hasCertifications && <section className="cf-section"><h2 className="cf-section-title" style={{ fontSize: `${fontSizes.sectionTitle}pt`, fontWeight: 700 }}>CERTIFICATIONS & LICENSES</h2>{formData.certifications.map((c, i) => (<div key={i} className="cf-certification-item" style={{ fontSize: `${fontSizes.certificationText}pt`, fontWeight: 400 }}><strong style={{ fontWeight: 700 }}>{c.name}</strong>{c.issuingAuthority && ` – ${c.issuingAuthority}`}{c.certificationNumber && ` (Cert #: ${c.certificationNumber})`}{c.expiryDate && ` – Expires: ${c.expiryDate}`}</div>))}</section>}
        {hasSafety && <section className="cf-section"><h2 className="cf-section-title" style={{ fontSize: `${fontSizes.sectionTitle}pt`, fontWeight: 700 }}>SAFETY TRAINING & COMPLIANCE</h2>{formData.safetyTrainings.map((s, i) => (<div key={i} className="cf-safety-item" style={{ fontSize: `${fontSizes.regularText}pt`, fontWeight: 400 }}><strong style={{ fontWeight: 700 }}>{s.training}</strong>{s.certification && ` – ${s.certification}`}</div>))}</section>}
  ...
  const canonicalUrl = "https://professionalresumefree.com/ats-friendly-warehouse-worker-resume-builder";

  return (
    <>
      <style>{warehouseResumeStyles}</style>
      <div className="cf-resume-builder" lang="en-US">
        <Head>
          <title>Free Warehouse Resume Builder 2026: ATS Templates for Logistics Pros</title>
          <meta name="title" content="Free Warehouse Resume Builder 2026: ATS Templates for Logistics Pros" />
          <meta name="description" content="Create your professional warehouse resume for free in 2026. ATS-optimized templates help logistics professionals highlight equipment skills, safety training & experience. Start now—no sign-up." />
          ...
          <meta property="og:title" content="Free Warehouse Resume Builder 2026: ATS Templates for Logistics Pros" />
          <meta property="og:description" content="Create your professional warehouse resume for free in 2026. ATS-optimized templates help logistics professionals highlight equipment skills, safety training & experience. Start now—no sign-up." />
          <meta property="og:image" content="https://professionalresumefree.com/images/og-warehouse-resume-builder-preview.jpg" />
          ...
          <meta name="twitter:title" content="Free Warehouse Resume Builder 2026: ATS Templates for Logistics Pros" />
          <meta name="twitter:description" content="Create your professional warehouse resume for free in 2026. ATS-optimized templates help logistics professionals highlight equipment skills, safety training & experience. Start now—no sign-up." />
          <meta name="twitter:image" content="https://professionalresumefree.com/images/twitter-warehouse-resume-builder-preview.jpg" />
                ...
                "@graph": [
                  {
                    ...
                    "url": canonicalUrl,
                    "name": "Free Warehouse Resume Builder 2026 - ATS Templates for Logistics Pros",
                    "description": "Create professional ATS-optimized warehouse resumes for free. Land interviews faster with our logistics resume builder.",
                    ...
                    "isPartOf": {
                      ...
                      "name": "Professional Warehouse Resume Free",
                      "description": "Free online resume builder for warehouse professionals",
                      "publisher": {
                        "@type": "Organization",
                    ...
                    "primaryImageOfPage": {
                      "@type": "ImageObject",
                      "url": "https://professionalresumefree.com/images/og-warehouse-resume-builder-preview.jpg",
                      "width": 1200,
                      "height": 630
                    },
                    "breadcrumb": {
                    ...
                    "mainEntity": {
                      ...
                      "name": "Warehouse Resume Builder - ATS Optimized Logistics Resume Maker",
                      ...
                      "description": "Free online ATS-friendly warehouse resume builder for logistics professionals, forklift operators, warehouse associates, and distribution center staff.",
                      "featureList": [
                        "Warehouse ATS-Optimized Templates",
                        "Logistics Content Suggestions",
                        "One-Click PDF Download",
                        "Equipment Experience Formatting",
                        "Mobile-Friendly Editor",
                        "No Sign Up Required",
                        "Free Forever"
                      ],
                      "softwareVersion": "2026.1.0",
                      "screenshot": "https://professionalresumefree.com/images/screenshot-warehouse-resume-builder.jpg",
                  ...
                  {
                    "@type": "HowTo",
                    "name": "How to Create a Professional Warehouse Resume with Our Free Builder",
                    "description": "Step-by-step guide to create an ATS-optimized warehouse resume for free",
                    "totalTime": "PT15M",
                    "estimatedCost": {
                      "@type": "MonetaryAmount",
                      "currency": "USD",
                    ...
                    "step": [
                      {
                        ...
                        "image": "https://professionalresumefree.com/images/step1-warehouse-template.jpg"
                      },
                      {
                        "@type": "HowToStep",
                        "position": 2,
                        ...
                        "image": "https://professionalresumefree.com/images/step2-warehouse-info.jpg"
                      },
                      {
                        "@type": "HowToStep",
                        "position": 3,
                      ...
                      },
                      {
                        ...
                        "text": "Export your professional warehouse resume as PDF - completely free, no watermarks.",
                  ...
                  {
                    "@type": "Service",
                    "serviceType": "Online Warehouse Resume Building Service",
                    "provider": {
                      "@type": "Organization",
                      "name": "Professional Warehouse Resume Free",
                      "url": "https://professionalresumefree.com",
                    ...
                    "hasOfferCatalog": {
                      "@type": "OfferCatalog",
                      "name": "Free Warehouse Resume Building Services",
                      "itemListElement": [
                        {
                          "@type": "Offer",
                          "itemOffered": {
                            "@type": "Service",
                            "name": "Warehouse ATS Resume Templates"
                          }
                        },
                        {
                          "@type": "Offer",
                          "itemOffered": {
                            "@type": "Service",
                            "name": "Logistics Resume Editing"
                          }
                        }
                      ]
                    },
                    "description": "Free ATS-friendly warehouse resume builder for logistics professionals worldwide",
                    "offers": {
                      "@type": "Offer",
                      "price": "0",
                      "priceCurrency": "USD"
        ...
        <nav className="cf-breadcrumb" aria-label="Breadcrumb"><ol><li><Link href="/" className="cf-breadcrumb-link"><FiHome className="cf-breadcrumb-icon"/><span className="cf-breadcrumb-text">Home</span></Link></li><li className="cf-breadcrumb-separator"><FiChevronRightIcon/></li><li><span className="cf-breadcrumb-text">Free Logistics Templates</span></li></ol></nav>

        <section className="cf-hero">
          <div className="cf-container">
            <div className="cf-hero-content">
              <div className="cf-trust-badge"><FiStar className="cf-trust-icon"/><span className="cf-trust-text">Best Free Warehouse Resume Builder 2026</span></div>
              <h1 className="cf-hero-title">Free Warehouse Resume Builder 2026</h1>
              <p className="cf-hero-subtitle">Create a <strong className="cf-hero-highlight">professional, ATS-optimized warehouse resume for free in minutes.</strong> Our warehouse resume builder helps you highlight equipment skills, safety training, and logistics experience that impress hiring managers.</p>
              <div className="cf-cta-buttons">
                <button onClick={() => setActiveSection('personal')} className="cf-btn-primary" aria-label="Start building your free warehouse resume now—no sign-up required">
                  <span className="cf-btn-text">Start Building Your Warehouse Resume Now</span>
                  <FiArrowRight className="cf-btn-icon" />
                  <div className="cf-btn-pulse"></div>
                </button>
                <button onClick={generatePDF} className="cf-btn-secondary" disabled={isGeneratingPDF || !hasContent()} aria-label="Download warehouse resume as PDF"><FiDownload className="cf-btn-icon"/><span className="cf-btn-text">Download Warehouse Resume PDF</span></button>
              </div>
              <div className="cf-industry-badges"><div className="cf-badge-grid"><span className="cf-badge-item"><FiTruck/> Forklift Operator</span><span className="cf-badge-item"><FiPackage/> Order Picker</span><span className="cf-badge-item"><FiGrid/> Inventory Specialist</span><span className="cf-badge-item"><FiBriefcase/> Warehouse Supervisor</span><span className="cf-badge-item"><FiBox/> Shipping/Receiving</span><span className="cf-badge-item"><FiActivity/> Logistics Coordinator</span></div></div>
            </div>
          </div>
        </section>
        ...
        <div className="cf-layout">
          <div className="cf-preview-section">
            ...
            <div className={`cf-preview-container ${showFullPreview ? 'cf-full-preview' : ''}`}><div className="cf-resume-card"><div className="cf-preview-content"><div className="cf-resume-preview" ref={resumeRef}><WarehouseTemplate formData={formData}/></div></div></div></div>
          </div>
          <div className="cf-form-section">
            <div className="cf-form-nav">
              {[{id:'personal',label:'Personal',icon:<FiUser/>},{id:'experience',label:'Warehouse Exp.',icon:<FiPackage/>},{id:'education',label:'Education',icon:<FiBook/>},{id:'skills',label:'Skills & Equip.',icon:<FiTool/>},{id:'certifications',label:'Certifications',icon:<FiShield/>},{id:'settings',label:'Font Settings',icon:<FiSettings/>}].map(item => (<button key={item.id} className={`cf-nav-btn ${activeSection===item.id?'cf-nav-btn-active':''}`} onClick={()=>setActiveSection(item.id)}>{item.icon}<span>{item.label}</span></button>))}
            </div>
            <div className="cf-form-content">
              {activeSection==='personal'&&(
                <div className="cf-form-section-content">
                  ...
                  <div className="cf-form-card">
                    <div className="cf-form-group"><label className="cf-form-label">Full Name*<input type="text" name="fullName" value={formData.fullName} onChange={handleInputChange} placeholder="John Smith, Certified Forklift Operator" required className="cf-form-input"/></label><label className="cf-form-label">Email*<input type="email" name="email" value={formData.email} onChange={handleInputChange} placeholder="john.smith@example.com" required className="cf-form-input"/></label></div>
                  ...
                  <div className="cf-form-card"><label className="cf-form-label">Warehouse Professional Summary*<textarea name="summary" value={formData.summary} onChange={handleInputChange} placeholder="Reliable and safety-conscious Warehouse Professional with 5+ years..." required className="cf-form-textarea" rows="6"/><div className="cf-char-count">{formData.summary.length}/500 characters</div></label></div>
                  <div className="cf-form-card">
                    <h4 className="cf-sub-section-title"><FiGlobe/> Professional Links</h4><p className="cf-section-desc">Add your professional profiles</p>
              ...
              {activeSection==='experience'&&(
                <div className="cf-form-section-content">
                  <h3 className="cf-form-section-title"><FiPackage/> Warehouse Experience</h3>
                  <p className="cf-section-desc">List your warehouse positions in reverse chronological order (most recent first)</p>
                  <div className="cf-form-card">
                    <div className="cf-form-group"><label className="cf-form-label">Position Title*<input value={currentExperience.position} onChange={e=>setCurrentExperience({...currentExperience,position:e.target.value})} placeholder="Forklift Operator, Order Picker, Warehouse Associate" required className="cf-form-input"/></label><label className="cf-form-label">Company/Warehouse*<input value={currentExperience.employer} onChange={e=>setCurrentExperience({...currentExperience,employer:e.target.value})} placeholder="Amazon Fulfillment Center" required className="cf-form-input"/></label></div>
                    <label className="cf-form-label">Warehouse Type / Facility<input value={currentExperience.warehouseType} onChange={e=>setCurrentExperience({...currentExperience,warehouseType:e.target.value})} placeholder="Distribution Center, Fulfillment Center, Cold Storage" className="cf-form-input"/></label>
                    ...
                    <label className="cf-form-label">Key Responsibilities & Achievements*<textarea value={currentExperience.description} onChange={e=>setCurrentExperience({...currentExperience,description:e.target.value})} placeholder="• Operated sit-down and stand-up forklifts..." required className="cf-form-textarea" rows="8"/><div className="cf-char-count">{currentExperience.description.length}/2000 characters</div></label>
                    <div className="cf-form-actions"><button onClick={addExperience} className="cf-add-btn" disabled={!currentExperience.position||!currentExperience.employer||!currentExperience.startDate}><FiPlus/> {currentExperience.isEditing?'Update Warehouse Experience':'Add Warehouse Experience'}</button>{currentExperience.isEditing&&<button onClick={()=>setCurrentExperience(defaultExperience())} className="cf-cancel-btn"><FiX/> Cancel</button>}</div>
                  </div>
                  <div className="cf-form-card"><h4 className="cf-sub-section-title">Your Warehouse Experience</h4>{formData.experience.length===0?<p className="cf-empty-msg">No warehouse experience added yet</p>:<div className="cf-items-list">{formData.experience.map((exp,i)=>(<div key={i} className="cf-list-item"><div className="cf-item-content"><div className="cf-item-header"><strong className="cf-item-title">{exp.position}</strong><span className="cf-item-subtitle">at {exp.employer}</span></div><div className="cf-item-meta"><span>{exp.startDate} – {exp.endDate||'Present'}</span>{exp.warehouseType&&<span>{exp.warehouseType}</span>}</div><div className="cf-item-desc">{exp.description.split('\n').filter(l=>l.trim()).map((line,j)=><p key={j} className="cf-bullet-point">• {line}</p>)}</div></div><div className="cf-item-actions"><button onClick={()=>editExperience(i)} className="cf-edit-btn"><FiEdit2/></button><button onClick={()=>deleteExperience(i)} className="cf-delete-btn"><FiTrash2/></button></div></div>))}</div>}</div>
                </div>
              )}
              {activeSection==='education'&&(
                <div className="cf-form-section-content">
                  <h3 className="cf-form-section-title"><FiBook/> Education & Training</h3>
                  <p className="cf-section-desc">List your education, degrees, and relevant warehouse training</p>
                  <div className="cf-form-card">
                    <div className="cf-form-group"><label className="cf-form-label">Institution*<input value={currentEducation.institution} onChange={e=>setCurrentEducation({...currentEducation,institution:e.target.value})} placeholder="Logistics Training Institute" required className="cf-form-input"/></label><label className="cf-form-label">Degree/Certification*<input value={currentEducation.degree} onChange={e=>setCurrentEducation({...currentEducation,degree:e.target.value})} placeholder="Warehouse Operations Certificate" required className="cf-form-input"/></label></div>
                    <label className="cf-form-label">Program / Specialization<input value={currentEducation.program} onChange={e=>setCurrentEducation({...currentEducation,program:e.target.value})} placeholder="Inventory Management, Logistics Technology" className="cf-form-input"/></label>
              ...
              {activeSection==='skills'&&(
                <div className="cf-form-section-content">
                  <h3 className="cf-form-section-title"><FiTool/> Warehouse Skills & Equipment</h3>
                  <div className="cf-form-card">
                    <h4 className="cf-sub-section-title">Warehouse Skills</h4><p className="cf-section-desc">List your warehouse skills and proficiency levels</p>
                    <div className="cf-skills-input">
                      <div className="cf-form-group"><label className="cf-form-label">Skill Name*<input value={currentSkill.name} onChange={e=>setCurrentSkill({...currentSkill,name:e.target.value})} placeholder="Inventory Management, Order Picking, Quality Control" className="cf-form-input"/></label><label className="cf-form-label">Proficiency Level<input value={currentSkill.level} onChange={e=>setCurrentSkill({...currentSkill,level:e.target.value})} placeholder="Expert, Advanced, Intermediate" className="cf-form-input"/></label></div>
                    ...
                    <div className="cf-items-list">{formData.skills.map((s,i)=>(<div key={i} className="cf-list-item"><span>{s.name}{s.level&&` (${s.level})`}</span><div className="cf-item-actions"><button onClick={()=>editSkill(i)} className="cf-edit-btn"><FiEdit2/></button><button onClick={()=>deleteSkill(i)} className="cf-delete-btn"><FiTrash2/></button></div></div>))}{formData.skills.length===0&&<p className="cf-empty-msg">No skills added yet</p>}</div>
                  </div>
                  <div className="cf-form-card">
                    <h4 className="cf-sub-section-title"><FiTruck/> Equipment & Machinery</h4><p className="cf-section-desc">Add warehouse equipment you're trained to operate</p>
                    <div className="cf-skills-input">
                      <div className="cf-form-group"><label className="cf-form-label">Equipment Name*<input value={currentEquipment.name} onChange={e=>setCurrentEquipment({...currentEquipment,name:e.target.value})} placeholder="Forklift (Counterbalance, Reach, Order Picker)" className="cf-form-input"/></label><label className="cf-form-label">Years Experience<input value={currentEquipment.yearsExperience} onChange={e=>setCurrentEquipment({...currentEquipment,yearsExperience:e.target.value})} placeholder="5 years, 2+ years" className="cf-form-input"/></label></div>
                    ...
                    <div className="cf-items-list">{formData.equipment.map((eq,i)=>(<div key={i} className="cf-list-item"><div><strong>{eq.name}</strong>{eq.yearsExperience&&` – ${eq.yearsExperience}`}</div><div className="cf-item-actions"><button onClick={()=>editEquipment(i)} className="cf-edit-btn"><FiEdit2/></button><button onClick={()=>deleteEquipment(i)} className="cf-delete-btn"><FiTrash2/></button></div></div>))}{formData.equipment.length===0&&<p className="cf-empty-msg">No equipment added yet</p>}</div>
                  </div>
                  <div className="cf-form-card">
                    <h4 className="cf-sub-section-title"><FiShield/> Safety Training & Compliance</h4><p className="cf-section-desc">List your safety training and certifications</p>
                    <div className="cf-skills-input">
                      <div className="cf-form-group"><label className="cf-form-label">Training Name*<input value={currentSafety.training} onChange={e=>setCurrentSafety({...currentSafety,training:e.target.value})} placeholder="OSHA Warehouse Safety, Forklift Safety" className="cf-form-input"/></label><label className="cf-form-label">Certification<input value={currentSafety.certification} onChange={e=>setCurrentSafety({...currentSafety,certification:e.target.value})} placeholder="OSHA 10-Hour, First Aid/CPR" className="cf-form-input"/></label></div>
                      <div className="cf-form-actions"><button onClick={addSafety} className="cf-add-btn" disabled={!currentSafety.training.trim()}><FiPlus/> {currentSafety.isEditing?'Update Training':'Add Training'}</button>{currentSafety.isEditing&&<button onClick={()=>setCurrentSafety(defaultSafety())} className="cf-cancel-btn"><FiX/> Cancel</button>}</div>
                    </div>
                    <div className="cf-items-list">{formData.safetyTrainings.map((s,i)=>(<div key={i} className="cf-list-item"><div><strong>{s.training}</strong>{s.certification&&` – ${s.certification}`}</div><div className="cf-item-actions"><button onClick={()=>editSafety(i)} className="cf-edit-btn"><FiEdit2/></button><button onClick={()=>deleteSafety(i)} className="cf-delete-btn"><FiTrash2/></button></div></div>))}{formData.safetyTrainings.length===0&&<p className="cf-empty-msg">No safety training added yet</p>}</div>
                  </div>
                  <div className="cf-form-card">
                    ...
                    <div className="cf-skills-input">
                      <div className="cf-form-group"><label className="cf-form-label">Language*<input value={currentLanguage.name} onChange={e=>setCurrentLanguage({...currentLanguage,name:e.target.value})} placeholder="Spanish" className="cf-form-input"/></label><label className="cf-form-label">Proficiency Level<input value={currentLanguage.proficiency} onChange={e=>setCurrentLanguage({...currentLanguage,proficiency:e.target.value})} placeholder="Fluent, Conversational, Basic" className="cf-form-input"/></label></div>
              ...
              {activeSection==='certifications'&&(
                <div className="cf-form-section-content">
                  ...
                  <div className="cf-form-card">
                    <h4 className="cf-sub-section-title">Professional Certifications</h4><p className="cf-section-desc">Add your warehouse and logistics certifications</p>
                    <div className="cf-skills-input">
                      <div className="cf-form-group"><label className="cf-form-label">Certification Name*<input value={currentCertification.name} onChange={e=>setCurrentCertification({...currentCertification,name:e.target.value})} placeholder="Forklift Operator Certification" className="cf-form-input"/></label><label className="cf-form-label">Issuing Authority<input value={currentCertification.issuingAuthority} onChange={e=>setCurrentCertification({...currentCertification,issuingAuthority:e.target.value})} placeholder="OSHA, Company Name" className="cf-form-input"/></label></div>
                      <div className="cf-form-group"><label className="cf-form-label">Certification Number<input value={currentCertification.certificationNumber} onChange={e=>setCurrentCertification({...currentCertification,certificationNumber:e.target.value})} placeholder="FL1234567" className="cf-form-input"/></label><label className="cf-form-label">Expiry Date<input value={currentCertification.expiryDate} onChange={e=>setCurrentCertification({...currentCertification,expiryDate:e.target.value})} placeholder="Month Year" className="cf-form-input"/></label></div>
        ...
        <section className="cf-faq-section" aria-labelledby="faq-title"><div className="cf-container"><div className="cf-section-header"><h2 className="cf-section-title" id="faq-title">Frequently Asked Questions</h2><p>Everything you need to know about creating professional warehouse resumes with our tool.</p></div><div className="cf-faq-grid">{faqs.map((faq,i)=>(<div key={i} className="cf-faq-item"><h3 className="cf-faq-question">{faq.question}</h3><p className="cf-faq-answer">{faq.answer}</p></div>))}</div></div></section>

        <section className="cf-cta-section" aria-labelledby="cta-title"><div className="cf-container"><div className="cf-cta-content"><h2 className="cf-cta-title" id="cta-title">Ready to Advance Your Warehouse Career?</h2><p className="cf-cta-subtitle">Join 2 million+ warehouse professionals who landed their dream jobs with our free ATS-friendly warehouse resume builder.</p><div className="cf-cta-btn-wrap"><button onClick={()=>setActiveSection('personal')} className="cf-cta-btn" aria-label="Create your free warehouse resume now—no sign-up required"><span className="cf-cta-btn-text">Create Your Free Warehouse Resume Now</span><FiArrowRight className="cf-cta-btn-icon"/></button></div><div className="cf-cta-guarantee"><FiCheck className="cf-guarantee-icon"/><span className="cf-guarantee-text">No credit card required • Free forever • Download in minutes • ATS Optimized for Warehouses</span></div></div></div></section>

        {showFullPreview&&(<div className="cf-modal" onClick={()=>setShowFullPreview(false)}><div className="cf-modal-content" onClick={e=>e.stopPropagation()}><div className="cf-modal-header"><h3>Full Warehouse Resume Preview</h3><button className="cf-close-btn" onClick={()=>setShowFullPreview(false)}><FiX/></button></div><div className="cf-modal-pages"><div className="cf-modal-page"><WarehouseTemplate formData={formData}/></div></div></div></div>)}
...
export async function getStaticProps() {
  ...
  const breadcrumbData = [
    ...
    { name: 'Warehouse Resume Builder', item: 'https://professionalresumefree.com/ats-friendly-warehouse-worker-resume-builder' }
  ];
