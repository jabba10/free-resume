...
const Resume = ({ seoData, buildTimestamp }) => {
  ...
  const defaultAchievement = () => ({ title: '', description: '', year: '', isEditing: false, editIndex: null });
  const defaultProject = () => ({ name: '', description: '', technologies: '', year: '', isEditing: false, editIndex: null });
  ...
  const [formData, setFormData] = useState({ fullName: '', email: '', phone: '', address: '', professionalTitle: '', summary: '', experience: [], education: [], skills: [], certifications: [], achievements: [], projects: [], languages: [], socialLinks: [] });
  const [fontSizes, setFontSizes] = useState({ name: 16, professionalTitle: 11, sectionTitle: 11, contactInfo: 8, jobTitle: 10, company: 8, degree: 10, institution: 8, institutionDate: 7, regularText: 9, bulletText: 9, skillText: 8, certificationText: 9 });
  ...
  const industryCategories = ['Technology','Business & Management','Finance & Accounting','Marketing & Sales','Engineering','Healthcare','Education','Creative Arts','Operations','Customer Service','Legal','Science & Research','Other'];
  const faqs = [
  ...
  const resetFontSizes = () => setFontSizes({ name: 16, professionalTitle: 11, sectionTitle: 11, contactInfo: 8, jobTitle: 10, company: 8, degree: 10, institution: 8, institutionDate: 7, regularText: 9, bulletText: 9, skillText: 8, certificationText: 9 });
  const getSocialIcon = () => <FiGlobe />;
  const formatSocialUrl = (url) => url ? url.replace(/(https?:\/\/)?(www\.)?/, '').replace(/\/$/, '') : '';
  const handleInputChange = (e) => setFormData({ ...formData, [e.target.name]: e.target.value });
  const hasContent = () => formData.fullName || formData.email || formData.summary || formData.socialLinks.length > 0 || formData.experience.length > 0 || formData.education.length > 0 || formData.skills.length > 0 || formData.certifications.length > 0 || formData.achievements.length > 0 || formData.projects.length > 0 || formData.languages.length > 0;
    ...
    return (
      <div className="cf-universal-template">
        <header className="cf-resume-header">
          ...
          {formData.professionalTitle && <h2 className="cf-professional-title" style={{ fontSize: `${fontSizes.professionalTitle}pt`, fontWeight: 400 }}>{formData.professionalTitle}</h2>}
        ...
        {hasSkills && <section className="cf-section"><h2 className="cf-section-title" style={{ fontSize: `${fontSizes.sectionTitle}pt`, fontWeight: 700 }}>SKILLS</h2><ul className="cf-skills-list">{formData.skills.map((s, i) => <li key={i} style={{ fontSize: `${fontSizes.skillText}pt`, fontWeight: 400 }}>{s.name}{s.category && ` (${s.category})`}</li>)}</ul></section>}
        {hasCertifications && <section className="cf-section"><h2 className="cf-section-title" style={{ fontSize: `${fontSizes.sectionTitle}pt`, fontWeight: 700 }}>CERTIFICATIONS</h2>{formData.certifications.map((c, i) => (<div key={i} className="cf-cert-item" style={{ fontSize: `${fontSizes.certificationText}pt`, fontWeight: 400 }}><strong style={{ fontWeight: 700 }}>{c.name}</strong>{c.issuingAuthority && ` – ${c.issuingAuthority}`}{c.credentialNumber && ` (Credential #: ${c.credentialNumber})`}{c.expiryDate && ` – Expires: ${c.expiryDate}`}</div>))}</section>}
        {hasAchievements && <section className="cf-section"><h2 className="cf-section-title" style={{ fontSize: `${fontSizes.sectionTitle}pt`, fontWeight: 700 }}>ACHIEVEMENTS</h2><ul className="cf-bullet-list">{formData.achievements.map((a, i) => (<li key={i} style={{ fontSize: `${fontSizes.bulletText}pt`, fontWeight: 400 }}><strong style={{ fontWeight: 700 }}>{a.title}</strong>{a.year && ` (${a.year})`}{a.description && ` – ${a.description}`}</li>))}</ul></section>}
        {hasProjects && <section className="cf-section"><h2 className="cf-section-title" style={{ fontSize: `${fontSizes.sectionTitle}pt`, fontWeight: 700 }}>PROJECTS</h2>{formData.projects.map((p, i) => (<div key={i} className="cf-experience-item"><div className="cf-experience-header"><h3 style={{ fontSize: `${fontSizes.jobTitle}pt`, fontWeight: 700 }}>{p.name}</h3>{p.year && <p className="cf-company" style={{ fontSize: `${fontSizes.company}pt`, fontWeight: 400 }}>{p.year}</p>}</div>{p.description && <p className="cf-summary-text" style={{ fontSize: `${fontSizes.regularText}pt`, fontWeight: 400 }}>{p.description}</p>}{p.technologies && <p className="cf-company" style={{ fontSize: `${fontSizes.company}pt`, fontWeight: 400 }}><strong style={{ fontWeight: 700 }}>Technologies:</strong> {p.technologies}</p>}</div>))}</section>}
  ...
  const canonicalUrl = "https://professionalresumefree.com/free-resume-builder";
  ...
  return (
    <>
      ...
      <div className="cf-resume-builder" lang="en-US">
                ...
                "@graph": [
                  {
                    ...
                    "primaryImageOfPage": {
                      "@type": "ImageObject",
                      "url": "https://professionalresumefree.com/images/og-resume-builder-preview.jpg",
                      "width": 1200,
                      "height": 630
                    },
                    "breadcrumb": {
                    ...
                    "mainEntity": {
                      ...
                      "description": "Free online ATS-friendly resume builder for professionals, job seekers, and career changers.",
                      "featureList": [
                    ...
                    "step": [
                      {
                        ...
                        "text": "Select from our ATS-optimized resume templates designed for professionals across all industries.",
                        "url": `${canonicalUrl}#templates`,
                        "image": "https://professionalresumefree.com/images/step1-resume-template.jpg"
                      },
                      {
                        "@type": "HowToStep",
                        "position": 2,
                        "name": "Enter Your Professional Information",
                        "text": "Add your experience, education, skills, certifications, and achievements using our guided forms.",
                        "url": `${canonicalUrl}#editor`,
                        "image": "https://professionalresumefree.com/images/step2-resume-info.jpg"
                      },
                      {
                        "@type": "HowToStep",
                        "position": 3,
                        "name": "Customize and Optimize",
                        "text": "Use our suggestions to improve keywords and formatting for ATS compatibility.",
                  ...
                  },
                  {
                    ...
                    "description": "Free ATS-friendly resume builder for professionals worldwide",
                    "offers": {
                      "@type": "Offer",
                      "price": "0",
                      "priceCurrency": "USD"
        ...
        <nav className="cf-breadcrumb" aria-label="Breadcrumb"><ol><li><Link href="/" className="cf-breadcrumb-link"><FiHome className="cf-breadcrumb-icon"/><span className="cf-breadcrumb-text">Home</span></Link></li><li className="cf-breadcrumb-separator"><FiChevronRightIcon/></li><li><span className="cf-breadcrumb-text">Free Professional Template</span></li></ol></nav>
            ...
            <div className="cf-hero-content">
              ...
              <div className="cf-cta-buttons">
                <button onClick={() => setActiveSection('personal')} className="cf-btn-primary" aria-label="Start building your free professional resume now—no sign-up required">
                  <span className="cf-btn-text">Start Building Your Resume Now</span>
                  <FiArrowRight className="cf-btn-icon" />
                  <div className="cf-btn-pulse"></div>
                </button>
                <button onClick={generatePDF} className="cf-btn-secondary" disabled={isGeneratingPDF || !hasContent()} aria-label="Download resume as PDF"><FiDownload className="cf-btn-icon"/><span className="cf-btn-text">Download Resume PDF</span></button>
              </div>
              <div className="cf-industry-badges"><div className="cf-badge-grid"><span className="cf-badge-item"><FiCode/> Tech</span><span className="cf-badge-item"><FiBriefcase/> Business</span><span className="cf-badge-item"><FiDollarSign/> Finance</span><span className="cf-badge-item"><FiShoppingBag/> Marketing</span><span className="cf-badge-item"><FiTool/> Engineering</span><span className="cf-badge-item"><FiHeart/> Healthcare</span><span className="cf-badge-item"><FiMonitor/> Creative</span><span className="cf-badge-item"><FiTruck/> Operations</span></div></div>
            </div>
          </div>
        </section>
            ...
            <div className="cf-form-content">
              {activeSection==='personal'&&(
                <div className="cf-form-section-content">
                  ...
                  <div className="cf-form-card">
                    <div className="cf-form-group"><label className="cf-form-label">Full Name*<input type="text" name="fullName" value={formData.fullName} onChange={handleInputChange} placeholder="John Smith" required className="cf-form-input"/></label><label className="cf-form-label">Professional Title<input type="text" name="professionalTitle" value={formData.professionalTitle} onChange={handleInputChange} placeholder="Senior Project Manager | MBA" className="cf-form-input"/></label></div>
                    <div className="cf-form-group"><label className="cf-form-label">Email*<input type="email" name="email" value={formData.email} onChange={handleInputChange} placeholder="john.smith@email.com" required className="cf-form-input"/></label><label className="cf-form-label">Phone<input type="tel" name="phone" value={formData.phone} onChange={handleInputChange} placeholder="(555) 123-4567" className="cf-form-input"/></label></div>
                    <label className="cf-form-label">Location<input type="text" name="address" value={formData.address} onChange={handleInputChange} placeholder="City, State" className="cf-form-input"/></label>
                  </div>
                  <div className="cf-form-card"><label className="cf-form-label">Professional Summary*<textarea name="summary" value={formData.summary} onChange={handleInputChange} placeholder="Results-driven professional with 8+ years of experience..." required className="cf-form-textarea" rows="6"/><div className="cf-char-count">{formData.summary.length}/500 characters</div></label></div>
                  <div className="cf-form-card">
                    ...
                    <div className="cf-social-input">
                      <select value={currentSocialLink.platform} onChange={e=>setCurrentSocialLink({...currentSocialLink,platform:e.target.value})} className="cf-form-select"><option value="">Select Platform</option><option value="LinkedIn">LinkedIn</option><option value="GitHub">GitHub</option><option value="Portfolio">Portfolio</option><option value="Website">Website</option><option value="Twitter">Twitter/X</option><option value="Dribbble">Dribbble</option></select>
              ...
              {activeSection==='experience'&&(
                <div className="cf-form-section-content">
                  ...
                  <p className="cf-section-desc">List your positions in reverse chronological order (most recent first)</p>
                  <div className="cf-form-card">
                    <div className="cf-form-group"><label className="cf-form-label">Position Title*<input value={currentExperience.position} onChange={e=>setCurrentExperience({...currentExperience,position:e.target.value})} placeholder="Senior Project Manager" required className="cf-form-input"/></label><label className="cf-form-label">Company/Organization*<input value={currentExperience.employer} onChange={e=>setCurrentExperience({...currentExperience,employer:e.target.value})} placeholder="Tech Corporation Inc." required className="cf-form-input"/></label></div>
                    <label className="cf-form-label">Department / Division / Team<input value={currentExperience.department} onChange={e=>setCurrentExperience({...currentExperience,department:e.target.value})} placeholder="Product Development / Engineering" className="cf-form-input"/></label>
                    ...
                    <label className="cf-form-label">Key Responsibilities & Achievements*<textarea value={currentExperience.description} onChange={e=>setCurrentExperience({...currentExperience,description:e.target.value})} placeholder="• Managed a team of 12 developers..." required className="cf-form-textarea" rows="8"/><div className="cf-char-count">{currentExperience.description.length}/2000 characters</div></label>
                  ...
                  <div className="cf-form-card">
                    <h4 className="cf-sub-section-title"><FiAward/> Key Achievements</h4><p className="cf-section-desc">Highlight significant accomplishments, awards, or recognitions</p>
                    <div className="cf-skills-input">
                      <div className="cf-form-group"><label className="cf-form-label">Achievement Title*<input value={currentAchievement.title} onChange={e=>setCurrentAchievement({...currentAchievement,title:e.target.value})} placeholder="Project of the Year Award" className="cf-form-input"/></label><label className="cf-form-label">Year<input value={currentAchievement.year} onChange={e=>setCurrentAchievement({...currentAchievement,year:e.target.value})} placeholder="2023" className="cf-form-input"/></label></div>
                      <label className="cf-form-label">Description<textarea value={currentAchievement.description} onChange={e=>setCurrentAchievement({...currentAchievement,description:e.target.value})} placeholder="Recognized for leading a critical project..." className="cf-form-textarea" rows="3"/></label>
              ...
              {activeSection==='education'&&(
                <div className="cf-form-section-content">
                  ...
                  <div className="cf-form-card">
                    <div className="cf-form-group"><label className="cf-form-label">Institution*<input value={currentEducation.institution} onChange={e=>setCurrentEducation({...currentEducation,institution:e.target.value})} placeholder="Stanford University" required className="cf-form-input"/></label><label className="cf-form-label">Degree/Certification*<input value={currentEducation.degree} onChange={e=>setCurrentEducation({...currentEducation,degree:e.target.value})} placeholder="Master of Business Administration (MBA)" required className="cf-form-input"/></label></div>
                    <label className="cf-form-label">Program / Specialization<input value={currentEducation.program} onChange={e=>setCurrentEducation({...currentEducation,program:e.target.value})} placeholder="Finance & Strategy" className="cf-form-input"/></label>
              ...
              {activeSection==='skills'&&(
                <div className="cf-form-section-content">
                  <h3 className="cf-form-section-title"><FiTool/> Skills & Expertise</h3>
                  <div className="cf-form-card">
                    <h4 className="cf-sub-section-title">Professional Skills</h4><p className="cf-section-desc">List your technical and soft skills by category</p>
                    <div className="cf-skills-input">
                      <div className="cf-form-group"><label className="cf-form-label">Skill Name*<input value={currentSkill.name} onChange={e=>setCurrentSkill({...currentSkill,name:e.target.value})} placeholder="Project Management" className="cf-form-input"/></label><label className="cf-form-label">Category<select value={currentSkill.category} onChange={e=>setCurrentSkill({...currentSkill,category:e.target.value})} className="cf-form-select"><option value="">Select Category</option>{industryCategories.map(cat=><option key={cat} value={cat}>{cat}</option>)}</select></label></div>
                    ...
                    <div className="cf-items-list">{formData.skills.map((s,i)=>(<div key={i} className="cf-list-item"><div>{s.name}{s.category&&<span className="cf-skill-cat"> ({s.category})</span>}</div><div className="cf-item-actions"><button onClick={()=>editSkill(i)} className="cf-edit-btn"><FiEdit2/></button><button onClick={()=>deleteSkill(i)} className="cf-delete-btn"><FiTrash2/></button></div></div>))}{formData.skills.length===0&&<p className="cf-empty-msg">No skills added yet</p>}</div>
                  </div>
                  <div className="cf-form-card">
                    <h4 className="cf-sub-section-title"><FiAward/> Certifications</h4><p className="cf-section-desc">Add your professional certifications and credentials</p>
                    <div className="cf-skills-input">
                      <div className="cf-form-group"><label className="cf-form-label">Certification Name*<input value={currentCertification.name} onChange={e=>setCurrentCertification({...currentCertification,name:e.target.value})} placeholder="Project Management Professional (PMP)" className="cf-form-input"/></label><label className="cf-form-label">Issuing Authority<input value={currentCertification.issuingAuthority} onChange={e=>setCurrentCertification({...currentCertification,issuingAuthority:e.target.value})} placeholder="Project Management Institute" className="cf-form-input"/></label></div>
                      <div className="cf-form-group"><label className="cf-form-label">Credential Number<input value={currentCertification.credentialNumber} onChange={e=>setCurrentCertification({...currentCertification,credentialNumber:e.target.value})} placeholder="PMP1234567" className="cf-form-input"/></label><label className="cf-form-label">Expiry Date<input value={currentCertification.expiryDate} onChange={e=>setCurrentCertification({...currentCertification,expiryDate:e.target.value})} placeholder="Month Year" className="cf-form-input"/></label></div>
                  ...
                  <div className="cf-form-card">
                    <h4 className="cf-sub-section-title">Notable Projects</h4><p className="cf-section-desc">Highlight key projects that demonstrate your capabilities</p>
                    <div className="cf-skills-input">
                      <div className="cf-form-group"><label className="cf-form-label">Project Name*<input value={currentProject.name} onChange={e=>setCurrentProject({...currentProject,name:e.target.value})} placeholder="Enterprise CRM Implementation" className="cf-form-input"/></label><label className="cf-form-label">Year<input value={currentProject.year} onChange={e=>setCurrentProject({...currentProject,year:e.target.value})} placeholder="2023" className="cf-form-input"/></label></div>
                      <label className="cf-form-label">Project Description<textarea value={currentProject.description} onChange={e=>setCurrentProject({...currentProject,description:e.target.value})} placeholder="Led the implementation of a new CRM system..." className="cf-form-textarea" rows="3"/></label>
                      <label className="cf-form-label">Technologies / Tools Used<input value={currentProject.technologies} onChange={e=>setCurrentProject({...currentProject,technologies:e.target.value})} placeholder="Salesforce, Jira, Python, SQL" className="cf-form-input"/></label>
                    ...
                    <div className="cf-items-list">{formData.projects.map((p,i)=>(<div key={i} className="cf-list-item"><div><strong>{p.name}</strong>{p.year&&` (${p.year})`}{p.description&&<div className="cf-item-desc">{p.description}</div>}{p.technologies&&<div className="cf-item-meta"><strong>Technologies:</strong> {p.technologies}</div>}</div><div className="cf-item-actions"><button onClick={()=>editProject(i)} className="cf-edit-btn"><FiEdit2/></button><button onClick={()=>deleteProject(i)} className="cf-delete-btn"><FiTrash2/></button></div></div>))}{formData.projects.length===0&&<p className="cf-empty-msg">No projects added yet</p>}</div>
                  </div>
                  <div className="cf-form-card">
              ...
              {activeSection==='certifications'&&(
                <div className="cf-form-section-content">
                  <h3 className="cf-form-section-title"><FiAward/> Certifications & Credentials</h3>
                  <p className="cf-section-desc">Showcase your professional certifications, licenses, and credentials</p>
                  <div className="cf-form-card">
                    <div className="cf-form-group"><label className="cf-form-label">Certification Name*<input value={currentCertification.name} onChange={e=>setCurrentCertification({...currentCertification,name:e.target.value})} placeholder="Certified Scrum Master (CSM)" className="cf-form-input"/></label><label className="cf-form-label">Issuing Authority<input value={currentCertification.issuingAuthority} onChange={e=>setCurrentCertification({...currentCertification,issuingAuthority:e.target.value})} placeholder="Scrum Alliance" className="cf-form-input"/></label></div>
                    <div className="cf-form-group"><label className="cf-form-label">Credential Number<input value={currentCertification.credentialNumber} onChange={e=>setCurrentCertification({...currentCertification,credentialNumber:e.target.value})} placeholder="CSM123456" className="cf-form-input"/></label><label className="cf-form-label">Expiry Date<input value={currentCertification.expiryDate} onChange={e=>setCurrentCertification({...currentCertification,expiryDate:e.target.value})} placeholder="Month Year" className="cf-form-input"/></label></div>
                    <div className="cf-form-actions"><button onClick={addCertification} className="cf-add-btn" disabled={!currentCertification.name.trim()}><FiPlus/> {currentCertification.isEditing?'Update Certification':'Add Certification'}</button>{currentCertification.isEditing&&<button onClick={()=>setCurrentCertification(defaultCertification())} className="cf-cancel-btn"><FiX/> Cancel</button>}</div>
                  </div>
                  <div className="cf-form-card"><h4 className="cf-sub-section-title">Your Certifications</h4>{formData.certifications.length===0?<p className="cf-empty-msg">No certifications added yet</p>:<div className="cf-items-list">{formData.certifications.map((c,i)=>(<div key={i} className="cf-list-item"><div><strong>{c.name}</strong>{c.issuingAuthority&&` – ${c.issuingAuthority}`}{c.credentialNumber&&` (#${c.credentialNumber})`}{c.expiryDate&&` – Expires: ${c.expiryDate}`}</div><div className="cf-item-actions"><button onClick={()=>editCertification(i)} className="cf-edit-btn"><FiEdit2/></button><button onClick={()=>deleteCertification(i)} className="cf-delete-btn"><FiTrash2/></button></div></div>))}</div>}</div>
                </div>
              )}
                    ...
                    <div className="cf-font-grid">
                      {[{key:'name',label:'Name'},{key:'professionalTitle',label:'Professional Title'},{key:'sectionTitle',label:'Section Titles'},{key:'jobTitle',label:'Job Titles'},{key:'degree',label:'Degrees'},{key:'institution',label:'Company/Institution Names'},{key:'institutionDate',label:'Institution Dates'},{key:'regularText',label:'Regular Text'},{key:'bulletText',label:'Bullet Points'},{key:'contactInfo',label:'Contact Info'},{key:'skillText',label:'Skills Text'},{key:'certificationText',label:'Certification Text'}].map(({key,label})=>(
                        <div key={key} className="cf-font-control">
                          ...
                          <input type="range" min={key==='institutionDate'?6:8} max={key==='name'?24:key==='professionalTitle'||key==='sectionTitle'?18:key==='jobTitle'||key==='degree'?16:14} value={fontSizes[key]} onChange={e=>handleFontSizeChange(key,e.target.value)} className="cf-font-slider"/>
                        </div>
                      ))}
                    </div>
        ...
        <section className="cf-faq-section" aria-labelledby="faq-title"><div className="cf-container"><div className="cf-section-header"><h2 className="cf-section-title" id="faq-title">Frequently Asked Questions</h2><p>Everything you need to know about creating professional resumes with our tool.</p></div><div className="cf-faq-grid">{faqs.map((faq,i)=>(<div key={i} className="cf-faq-item"><h3 className="cf-faq-question">{faq.question}</h3><p className="cf-faq-answer">{faq.answer}</p></div>))}</div></div></section>
...
export async function getStaticProps() {
  ...
  const breadcrumbData = [
    ...
    { name: 'Resume Builder', item: 'https://professionalresumefree.com/free-resume-builder' }
  ];
